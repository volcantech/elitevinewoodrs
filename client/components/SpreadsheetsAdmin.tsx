...
export function SpreadsheetsAdmin({ permissions }: SpreadsheetsAdminProps) {
  ...
  useEffect(() => {
    setRowSearch(""); setTablePage(1);
    setCompanions(Array.isArray(currentSheet?.companions) ? (currentSheet!.companions as CompanionEntry[]) : []);
  }, [currentSheet?.id]);
  ...
  const saveCompanions = async (newCompanions: CompanionEntry[]) => {
    ...
    try {
      ...
      if (res.ok) {
        const updated = await res.json();
        setCurrentSheet(prev => prev ? { ...prev, companions: updated.companions ?? newCompanions } : prev);
        setSheets(prev => prev.map(s => s.id === currentSheet.id ? { ...s, companions: updated.companions ?? newCompanions } : s));
  ...
  if (currentSheet) {
    ...
    const totalAdminPages = Math.max(1, Math.ceil(filteredAdminRows.length / _PAGE_SIZE));
    const pagedAdminRows = filteredAdminRows.slice((tablePage - 1) * _PAGE_SIZE, tablePage * _PAGE_SIZE);
      ...
      <div className="space-y-4">
        ...
        {isPublic && (
          <div className="space-y-2">
            ...
            {canEdit && (() => {
              const LAYOUT_ZONES: { pos: CompanionEntry["layout"]; label: string }[] = [
                { pos: "above", label: "Au-dessus" },
                { pos: "left",  label: "À gauche" },
                { pos: "right", label: "À droite" },
                { pos: "below", label: "En dessous" },
              ];
              ...
              const anyDragging = draggingCompanionIdx !== null;

              const renderLayoutPicker = (idx: number, layout: CompanionEntry["layout"]) => {
                const companionTitle = sheets.find(s => s.share_token === companions[idx]?.token)?.title ?? "";

                const makeZone = (pos: CompanionEntry["layout"]) => {
                  ...
                  return (
                    <div
                      ...
                      onDrop={() => {
                        ...
                        const updated = companions.map((c, i) => i === idx ? { ...c, layout: pos } : c);
                        setCompanions(updated);
                        saveCompanions(updated);
                      }}
                      className={`flex items-center justify-center rounded transition-all
                        ${isHoriz ? "w-[72px] h-12" : "w-full h-7"}
                        ${isActive ? "bg-blue-500/20 border-2 border-blue-500/60"
                          : anyDragging ? "bg-slate-700/50 border-2 border-dashed border-slate-600"
                          : "bg-slate-800/30 border border-slate-700/40"}`}
                    >
                      {isActive ? (
                        <span
                          draggable
                          onDragStart={() => setDraggingCompanionIdx(idx)}
                          onDragEnd={() => setDraggingCompanionIdx(null)}
                          className="text-[10px] text-blue-300 bg-blue-900/50 border border-blue-500/50 rounded px-1 py-0.5 cursor-grab active:cursor-grabbing truncate max-w-[64px] select-none leading-tight"
                        >
                          {shortTitle(companionTitle)}
                        </span>
                      ) : anyDragging ? (
                        <span className="text-[10px] text-slate-500 leading-tight">{LAYOUT_ZONES.find(z => z.pos === pos)?.label}</span>
                      ) : null}
                    </div>
                  );
                };
                ...
                return (
                  <div className="inline-flex flex-col items-center gap-0.5 p-1.5 bg-slate-900/60 rounded-lg border border-slate-700/50 select-none">
                    <div className="w-full flex justify-center">{makeZone("above")}</div>
                    <div className="flex items-center gap-0.5">
                      {makeZone("left")}
                      <div className="w-[72px] h-12 bg-amber-500/20 border-2 border-amber-500/40 rounded flex items-center justify-center shrink-0">
                        <span className="text-[10px] text-amber-300 font-semibold text-center px-1 leading-tight">{shortTitle(currentSheet.title)}</span>
                      </div>
                      {makeZone("right")}
                    </div>
                    <div className="w-full flex justify-center">{makeZone("below")}</div>
                  </div>
                );
              };
              ...
              return (
                <div className="space-y-2">
                  ...
                  {companions.map((c, idx) => {
                    const s = sheets.find(sh => sh.share_token === c.token);
                    return (
                      <div key={c.token} className="p-2 bg-slate-800/50 border border-slate-700/50 rounded-lg space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs text-gray-300 font-medium truncate">{s?.title ?? c.token.slice(0, 12) + "…"}</span>
                          <button
                            onClick={() => {
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          {renderLayoutPicker(idx, c.layout)}
                          <span className="text-[10px] text-gray-500 italic">
                            {LAYOUT_ZONES.find(z => z.pos === c.layout)?.label ?? c.layout}
                          </span>
                        </div>
                      </div>
                    );
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        onChange={e => {
                          ...
                          const updated = [...companions, { token: e.target.value, layout: "below" as const }];
                          setCompanions(updated);
                          saveCompanions(updated);
                        }}
                        ...
                        <td key={col.id} className="px-2 py-1 border-r border-slate-700/30 align-middle min-w-[140px]">
                          {isEditing ? (
                            col.type === "date" ? (
                              ...
                              <Input type={col.type === "number" ? "number" : "text"} value={cellDraft}
                                onChange={e => setCellDraft(e.target.value)}
                                onBlur={() => commitCell(col)}
                                onKeyDown={e => { if (e.key === "Tab") { handleTab(e, col, row.id); return; } if (e.key === "Enter") commitCell(col); if (e.key === "Escape") setEditingCell(null); }}
                                className="h-7 text-sm bg-slate-700 border-amber-500/50 text-white w-full" autoFocus />
                            )
                          ) : col.type === "checkbox" ? (
