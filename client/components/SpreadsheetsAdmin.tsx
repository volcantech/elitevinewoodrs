...
export function SpreadsheetsAdmin({ permissions }: SpreadsheetsAdminProps) {
  ...
  const handleTab = useCallback(async (e: React.KeyboardEvent, col: Column, rowId: number) => {
    ...
    if (nextIdx >= 0 && nextIdx < cols.length) {
      ...
      const row = currentSheet.rows?.find(r => r.id === rowId);
      if (row) {
        setTimeout(() => startEditCell(rowId, nextCol, row.data[nextCol.id]), 40);
      }
    }
  }, [cellDraft, linkDraft, currentSheet, saveCellValue, startEditCell]);
                    ...
                    <th key={col.id} className="px-3 py-3 text-left border-b border-slate-700/50 border-r border-slate-700/50 min-w-[140px]">
                      <div className="flex items-center justify-between gap-1">
                        <div className="flex items-center gap-1 min-w-0">
                          <span className="text-sm font-semibold text-amber-300 truncate">{col.name}</span>
                        ...
                        {canEdit && (
                          <div className="flex items-center gap-0.5 shrink-0">
                            <button
                              ...
                              className="text-gray-600 hover:text-amber-400 transition-all disabled:opacity-20 disabled:cursor-not-allowed p-0.5">
                              <ChevronLeft className="w-3 h-3" />
                            </button>
                            <button
                              onClick={() => moveColumn(col.id, "right")}
                              disabled={colIdx === currentSheet.columns.length - 1}
                              title="Déplacer à droite"
                              className="text-gray-600 hover:text-amber-400 transition-all disabled:opacity-20 disabled:cursor-not-allowed p-0.5">
                              <ChevronRight className="w-3 h-3" />
                            </button>
                            <button onClick={() => deleteColumn(col.id)} title="Supprimer" className="text-gray-600 hover:text-red-400 transition-all p-0.5">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        )}
                            ...
                            col.type === "date" ? (
                              ...
                              <select
                                ...
                                onChange={async e => {
                                  ...
                                  if (val) { await saveCellValue(editingCell!.rowId, col.id, val); setEditingCell(null); }
                                }}
                      ...
                      {newColOptions.map((opt, i) => (
                        opt.startsWith("__group__:") ? (
                          <span key={i} draggable
                            ...
                            onDragOver={e => { e.preventDefault(); if (optDragIdx === null || optDragIdx === i) return; setNewColOptions(prev => { const a = [...prev]; const [item] = a.splice(optDragIdx, 1); a.splice(i, 0, item); return a; }); setOptDragIdx(i); }}
                            onDragEnd={() => setOptDragIdx(null)}
                            className={`flex items-center gap-1 bg-amber-500/15 border border-amber-500/40 text-amber-300 text-xs px-2.5 py-1 rounded-md font-semibold cursor-grab select-none transition-opacity ${optDragIdx === i ? "opacity-30" : ""}`}>
                          ...
                          <span key={i} draggable
                            ...
                            onDragOver={e => { e.preventDefault(); if (optDragIdx === null || optDragIdx === i) return; setNewColOptions(prev => { const a = [...prev]; const [item] = a.splice(optDragIdx, 1); a.splice(i, 0, item); return a; }); setOptDragIdx(i); }}
                            onDragEnd={() => setOptDragIdx(null)}
                            className={`flex items-center gap-1 bg-slate-700 text-gray-200 text-xs px-2.5 py-1 rounded-full cursor-grab select-none transition-opacity ${optDragIdx === i ? "opacity-30" : ""}`}>
