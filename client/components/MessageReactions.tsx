          ...
          <div key={g.emoji} className="relative">
            ...
            {tooltipEmoji === g.emoji && g.users.length > 0 && (
              <div className={`absolute bottom-full mb-1 ${isMe ? "right-0" : "left-0"} z-50 pointer-events-none`}>
                <div className="bg-gray-900 border border-gray-700 rounded-lg px-2.5 py-1.5 shadow-xl">
                  <div className="flex items-center gap-1 mb-1">
                    <span className="text-sm leading-none">{g.emoji}</span>
                    <span className="text-[10px] text-gray-500 font-medium">{g.count > 1 ? `${g.count} réactions` : "1 réaction"}</span>
                  </div>
