...
export function VehicleAdminTable({ vehicles, categories, particularities = [], token, onRefresh, onSort, currentSortField, currentSortOrder = "asc", categoryMaxPages = DEFAULT_CATEGORY_MAX_PAGES }: VehicleAdminTableProps) {
  const [editingVehicle, setEditingVehicle] = useState<Vehicle | null>(null);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [formData, setFormData] = useState<Partial<Vehicle>>({});

  const particularityOptions = useMemo(() => {
    const opts = ["Aucune", ...particularities];
