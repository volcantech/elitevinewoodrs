...
import { Pencil, Trash2, ArrowUpDown, Sparkles, Star } from "lucide-react";
...
type SortField = "name" | "price" | "category" | "trunk_weight" | "seats" | "particularity" | "avg_rating";
type SortOrder = "asc" | "desc";

const FALLBACK_PARTICULARITY_OPTIONS = ["Aucune", "Les plus rapides", "Drift", "Suspension hydraulique", "Karting", "Électrique"];
...
export function VehicleAdminTable({ vehicles, categories, token, onRefresh, onSort, currentSortField, currentSortOrder = "asc", categoryMaxPages = DEFAULT_CATEGORY_MAX_PAGES }: VehicleAdminTableProps) {
  ...
  const [particularityOptions, setParticularityOptions] = useState<string[]>(FALLBACK_PARTICULARITY_OPTIONS);

  useEffect(() => {
    fetch("/api/particularities")
      .then(r => r.json())
      .then((data: { id: number; name: string }[]) => {
        const names = data.map(p => p.name);
        setParticularityOptions(["Aucune", ...names]);
      })
      .catch(() => {});
  }, []);
        ...
        <Table>
          <TableHeader className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-b border-amber-600/40">
            <TableRow className="hover:bg-slate-900">
              ...
              <SortHeader field="avg_rating" label="Note" />
          ...
          <TableBody>
            {vehicles.length === 0 ? (
              <TableRow className="hover:bg-slate-900">
                <TableCell colSpan={9} className="text-center py-12 text-amber-200/50">
                  ...
                  <TableCell className="text-white">
                    {vehicle.avg_rating != null ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-amber-300 font-semibold text-sm">{vehicle.avg_rating.toFixed(1)}</span>
                        <span className="text-gray-500 text-xs">({vehicle.review_count})</span>
                      </div>
                    ) : (
