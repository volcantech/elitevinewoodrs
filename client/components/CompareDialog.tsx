...
import { Scale, X, Sparkles, Plus, Search, DollarSign, Package, Users, BookOpen, Star, Car } from "lucide-react";
import type { ReviewSummariesMap } from "@/hooks/useReviewSummaries";
import { getIrlBrand } from "@/data/brands";
              ...
              <div key={idx} className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg overflow-hidden border border-gray-700 hover:border-amber-500/50 transition-all duration-300">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {(() => {
                      const irl = getIrlBrand(vehicle.name);
                      const realName = vehicle.realname;
                      if (!irl && !realName) return null;
                      return (
                        <Badge variant="secondary" className="text-xs bg-blue-500/20 text-blue-300 border-blue-500/30">
                          <Car className="w-3 h-3 mr-1" />
                          {realName ? realName : irl}
                          {realName && irl ? ` (${irl})` : ""}
                        </Badge>
                      );
                    })()}
                  ...
                  {(() => {
                    const review = reviewSummaries[vehicle.id];
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <div className="flex items-center gap-0.5">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                              key={star}
                              className={`w-3.5 h-3.5 ${star <= Math.round(review.average) ? "text-amber-400 fill-amber-400" : "text-gray-600"}`}
                            />
                          ))}
                        </div>
                        <span className="text-xs text-amber-300 font-semibold">{review.average.toFixed(1)}</span>
                        <span className="text-xs text-gray-500">({review.total} avis)</span>
                      </div>
                    );
                  })()}
