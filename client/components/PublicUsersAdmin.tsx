...
const DEFAULT_PERMISSIONS: UserPermissions = {
  vehicles: { view: false, create: false, update: false, delete: false, toggle_categories: false },
...
export function PublicUsersAdmin({ token, currentUser, permissions }: PublicUsersAdminProps) {
  ...
  const openHistoryDialog = async (user: PublicUser) => {
    ...
    try {
      const [reviewsRes, ordersRes, loginsRes] = await Promise.all([
        authenticatedFetch(`/api/admin/public-users/${user.id}/history/reviews`, token),
        authenticatedFetch(`/api/admin/public-users/${user.id}/history/orders`, token),
        authenticatedFetch(`/api/admin/public-users/${user.id}/history/logins`, token),
      ]);
      const reviewsData = await reviewsRes.json();
      const ordersData = await ordersRes.json();
      const loginsData = await loginsRes.json();
      setHistoryReviews(reviewsData.reviews || []);
      setHistoryOrders(ordersData.orders || []);
      setHistoryLogins(loginsData.history || []);
    } catch {
            ...
            <Tabs defaultValue="reviews" className="flex-1 flex flex-col min-h-0">
              ...
              <TabsContent value="reviews" className="flex-1 min-h-0 mt-0">
                  {historyReviews.length === 0 ? (
                    ...
                    <div className="space-y-3 p-1">
                      {historyReviews.slice((historyReviewPage - 1) * HISTORY_PAGE_SIZE, historyReviewPage * HISTORY_PAGE_SIZE).map((review) => (
                    ...
                    {historyReviews.length > HISTORY_PAGE_SIZE && (
                      <div className="flex items-center justify-between px-1 pt-3">
                        <span className="text-xs text-gray-400">Page {historyReviewPage} / {Math.ceil(historyReviews.length / HISTORY_PAGE_SIZE)}</span>
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm" onClick={() => setHistoryReviewPage((p) => Math.max(1, p - 1))} disabled={historyReviewPage === 1} className="h-7 text-xs border-gray-600 text-gray-300 hover:bg-gray-700 disabled:opacity-40"><ChevronLeft className="h-3 w-3" /></Button>
                          <Button variant="outline" size="sm" onClick={() => setHistoryReviewPage((p) => Math.min(Math.ceil(historyReviews.length / HISTORY_PAGE_SIZE), p + 1))} disabled={historyReviewPage >= Math.ceil(historyReviews.length / HISTORY_PAGE_SIZE)} className="h-7 text-xs border-gray-600 text-gray-300 hover:bg-gray-700 disabled:opacity-40"><ChevronRight className="h-3 w-3" /></Button>
                        </div>
                      </div>
                    )}
                    </>
              ...
              <TabsContent value="orders" className="flex-1 min-h-0 mt-0">
                  {historyOrders.length === 0 ? (
                    ...
                    <>
                    <div className="space-y-3 p-1">
                      {historyOrders.slice((historyOrderPage - 1) * HISTORY_PAGE_SIZE, historyOrderPage * HISTORY_PAGE_SIZE).map((order) => (
                    ...
                    {historyOrders.length > HISTORY_PAGE_SIZE && (
                      <div className="flex items-center justify-between px-1 pt-3">
                        <span className="text-xs text-gray-400">Page {historyOrderPage} / {Math.ceil(historyOrders.length / HISTORY_PAGE_SIZE)}</span>
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm" onClick={() => setHistoryOrderPage((p) => Math.max(1, p - 1))} disabled={historyOrderPage === 1} className="h-7 text-xs border-gray-600 text-gray-300 hover:bg-gray-700 disabled:opacity-40"><ChevronLeft className="h-3 w-3" /></Button>
                          <Button variant="outline" size="sm" onClick={() => setHistoryOrderPage((p) => Math.min(Math.ceil(historyOrders.length / HISTORY_PAGE_SIZE), p + 1))} disabled={historyOrderPage >= Math.ceil(historyOrders.length / HISTORY_PAGE_SIZE)} className="h-7 text-xs border-gray-600 text-gray-300 hover:bg-gray-700 disabled:opacity-40"><ChevronRight className="h-3 w-3" /></Button>
                        </div>
                      </div>
                    )}
                    </>
              ...
              <TabsContent value="logins" className="flex-1 min-h-0 mt-0">
                {historyLogins.length === 0 ? (
                  ...
                  <div className="space-y-2 p-1">
                    {historyLogins.slice((historyLoginPage - 1) * HISTORY_PAGE_SIZE, historyLoginPage * HISTORY_PAGE_SIZE).map((evt) => (
                  ...
                  {historyLogins.length > HISTORY_PAGE_SIZE && (
                    <div className="flex items-center justify-between px-1 pt-3">
                      <span className="text-xs text-gray-400">Page {historyLoginPage} / {Math.ceil(historyLogins.length / HISTORY_PAGE_SIZE)}</span>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => setHistoryLoginPage((p) => Math.max(1, p - 1))} disabled={historyLoginPage === 1} className="h-7 text-xs border-gray-600 text-gray-300 hover:bg-gray-700 disabled:opacity-40"><ChevronLeft className="h-3 w-3" /></Button>
                        <Button variant="outline" size="sm" onClick={() => setHistoryLoginPage((p) => Math.min(Math.ceil(historyLogins.length / HISTORY_PAGE_SIZE), p + 1))} disabled={historyLoginPage >= Math.ceil(historyLogins.length / HISTORY_PAGE_SIZE)} className="h-7 text-xs border-gray-600 text-gray-300 hover:bg-gray-700 disabled:opacity-40"><ChevronRight className="h-3 w-3" /></Button>
                      </div>
                    </div>
                  )}
                  </>
      ...
      <Dialog open={adminDialogOpen} onOpenChange={setAdminDialogOpen}>
        <DialogContent className="bg-slate-900 border-amber-600/30 text-white max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            {/* Permissions — uniquement si admin activé */}
            {formIsAdmin && (
              <div className="space-y-4">
                {/* Véhicules */}
                <div className="bg-slate-800/40 rounded-xl p-4 border border-gray-700/50">
