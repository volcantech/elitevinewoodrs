...
const BANNER_OPTIONS: Record<string, string> = {
  amber:  "from-amber-900/80 via-amber-800/40 to-slate-900/80",
  blue:   "from-blue-900/80 via-blue-800/40 to-slate-900/80",
  green:  "from-emerald-900/80 via-emerald-800/40 to-slate-900/80",
  pink:   "from-pink-900/80 via-pink-800/40 to-slate-900/80",
  purple: "from-purple-900/80 via-purple-800/40 to-slate-900/80",
  teal:   "from-teal-900/80 via-teal-800/40 to-slate-900/80",
  red:    "from-red-900/80 via-red-800/40 to-slate-900/80",
  slate:  "from-slate-700/80 via-slate-800/40 to-slate-900/80",
};
...
const DEFAULT_PERMISSIONS: UserPermissions = {
  ...
  badges: { view: false, create: false, edit: false, delete: false, assign: false },
  settings: { view: false, toggle_calls: false },
  spreadsheets: { view: false, create: false, edit: false, delete: false },
...
export function PublicUsersAdmin({ token, currentUser, permissions }: PublicUsersAdminProps) {
  ...
  // Actions (moderation) dialog
  const [actionsDialogUser, setActionsDialogUser] = useState<PublicUser | null>(null);
  const [actionsDialogOpen, setActionsDialogOpen] = useState(false);
  const [actionsForm, setActionsForm] = useState({ is_orders_blocked: false, is_reviews_blocked: false, is_messages_blocked: false, is_calls_blocked: false, is_images_blocked: false, unique_id: "", rp_phone: "", rp_firstname: "", rp_lastname: "", clear_avatar: false, clear_banner: false });
  ...
  const openActionsDialog = (user: PublicUser) => {
    setActionsDialogUser(user);
    setActionsForm({
      is_orders_blocked: user.is_orders_blocked,
      is_reviews_blocked: user.is_reviews_blocked,
      is_messages_blocked: user.is_messages_blocked,
      is_calls_blocked: user.is_calls_blocked,
      is_images_blocked: user.is_images_blocked ?? false,
  ...
  const handleActionsSave = async () => {
    if (!actionsDialogUser) return;
    setActionsLoading(true);
    try {
      const body: any = {
        is_orders_blocked: actionsForm.is_orders_blocked,
        is_reviews_blocked: actionsForm.is_reviews_blocked,
        is_messages_blocked: actionsForm.is_messages_blocked,
        is_calls_blocked: actionsForm.is_calls_blocked,
        is_images_blocked: actionsForm.is_images_blocked,
        unique_id: actionsForm.unique_id.trim() || null,
        rp_phone: actionsForm.rp_phone.trim() || null,
        rp_firstname: actionsForm.rp_firstname.trim() || null,
        rp_lastname: actionsForm.rp_lastname.trim() || null,
        clear_avatar: actionsForm.clear_avatar,
        clear_banner: actionsForm.clear_banner,
      };

      const res = await authenticatedFetch(`/api/admin/public-users/${actionsDialogUser.id}/edit`, token, {
        method: "PATCH",
        body: JSON.stringify(body),
      });
      const data = await res.json();
      ...
      toast.success(`Actions appliquées sur "${actionsDialogUser.username}"`);
      setActionsDialogOpen(false);
      fetchUsers();
    } catch (err: any) {
      toast.error(err.message || "Impossible d'appliquer les actions");
    } finally {
  ...
  const mergePermissions = (userPerms: any): UserPermissions => ({
    ...
    badges: { ...DEFAULT_PERMISSIONS.badges, ...userPerms?.badges },
    settings: { ...DEFAULT_PERMISSIONS.settings, ...userPerms?.settings },
    spreadsheets: { ...DEFAULT_PERMISSIONS.spreadsheets, ...userPerms?.spreadsheets },
    documents: { ...DEFAULT_PERMISSIONS.documents, ...userPerms?.documents },
  });
  ...
  return (
    <div className="space-y-6">
                            ...
                            <DropdownMenuContent align="end" className="bg-slate-900 border-gray-700 text-white w-44">
                              ...
                              {canEdit && (
                                <DropdownMenuItem
                                  ...
                                  className="cursor-pointer hover:bg-orange-500/10 text-orange-300 focus:bg-orange-500/10 focus:text-orange-300"
                                >
      ...
      <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-lg flex items-start gap-3">
        ...
        <div className="text-sm text-gray-400">
          ...
          <ul className="space-y-1 list-disc list-inside">
            <li><span className="text-blue-300 font-medium">Modifier</span> — modifier le pseudo et le mot de passe d'un joueur.</li>
            <li><span className="text-orange-300 font-medium">Profil public</span> — gérer les informations RP, l'identifiant FiveM, les restrictions d'accès et supprimer la photo/bannière.</li>
      ...
      {/* Dialog Actions (modération) */}
      <Dialog open={actionsDialogOpen} onOpenChange={setActionsDialogOpen}>
        <DialogContent className="bg-slate-900 border-orange-600/30 text-white max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-orange-400 flex items-center gap-2">
              <ShieldOff className="w-5 h-5" />
              Profil public — {actionsDialogUser?.username}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-800/40 rounded-xl p-4 border border-gray-700/50 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={actionsForm.rp_firstname}
                    onChange={(e) => setActionsForm((f) => ({ ...f, rp_firstname: e.target.value }))}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={actionsForm.rp_lastname}
                    onChange={(e) => setActionsForm((f) => ({ ...f, rp_lastname: e.target.value }))}
                    className="bg-slate-800/50 border-gray-700 text-white text-sm"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={actionsForm.rp_phone}
                    onChange={(e) => setActionsForm((f) => ({ ...f, rp_phone: e.target.value }))}
                    className="bg-slate-800/50 border-gray-700 text-white text-sm"
                ...
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs">ID Unique FiveM</Label>
                  <div className="relative">
                    <Hash className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
                    <Input
                      value={actionsForm.unique_id}
                      onChange={(e) => setActionsForm((f) => ({ ...f, unique_id: e.target.value.replace(/\D/g, "").slice(0, 7) }))}
                      className="pl-8 bg-slate-800/50 border-gray-700 text-white text-sm font-mono"
                      placeholder="1–7 chiffres"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-800/40 rounded-xl p-4 border border-gray-700/50">
              <p className="text-sm font-semibold text-gray-300 mb-3">Restrictions d'accès</p>
              <div className="grid grid-cols-2 gap-2.5">
                <div
                  className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all ${
                    actionsForm.is_orders_blocked ? "border-orange-500/40 bg-orange-500/5" : "border-gray-700/50 bg-slate-800/30"
                  }`}
                  onClick={() => setActionsForm((f) => ({ ...f, is_orders_blocked: !f.is_orders_blocked }))}
                >
                  <div className="flex-1 mr-2">
                    <p className={`text-sm font-medium ${actionsForm.is_orders_blocked ? "text-orange-300" : "text-gray-300"}`}>Commandes</p>
                    <p className="text-xs text-gray-500 mt-0.5">Empêche de passer des commandes de véhicules</p>
                  </div>
                  <div onClick={(e) => e.stopPropagation()}>
                    <Switch checked={actionsForm.is_orders_blocked} onCheckedChange={(v) => setActionsForm((f) => ({ ...f, is_orders_blocked: v }))} />
                  </div>
                </div>
                <div
                  className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all ${
                    actionsForm.is_reviews_blocked ? "border-orange-500/40 bg-orange-500/5" : "border-gray-700/50 bg-slate-800/30"
                  }`}
                  onClick={() => setActionsForm((f) => ({ ...f, is_reviews_blocked: !f.is_reviews_blocked }))}
                >
                  <div className="flex-1 mr-2">
                    <p className={`text-sm font-medium ${actionsForm.is_reviews_blocked ? "text-orange-300" : "text-gray-300"}`}>Avis</p>
                    <p className="text-xs text-gray-500 mt-0.5">Empêche de publier ou modifier des avis</p>
                  </div>
                  <div onClick={(e) => e.stopPropagation()}>
                    <Switch checked={actionsForm.is_reviews_blocked} onCheckedChange={(v) => setActionsForm((f) => ({ ...f, is_reviews_blocked: v }))} />
                  </div>
                </div>
                <div
                  className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all ${
                    actionsForm.is_messages_blocked ? "border-orange-500/40 bg-orange-500/5" : "border-gray-700/50 bg-slate-800/30"
                  }`}
                  onClick={() => {
                    const newVal = !actionsForm.is_messages_blocked;
                    setActionsForm((f) => ({ ...f, is_messages_blocked: newVal, is_images_blocked: newVal ? true : f.is_images_blocked }));
                  }}
                >
                  <div className="flex-1 mr-2">
                    <p className={`text-sm font-medium ${actionsForm.is_messages_blocked ? "text-orange-300" : "text-gray-300"}`}>Messages</p>
                    <p className="text-xs text-gray-500 mt-0.5">Bloque l'envoi de messages privés et d'images en MP</p>
                  </div>
                  <div onClick={(e) => e.stopPropagation()}>
                    <Switch checked={actionsForm.is_messages_blocked} onCheckedChange={(v) => setActionsForm((f) => ({ ...f, is_messages_blocked: v, is_images_blocked: v ? true : f.is_images_blocked }))} />
                  </div>
                </div>
                <div
                  className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all ${
                    actionsForm.is_images_blocked ? "border-yellow-500/40 bg-yellow-500/5" : "border-gray-700/50 bg-slate-800/30"
                  }`}
                  onClick={() => setActionsForm((f) => ({ ...f, is_images_blocked: !f.is_images_blocked }))}
                >
                  <div className="flex-1 mr-2">
                    <p className={`text-sm font-medium ${actionsForm.is_images_blocked ? "text-yellow-300" : "text-gray-300"}`}>Envoi d'images</p>
                    <p className="text-xs text-gray-500 mt-0.5">Bloque l'envoi d'images en MP et en groupe uniquement</p>
                  </div>
                  <div onClick={(e) => e.stopPropagation()}>
                    <Switch checked={actionsForm.is_images_blocked} onCheckedChange={(v) => setActionsForm((f) => ({ ...f, is_images_blocked: v }))} />
                  </div>
                </div>
                <div
                  className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all ${
                    actionsForm.is_calls_blocked ? "border-red-500/40 bg-red-500/5" : "border-gray-700/50 bg-slate-800/30"
                  }`}
                  onClick={() => setActionsForm((f) => ({ ...f, is_calls_blocked: !f.is_calls_blocked }))}
                >
                  <div className="flex-1 mr-2">
                    <p className={`text-sm font-medium ${actionsForm.is_calls_blocked ? "text-red-300" : "text-gray-300"}`}>Appels vocaux</p>
                    <p className="text-xs text-gray-500 mt-0.5">Bloque les appels vocaux privés et de groupe</p>
                  </div>
                  <div onClick={(e) => e.stopPropagation()}>
                    <Switch checked={actionsForm.is_calls_blocked} onCheckedChange={(v) => setActionsForm((f) => ({ ...f, is_calls_blocked: v }))} />
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Médias de profil */}
            <div className="bg-slate-800/40 rounded-xl p-4 border border-gray-700/50 space-y-3">
              <p className="text-sm font-semibold text-gray-300 mb-1">Médias de profil</p>
              {actionsDialogUser?.avatar_url ? (
                <div className="flex items-center gap-3 p-3 rounded-lg border border-gray-700/50 bg-slate-800/30">
                  <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-600 shrink-0">
                    <img src={actionsDialogUser.avatar_url} alt={actionsDialogUser.username} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1">
                    {actionsForm.clear_avatar ? (
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-red-300">Photo sera supprimée</span>
                        <button type="button" onClick={() => setActionsForm((f) => ({ ...f, clear_avatar: false }))} className="text-xs text-gray-400 underline hover:text-gray-200">Annuler</button>
                      </div>
                    ) : (
                      <button
                        type="button"
                        onClick={() => setActionsForm((f) => ({ ...f, clear_avatar: true }))}
                        className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-red-500/40 text-red-400 text-xs hover:bg-red-500/10 transition-colors"
                      >
                ...
                <p className="text-xs text-gray-500 italic">Aucune photo de profil</p>
              )}
              <div className="space-y-2">
                {/* Banner preview */}
                <div className="relative h-16 rounded-lg overflow-hidden border border-gray-700/50">
                  {actionsDialogUser?.banner_url ? (
                    <img src={actionsDialogUser.banner_url} alt="Bannière" className="w-full h-full object-cover" />
                  ) : (
                    <div className={`w-full h-full bg-gradient-to-r ${BANNER_OPTIONS[actionsDialogUser?.banner_color || "amber"] || BANNER_OPTIONS.amber}`} />
                  )}
                  {actionsForm.clear_banner && (
                    <div className="absolute inset-0 bg-red-900/70 flex items-center justify-center">
                      <span className="text-red-200 text-xs font-semibold">⚠ Sera supprimée</span>
                    </div>
                  )}
                </div>
                <div className="flex items-center justify-between">
                  <p className="text-xs text-gray-500">
                    {actionsDialogUser?.banner_url ? "Image personnalisée" : `Couleur : ${actionsDialogUser?.banner_color || "amber"}`}
                  </p>
                  {actionsDialogUser?.banner_url && (
                    actionsForm.clear_banner ? (
                      <button type="button" onClick={() => setActionsForm((f) => ({ ...f, clear_banner: false }))} className="text-xs text-gray-400 underline hover:text-gray-200">Annuler</button>
                    ) : (
                      <button
                        type="button"
                        onClick={() => setActionsForm((f) => ({ ...f, clear_banner: true }))}
                        className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-red-500/40 text-red-400 text-xs hover:bg-red-500/10 transition-colors"
                      >
                        <Trash2 className="w-3 h-3" />
                        Supprimer l'image
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setActionsDialogOpen(false)} disabled={actionsLoading}>
              Annuler
            </Button>
            <Button
              className="bg-orange-500 hover:bg-orange-400 text-white font-semibold"
              onClick={handleActionsSave}
              disabled={actionsLoading}
            >
              {actionsLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : "Appliquer"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-gray-700/50">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <PermissionToggle value={formPermissions.reviews.update} onChange={(v) => setFormPermissions((p) => ({ ...p, reviews: { ...p.reviews, update: v } }))} label="Modifier les avis" />
                    <PermissionToggle value={formPermissions.reviews.reassign} onChange={(v) => setFormPermissions((p) => ({ ...p, reviews: { ...p.reviews, reassign: v } }))} label="Réattribuer les avis" />
                  </div>
                </div>
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-amber-600/20">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("loyalty")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("loyalty")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-purple-600/20">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("particularities")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("particularities")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-amber-600/20">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("giveaways")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("giveaways")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-amber-600/20">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold text-amber-400 uppercase tracking-wide">🏆 Badges</h3>
                    </div>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("badges")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("badges")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(formPermissions.badges).map(([action, value]) => (
                      <PermissionToggle
                        key={action}
                        value={value}
                        onChange={(v) => togglePermission("badges", action, v)}
                        label={getPermissionLabel(action)}
                      />
                    ))}
                  </div>
                ...
                <div className="bg-slate-800/40 rounded-xl p-4 border border-blue-600/20">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("webhooks")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("webhooks")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 gap-2">
                    <PermissionToggle
                ...
                {/* Paramètres */}
                <div className="bg-slate-800/40 rounded-xl p-4 border border-orange-600/20">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold text-orange-400 uppercase tracking-wide">⚙️ Paramètres</h3>
                    </div>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("settings")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("settings")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 gap-2">
                    <PermissionToggle
                      value={formPermissions.settings.view}
                      onChange={(v) => togglePermission("settings", "view", v)}
                      label="Voir paramètres"
                      description="Accès à l'onglet Paramètres du panel admin"
                    />
                    <PermissionToggle
                      value={formPermissions.settings.toggle_calls}
                      onChange={(v) => togglePermission("settings", "toggle_calls", v)}
                      label="Activer/désactiver appels vocaux"
                      description="Bloquer ou autoriser les appels WebRTC pour tous les utilisateurs"
                    />
                  </div>
                </div>

                {/* Tableaux */}
                <div className="bg-slate-800/40 rounded-xl p-4 border border-emerald-600/20">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold text-emerald-400 uppercase tracking-wide">📊 Tableaux</h3>
                    </div>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("spreadsheets")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("spreadsheets")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <PermissionToggle value={formPermissions.spreadsheets.view} onChange={(v) => togglePermission("spreadsheets", "view", v)} label="Voir les tableaux" />
                    <PermissionToggle value={formPermissions.spreadsheets.create} onChange={(v) => togglePermission("spreadsheets", "create", v)} label="Créer un tableau" />
                    <PermissionToggle value={formPermissions.spreadsheets.edit} onChange={(v) => togglePermission("spreadsheets", "edit", v)} label="Modifier / éditer" description="Colonnes, lignes, cellules" />
                    <PermissionToggle value={formPermissions.spreadsheets.delete} onChange={(v) => togglePermission("spreadsheets", "delete", v)} label="Supprimer" />
                  </div>
                </div>

                {/* Documents */}
                <div className="bg-slate-800/40 rounded-xl p-4 border border-cyan-600/20">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold text-cyan-400 uppercase tracking-wide">📄 Documents</h3>
                    </div>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => checkAllInCategory("documents")} className="text-xs text-gray-400 hover:text-green-400 transition-colors">Tout activer</button>
                      <span className="text-gray-600">·</span>
                      <button type="button" onClick={() => uncheckAllInCategory("documents")} className="text-xs text-gray-400 hover:text-red-400 transition-colors">Tout désactiver</button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <PermissionToggle value={formPermissions.documents.view} onChange={(v) => togglePermission("documents", "view", v)} label="Voir les documents" />
                    <PermissionToggle value={formPermissions.documents.create} onChange={(v) => togglePermission("documents", "create", v)} label="Créer un document" />
                    <PermissionToggle value={formPermissions.documents.edit} onChange={(v) => togglePermission("documents", "edit", v)} label="Modifier / éditer" description="Contenu, titre, partage public" />
                    <PermissionToggle value={formPermissions.documents.delete} onChange={(v) => togglePermission("documents", "delete", v)} label="Supprimer" />
                  </div>
                </div>
              </div>
            )}
