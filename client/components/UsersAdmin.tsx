...
const DEFAULT_PERMISSIONS: UserPermissions = {
  ...
  particularities: { view: false, create: false, update: false, delete: false },
};
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
              ...
              <DialogContent className="bg-slate-900 border-amber-600/30 text-white max-w-2xl max-h-[80vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-semibold text-amber-400">Permissions Catégories</h3>
                      <Button 
                        size="sm" 
                        variant="outline" 
                        className="text-xs"
                        onClick={() => checkAllPermissionsInCategory("categories" as any)}
                      >
                        Cocher tout
                      </Button>
                    </div>
                    <div className="space-y-0">
                      {Object.entries(formPermissions.categories || { view: false, create: false, update: false, delete: false }).map(([action, value]) => (
                        <PermissionCheckbox
                          key={action}
                          action={action}
                          value={value}
                          onChange={(checked) => togglePermission("categories", action, checked)}
                          label={getPermissionLabel(action)}
                        />
                      ))}
                    </div>
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-semibold text-amber-400">Permissions Particularités</h3>
                      <Button 
                        size="sm" 
                        variant="outline" 
                        className="text-xs"
                        onClick={() => checkAllPermissionsInCategory("particularities" as any)}
                      >
                        Cocher tout
                      </Button>
                    </div>
                    <div className="space-y-0">
                      {Object.entries(formPermissions.particularities || { view: false, create: false, update: false, delete: false }).map(([action, value]) => (
                        <PermissionCheckbox
                          key={action}
                          action={action}
                          value={value}
                          onChange={(checked) => togglePermission("particularities", action, checked)}
                          label={getPermissionLabel(action)}
                        />
                      ))}
                    </div>
                            ...
                            <DialogContent className="bg-slate-900 border-amber-600/30 text-white">
                              ...
                              {selectedUser && (
                                <div className="space-y-4 text-sm max-h-[60vh] overflow-y-auto">
                                  ...
                                  <div>
                                    <p className="text-gray-400 mb-2">Permissions Catégories</p>
                                    <div className="flex flex-wrap gap-2">
                                      {Object.entries(selectedUser.permissions.categories || { view: false, create: false, update: false, delete: false })
                                        .filter(([_, value]) => value)
                                        .map(([action, _]) => (
                                          <div
                                            key={action}
                                            className="group inline-flex items-center rounded-full border px-2.5 py-0.5 font-semibold transition-all duration-200 border-transparent hover:bg-orange-500/30 bg-orange-500/20 text-orange-300 text-xs cursor-help"
                                          >
                                            <span className="whitespace-nowrap">{getPermissionEmoji(action)}</span>
                                            <span className="ml-0 w-0 overflow-hidden opacity-0 transition-all duration-200 whitespace-nowrap group-hover:ml-1 group-hover:w-auto group-hover:opacity-100">
                                              {getPermissionLabel(action)}
                                      ...
                                      {!Object.values(selectedUser.permissions.categories || { view: false }).some(v => v) && (
                                        <Badge className="bg-gray-500/20 text-gray-300 text-xs">Aucune</Badge>
                                      )}
                                    </div>
                                  </div>
                                  <div>
                                    <p className="text-gray-400 mb-2">Permissions Particularités</p>
                                    <div className="flex flex-wrap gap-2">
                                      {Object.entries(selectedUser.permissions.particularities || { view: false, create: false, update: false, delete: false })
                                        .filter(([_, value]) => value)
                                        .map(([action, _]) => (
                                          <div
                                            key={action}
                                            className="group inline-flex items-center rounded-full border px-2.5 py-0.5 font-semibold transition-all duration-200 border-transparent hover:bg-yellow-500/30 bg-yellow-500/20 text-yellow-300 text-xs cursor-help"
                                          >
                                            <span className="whitespace-nowrap">{getPermissionEmoji(action)}</span>
                                            <span className="ml-0 w-0 overflow-hidden opacity-0 transition-all duration-200 whitespace-nowrap group-hover:ml-1 group-hover:w-auto group-hover:opacity-100">
                                              {getPermissionLabel(action)}
                                      ...
                                      {!Object.values(selectedUser.permissions.particularities || { view: false }).some(v => v) && (
                                        <Badge className="bg-gray-500/20 text-gray-300 text-xs">Aucune</Badge>
                                      )}
                                    </div>
                                  </div>
