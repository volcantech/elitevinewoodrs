            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
              ...
              <DialogContent className="bg-slate-900 border-amber-600/30 text-white max-w-2xl max-h-[80vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <h3 className="text-sm font-semibold text-amber-400 mb-3">Permissions Avis</h3>
                    <div className="space-y-0">
                      <PermissionCheckbox
                        action="view"
                        value={formPermissions.reviews.view}
                        onChange={(checked) => setFormPermissions({ ...formPermissions, reviews: { ...formPermissions.reviews, view: checked } })}
                        label="Voir les avis"
                      />
                      <PermissionCheckbox
                        action="delete"
                        value={formPermissions.reviews.delete}
                        onChange={(checked) => setFormPermissions({ ...formPermissions, reviews: { ...formPermissions.reviews, delete: checked } })}
                        label="Supprimer les avis"
                      />
                    </div>
                  </div>
                </div>
                            ...
                            <DialogContent className="bg-slate-900 border-amber-600/30 text-white">
                              ...
                              {selectedUser && (
                                <div className="space-y-4 text-sm max-h-[60vh] overflow-y-auto">
                                  ...
                                  <div>
                                    <p className="text-gray-400 mb-2">Permissions Avis</p>
                                    <div className="flex flex-wrap gap-2">
                                      {selectedUser.permissions.reviews?.view && (
                                        <div className="group inline-flex items-center rounded-full border px-2.5 py-0.5 font-semibold transition-all duration-200 border-transparent hover:bg-amber-500/30 bg-amber-500/20 text-amber-300 text-xs cursor-help">
                                          <span className="whitespace-nowrap">⭐</span>
                                          <span className="ml-0 w-0 overflow-hidden opacity-0 transition-all duration-200 whitespace-nowrap group-hover:ml-1 group-hover:w-auto group-hover:opacity-100">Voir les avis</span>
                                        </div>
                                      )}
                                      {selectedUser.permissions.reviews?.delete && (
                                        <div className="group inline-flex items-center rounded-full border px-2.5 py-0.5 font-semibold transition-all duration-200 border-transparent hover:bg-red-500/30 bg-red-500/20 text-red-300 text-xs cursor-help">
                                          <span className="whitespace-nowrap">🗑️</span>
                                          <span className="ml-0 w-0 overflow-hidden opacity-0 transition-all duration-200 whitespace-nowrap group-hover:ml-1 group-hover:w-auto group-hover:opacity-100">Supprimer les avis</span>
                                        </div>
                                      )}
                                      {!selectedUser.permissions.reviews?.view && !selectedUser.permissions.reviews?.delete && (
                                        <Badge className="bg-gray-500/20 text-gray-300 text-xs">Aucune</Badge>
                                      )}
                                    </div>
                                  </div>
