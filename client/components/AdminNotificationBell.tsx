    ...
    <div className="relative" ref={panelRef}>
      ...
      {open && (
        <div className="absolute right-0 top-12 z-50 w-96 rounded-xl border border-slate-600/40 bg-slate-900 shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "520px" }}>
          ...
          <div className="flex gap-1 px-3 py-2 border-b border-slate-700/40 shrink-0 overflow-x-auto scrollbar-none">
            {CATEGORIES.map((cat) => {
              ...
              return (
                ...
                >
                  {cat.label}
                  {cnt > 0 && (
                    <span className="flex items-center justify-center min-w-[16px] h-4 px-0.5 rounded-full bg-red-500/90 text-white text-[9px] font-bold">
                      {cnt > 9 ? "9+" : cnt}
                    </span>
                  )}
                </button>
