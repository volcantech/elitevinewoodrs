...
import { Car, User, LogOut, ChevronDown, Shield, Bell, MessageSquare, ShoppingBag, Heart, Star, Gift, Users, Ticket, CircleUser, Home, BookOpen, Menu, X, Award, ChevronLeft, ChevronRight, Search, Settings2 } from "lucide-react";
...
export default function Navigation() {
  ...
  const [bellCategory, setBellCategory] = useState<string>("all");
  ...
  const BELL_CATEGORIES = [
    { key: "all", label: "Tout" },
    { key: "commandes", label: "Commandes", types: ["message", "delivered", "cancelled"] },
    { key: "messages", label: "Messages", types: ["private_message", "chat"] },
    { key: "reactions", label: "Réactions", types: ["reaction", "like"] },
    { key: "tickets", label: "Tickets", types: ["ticket", "ticket_closed"] },
    { key: "amis", label: "Amis", types: ["friend_request", "friend_accepted"] },
    { key: "recompenses", label: "Récompenses", types: ["badge", "giveaway_win"] },
  ];

  const fetchBellNotifs = async (page = 1, category = bellCategory) => {
    if (!token) return;
    setBellLoading(true);
    try {
      const cat = BELL_CATEGORIES.find((c) => c.key === category);
      const typesParam = cat && "types" in cat ? `&types=${cat.types.join(",")}` : "";
      const r = await fetch(`/api/public/notifications/history?page=${page}&limit=5${typesParam}`, {
  ...
  const handleBellToggle = () => {
    ...
    if (next) {
      setBellPage(1);
      setBellCategory("all");
      fetchBellNotifs(1, "all");
      clearUnread();
      setUserMenuOpen(false);
    }
  };

  const handleBellCategoryChange = (cat: string) => {
    setBellCategory(cat);
    setBellPage(1);
    fetchBellNotifs(1, cat);
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const r = await fetch(`/api/public/users/search?q=${encodeURIComponent(searchQuery)}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <CartDrawer onOpen={() => setBellOpen(false)} />
                    ...
                    {userMenuOpen && (
                      <div className="absolute right-0 top-full mt-2 w-52 bg-gray-900 border border-gray-700 rounded-xl shadow-xl overflow-hidden z-50">
                        ...
                        <div className="border-b border-gray-700/60">
                          ...
                          <button onClick={() => { navigate("/account"); setUserMenuOpen(false); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors">
                    ...
                    {bellOpen && (
                      <div className="absolute right-0 top-full mt-2 w-80 bg-gray-900 border border-gray-700 rounded-xl shadow-xl overflow-hidden z-50">
                        ...
                        <div className="px-3 py-2 border-b border-gray-700/60">
                          <select
                            value={bellCategory}
                            onChange={(e) => handleBellCategoryChange(e.target.value)}
                            className="w-full bg-gray-800 border border-gray-600 text-gray-200 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-blue-500 cursor-pointer"
                          >
                            {BELL_CATEGORIES.map((cat) => (
                              ...
                              {bellNotifs.map((n) => {
                                ...
                                >
                                  <span className="text-base shrink-0 mt-0.5">
                                    {n.type === "delivered" ? "📦" : n.type === "cancelled" ? "❌" : n.type === "ticket" ? "🎫" : n.type === "report" ? "🚩" : n.type === "like" ? "❤️" : n.type === "badge" ? "🏆" : n.type === "friend_accepted" ? "🤝" : n.type === "friend_request" ? "👤" : n.type === "private_message" ? "💬" : n.title.includes("Appel manqué") ? "📵" : n.title.includes("Appel vocal") ? "📞" : "💬"}
                                  </span>
                                  <div className="flex-1 min-w-0">
            ...
            <div className="md:hidden flex items-center gap-2">
              ...
              {user ? (
                <button
                  onClick={() => { setMobileMenuOpen((v) => !v); setBellOpen(false); }}
              ...
              {user && (
                <>
                  <button
                    onClick={() => { navigate("/messages"); clearUnreadMessages(); setMsgUnreadBase(0); setBellOpen(false); }}
                    className="relative w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-amber-400 hover:bg-gray-800 transition-colors"
                  >
                    <MessageSquare className="w-5 h-5" />
                    {totalMsgUnread > 0 && (
              ...
              <button
                onClick={() => { setMobileMenuOpen(!mobileMenuOpen); setBellOpen(false); }}
            ...
            <div className="md:hidden pb-4 space-y-1">
              ...
              {user ? (
                <>
                  ...
                  <button onClick={() => { navigate("/account"); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-md text-sm font-medium text-gray-300 hover:text-amber-400 hover:bg-gray-800 transition-colors duration-200">
                  ...
                  <button onClick={() => { navigate("/messages"); clearUnreadMessages(); setMsgUnreadBase(0); setMobileMenuOpen(false); setBellOpen(false); }} className="flex items-center gap-3 w-full text-left px-3 py-2.5 rounded-md text-sm font-medium text-gray-300 hover:text-amber-400 hover:bg-gray-800 transition-colors duration-200">
