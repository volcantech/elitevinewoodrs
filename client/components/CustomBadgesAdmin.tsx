...
const TRIGGER_GROUPS = [
  {
    ...
    keys: ["messages_sent_all", "messages_received", "reactions_sent", "reactions_received", "calls_participated", "images_sent", "images_received"],
  },
  {
    label: "📦 Achats",
    keys: ["orders_delivered", "total_spent"],
  },
  {
    label: "⭐ Avis & Likes",
    keys: ["reviews_posted", "likes_received", "likes_sent"],
  },
  {
    label: "👥 Social",
    keys: ["friends_count", "friends_requested", "referrals_count", "users_blocked", "favorites_count", "profiles_visited", "profile_views_received"],
  },
  {
    label: "🎖️ Fidélité & Progression",
    keys: ["loyalty_points", "loyalty_points_used", "days_member"],
  },
  {
    label: "✨ Spécial",
    keys: ["giveaway_won", "rp_profile_complete", "avatar_changed", "banner_changed", "preferences_discovered"],
  },
  {
    label: "👤 Manuel",
    keys: ["manual"],
              ...
              <Select value={form.trigger_type} onValueChange={(v) => setForm((f) => ({ ...f, trigger_type: v }))}>
                ...
                <SelectContent className="bg-gray-900 border-gray-700 max-h-80">
                  {TRIGGER_GROUPS.map((group) => {
                    const groupTriggers = group.keys.filter((k) => triggers[k]);
                    if (groupTriggers.length === 0) return null;
                    return (
                      <SelectGroup key={group.label}>
                        <SelectLabel className="text-[11px] text-gray-500 px-2 py-1 font-semibold tracking-wide uppercase">
                          {group.label}
                        </SelectLabel>
                        {groupTriggers.map((key) => (
                          <SelectItem key={key} value={key} className="text-white hover:bg-gray-800 pl-4">
                            {triggers[key]}
                          </SelectItem>
                        ))}
                      </SelectGroup>
