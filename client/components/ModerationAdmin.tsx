...
import { Shield, Plus, Trash2, RefreshCw, AlertTriangle, FileText, ChevronRight, ChevronLeft, Star, User, MessageSquare, Flag, Webhook, Megaphone, Gift, Award, Phone, Settings, Database, Download, Loader2 } from "lucide-react";
import { Switch } from "@/components/ui/switch";
import { ActivityLogDetailsDialog } from "./ActivityLogDetailsDialog";
import { ReviewsAdmin } from "./ReviewsAdmin";
import { TicketsAdmin } from "./TicketsAdmin";
import { ReviewReportsAdmin } from "./ReviewReportsAdmin";
...
import { CustomBadgesAdmin } from "./CustomBadgesAdmin";
...
function SubTabBadge({ count }: { count: number }) {
...
interface ModerationAdminProps {
  ...
  subBadges?: { reviews: number; tickets: number; reports: number };
  onSubTabSeen?: (tab: string) => void;
}
...
export function ModerationAdmin({ token, currentUser, permissions, subBadges, onSubTabSeen }: ModerationAdminProps) {
  ...
  const activeTab = searchParams.get("modtab") || "banned-ids";
  const setActiveTab = (tab: string) => {
    setSearchParams(prev => { const p = new URLSearchParams(prev); p.set("modtab", tab); return p; });
    onSubTabSeen?.(tab);
  };
  ...
  const [callsBlocked, setCallsBlocked] = useState<boolean>(false);
  const [callsToggleLoading, setCallsToggleLoading] = useState(false);
  ...
  const fetchCallsSetting = async () => {
    try {
      const res = await authenticatedFetch("/api/admin/settings/calls", token);
      if (res.ok) { const d = await res.json(); setCallsBlocked(!d.enabled); }
    } catch { /* silent */ }
  };
  ...
  const handleDatabaseDump = async () => {
    setDumping(true);
    try {
      const res = await fetch("/api/admin/dump", { credentials: "include" });
      ...
      a.download = `dump_${ts}.sql`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast.success("✅ Dump téléchargé avec succès !");
    } catch {
      toast.error("❌ Erreur lors de la génération du dump");
    } finally {
      setDumping(false);
    }
  };

  const handleToggleCalls = async (blocked: boolean) => {
    setCallsToggleLoading(true);
    try {
      const res = await authenticatedFetch("/api/admin/settings/calls", token, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ enabled: !blocked }),
      });
      if (res.ok) { setCallsBlocked(blocked); toast.success(blocked ? "🔇 Appels vocaux bloqués" : "✅ Appels vocaux autorisés"); }
      else toast.error("Erreur lors de la mise à jour");
    } catch { toast.error("Erreur réseau"); }
    finally { setCallsToggleLoading(false); }
  };
  ...
  }, [token, permissions?.moderation?.view_logs, refreshKey]);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-slate-800/50 border border-amber-600/30 flex-nowrap md:flex-wrap overflow-x-auto md:overflow-x-visible w-full justify-start">
          ...
          {permissions?.reviews?.view && (
            ...
            >
              ...
              Avis
              <SubTabBadge count={subBadges?.reviews ?? 0} />
            </TabsTrigger>
          )}
          {(permissions?.moderation?.view_reports || permissions?.moderation?.ban_uniqueids) && (
            <TabsTrigger
              value="reports"
              className="data-[state=active]:bg-amber-500 data-[state=active]:text-black"
            >
              ...
              Signalements
              <SubTabBadge count={subBadges?.reports ?? 0} />
            </TabsTrigger>
          )}
          {(permissions?.tickets?.manage || permissions?.tickets?.view || permissions?.moderation?.view) && (
            <TabsTrigger
              value="tickets"
              className="data-[state=active]:bg-amber-500 data-[state=active]:text-black"
            >
              ...
              Tickets
              <SubTabBadge count={subBadges?.tickets ?? 0} />
            </TabsTrigger>
          )}
          ...
          {(permissions?.webhooks?.manage || permissions?.loyalty?.manage) && (
            <TabsTrigger
              value="webhooks"
              className="data-[state=active]:bg-amber-500 data-[state=active]:text-black"
            >
        ...
        {(permissions?.moderation?.view_reports || permissions?.moderation?.ban_uniqueids) && (
          <TabsContent value="reports">
            <ReviewReportsAdmin token={token} permissions={permissions} />
          </TabsContent>
        )}

        {(permissions?.tickets?.manage || permissions?.tickets?.view || permissions?.moderation?.view) && (
          <TabsContent value="tickets">
            <TicketsAdmin token={token} adminUsername={currentUser?.username || "Admin"} permissions={permissions} />
          </TabsContent>
        )}
        {permissions?.loyalty?.manage && (
          <TabsContent value="fidelite">
            <LoyaltyAdmin token={token} currentUser={currentUser} />
          </TabsContent>
        )}
        {(permissions?.webhooks?.manage || permissions?.loyalty?.manage) && (
          <TabsContent value="webhooks">
            <WebhookAdmin token={token} />
          </TabsContent>
        )}
        {permissions?.announcements?.view && (
          <TabsContent value="announcements">
            <AnnouncementAdmin token={token} permissions={permissions} />
          </TabsContent>
        )}
        {permissions?.giveaways?.view && (
          <TabsContent value="giveaways">
            <GiveawayAdmin token={token} permissions={permissions?.giveaways} />
          </TabsContent>
        )}
        {permissions?.badges?.view && (
          <TabsContent value="badges">
            <CustomBadgesAdmin token={token} permissions={permissions?.badges} />
          </TabsContent>
        )}
        ...
        {permissions?.settings?.view && (
          <TabsContent value="settings">
            <div className="space-y-6">
              ...
              <Card className="bg-slate-800/40 border border-amber-600/20">
                <CardHeader>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-lg bg-slate-900/50 border border-gray-700/50">
                    <div>
                      <p className="text-sm font-medium text-gray-200">
                        {callsBlocked ? "Appels vocaux bloqués pour tous" : "Appels vocaux autorisés"}
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">
                        Bloquer ou autoriser les appels WebRTC pour l'ensemble des utilisateurs
                      </p>
                    </div>
                    {permissions?.settings?.toggle_calls ? (
                      <Switch
                        checked={callsBlocked}
                        onCheckedChange={handleToggleCalls}
                        disabled={callsToggleLoading}
                      />
                    ) : (
                      <span className="text-xs text-gray-500 italic">Permission requise</span>
                    )}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-slate-800/40 border border-amber-600/20">
                ...
                <CardContent>
                  <div className="flex items-center justify-between p-4 rounded-lg bg-slate-900/50 border border-gray-700/50">
                    <div>
                      <p className="text-sm font-medium text-gray-200">Télécharger un dump complet</p>
                      <p className="text-xs text-gray-500 mt-0.5">
                        Exporte toutes les tables avec leurs données (mots de passe chiffrés inclus) au format SQL
                      </p>
                    </div>
                    <Button
                      onClick={handleDatabaseDump}
                      disabled={dumping}
                      className="bg-amber-500 hover:bg-amber-400 text-black font-semibold shrink-0 ml-4"
                    >
                      {dumping
                        ? <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Génération...</>
                        : <><Download className="w-4 h-4 mr-2" /> Télécharger .sql</>
                      }
                    </Button>
                  </div>
                </CardContent>
