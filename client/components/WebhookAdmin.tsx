...
interface WebhookSettings {
  ...
  ticket_webhook_url: string | null;
  ...
  ticket_webhook_url_updated_by: string | null;
  ticket_webhook_url_updated_at: string | null;
}
...
export function WebhookAdmin({ token }: WebhookAdminProps) {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      setTicketWebhookUrl(data.ticket_webhook_url ?? "");
    } catch {
    ...
    try {
      const res = await authenticatedFetch("/api/admin/settings/webhooks", token, {
        ...
        body: JSON.stringify({
          ...
          ticket_webhook_url: ticketWebhookUrl.trim() || null,
        }),
      });
      if (!res.ok) {
        const data = await res.json();
  ...
  const handleTest = async (type: "main" | "discord" | "ticket") => {
    const setter = type === "main" ? setTestingMain : type === "discord" ? setTestingDiscord : setTestingTicket;
    setter(true);
    try {
  ...
  const hasTicketChanges = ticketWebhookUrl.trim() !== (settings?.ticket_webhook_url ?? "");
  const hasChanges = hasMainChanges || hasDiscordChanges || hasTicketChanges;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5">
        ...
        <Card className="bg-gray-900 border border-gray-700/60">
          <CardContent className="pt-5 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-base font-semibold text-white">Webhook Tickets</span>
            ...
            <p className="text-xs text-gray-400">
              Déclenché lors de la création d'un ticket ou d'une réponse admin. Permet de recevoir les tickets dans Discord.
            </p>
            <div className="space-y-2">
              <Label className="text-gray-300 text-sm">URL du webhook tickets</Label>
              <Input
                value={ticketWebhookUrl}
                onChange={(e) => setTicketWebhookUrl(e.target.value)}
                placeholder="https://discord.com/api/webhooks/..."
                className="bg-gray-800 border-gray-600 text-white placeholder-gray-500 focus:border-green-500 font-mono text-sm"
              />
            </div>
            {settings?.ticket_webhook_url_updated_by && (
              <p className="text-xs text-gray-500">
                Dernière modification par <span className="text-gray-300">{settings.ticket_webhook_url_updated_by}</span>
                {settings.ticket_webhook_url_updated_at && ` — ${formatDate(settings.ticket_webhook_url_updated_at)}`}
              </p>
            )}
            <div className="flex gap-2 pt-1">
              <Button
                size="sm"
                variant="outline"
                onClick={() => handleTest("ticket")}
                disabled={testingTicket || !settings?.ticket_webhook_url}
                className="border-green-500/40 text-green-400 hover:bg-green-500/10 disabled:opacity-40"
              >
                {testingTicket ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1" /> : <Send className="w-3.5 h-3.5 mr-1" />}
                Tester
              </Button>
              {ticketWebhookUrl && (
                <Button
