...
import {
  Plus, Trash2, ChevronLeft, ChevronRight, FileText, Pencil, Check, X, Globe, Lock,
  ...
  Search, Eye, EyeOff, Folder, FolderOpen, CalendarDays, ChevronUp, ChevronDown,
} from "lucide-react";
...
interface TemplateVar { key: string; label: string; isSig?: boolean; isDate?: boolean; options?: string[]; }
...
function extractVariables(content: string | undefined | null): TemplateVar[] {
  ...
  for (const match of matches) {
    ...
    const isDate = flag === "date";
    const options = flag.startsWith("drop:") ? flag.slice(5).split(",").map(o => o.trim()).filter(Boolean) : undefined;
    if (key && !seen.has(key)) { seen.add(key); result.push({ key, label: label || key, isSig, isDate, options }); }
  }
  return result;
}

function extractVarOrder(content: string): string[] {
  const m = content.match(/<!--var-order:([^-]*)-->/);
  return m ? m[1].split(",").filter(Boolean) : [];
}

function embedVarOrder(content: string, order: string[]): string {
  const tag = order.length ? `<!--var-order:${order.join(",")}-->` : "";
  const cleaned = content.replace(/<!--var-order:[^-]*-->/g, "");
  return tag + cleaned;
}

interface TemplateStep { title: string; varKeys: string[]; }

function extractSteps(content: string): TemplateStep[] {
  const m = content.match(/<!--template-steps:([\s\S]*?)-->/);
  if (!m) return [];
  try { return JSON.parse(m[1]); } catch { return []; }
}

function embedSteps(content: string, steps: TemplateStep[]): string {
  const tag = steps.length ? `<!--template-steps:${JSON.stringify(steps)}-->` : "";
  const cleaned = content.replace(/<!--template-steps:[\s\S]*?-->/g, "");
  return tag + cleaned;
}
...
type VarType = "text" | "sig" | "drop" | "date";

interface InsertVarDialogProps {
  open: boolean;
  onClose: () => void;
  onInsert: (key: string, label: string, isSig: boolean, options?: string[], isDate?: boolean) => void;
  editMode?: boolean;
...
function InsertVariableDialog({ open, onClose, onInsert, editMode = false, initialData }: InsertVarDialogProps) {
  ...
  const reset = () => { setVarKey(""); setVarLabel(""); setVarType("text"); setDropInput(""); setDropOptions([]); setDragIdx(null); };
  ...
  const addOption = () => {
    ...
    if (val) { setDropOptions(p => [...p, val]); setDropInput(""); }
  };

  const addGroup = () => {
    const val = dropInput.trim();
    if (val) { setDropOptions(p => [...p, `__group__:${val}`]); setDropInput(""); }
  };
  ...
  const handle = () => {
    ...
    onInsert(cleanKey, varLabel.trim() || cleanKey, varType === "sig", varType === "drop" ? dropOptions : undefined, varType === "date");
    reset(); onClose();
  };

  const previewKey = varKey.trim().replace(/\s+/g, "_");
  const previewLabel = varLabel.trim() || previewKey;
  const flag = varType === "sig" ? "|sig" : varType === "drop" && dropOptions.length > 0 ? `|drop:${dropOptions.join(",")}` : varType === "date" ? "|date" : "";
        ...
        <div className="space-y-4 py-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                ["date", "Date", "📅", "Sélecteur de date (jj/mm/aaaa)"],
          ...
          {varType === "sig" && (
            <div className="rounded-xl border border-amber-500/20 bg-white/5 p-4 flex flex-col items-start gap-2 min-h-[70px]">
              ...
              <div style={{ fontFamily: "'Dancing Script', cursive", fontSize: "26px", color: "#000000", letterSpacing: "1px", borderBottom: "1.5px solid #000000", paddingBottom: "4px", display: "block", maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          ...
          {varType === "date" && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 flex items-center gap-2">
              ...
              <p className="text-xs text-gray-300">Un sélecteur de date sera affiché lors du remplissage. La date est insérée au format <span className="font-mono text-amber-300">jj/mm/aaaa</span> et initialisée à la date du jour.</p>
            </div>
          )}
          ...
          {varType === "drop" && (
            <div className="space-y-2.5">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" size="sm"
                  onMouseDown={e => { e.preventDefault(); addGroup(); }}
                  title="Ajouter une catégorie (en-tête non cliquable)"
                  className="h-9 px-3 bg-slate-700 hover:bg-slate-600 text-amber-300 border border-amber-500/40 font-semibold shrink-0 text-xs">
                  ≡ Catégorie
                </Button>
              </div>
              {dropOptions.length > 0 ? (
                <div className="flex flex-wrap gap-1.5 p-2 bg-slate-800/50 rounded-lg border border-slate-700 min-h-[40px]">
                  {dropOptions.map((opt, i) => (
                    opt.startsWith("__group__:") ? (
                      <span key={i} draggable
                        onDragStart={() => setDragIdx(i)}
                        onDragOver={e => { e.preventDefault(); if (dragIdx === null || dragIdx === i) return; setDropOptions(prev => { const a = [...prev]; const [item] = a.splice(dragIdx, 1); a.splice(i, 0, item); return a; }); setDragIdx(i); }}
                        onDragEnd={() => setDragIdx(null)}
                        className={`flex items-center gap-1 bg-amber-500/15 border border-amber-500/40 text-amber-300 text-xs px-2.5 py-1 rounded-md font-semibold cursor-grab select-none transition-opacity ${dragIdx === i ? "opacity-30" : ""}`}>
                        <span className="opacity-70">⠿</span>
                        <span className="opacity-70">≡</span>
                        <span className="max-w-[120px] truncate">{opt.slice(10)}</span>
                        <button type="button" onMouseDown={e => { e.preventDefault(); removeOption(i); }} className="text-amber-500/50 hover:text-red-400 ml-0.5 transition-colors"><X className="w-3 h-3" /></button>
                      </span>
                    ) : (
                      <span key={i} draggable
                        onDragStart={() => setDragIdx(i)}
                        onDragOver={e => { e.preventDefault(); if (dragIdx === null || dragIdx === i) return; setDropOptions(prev => { const a = [...prev]; const [item] = a.splice(dragIdx, 1); a.splice(i, 0, item); return a; }); setDragIdx(i); }}
                        onDragEnd={() => setDragIdx(null)}
                        className={`flex items-center gap-1 bg-slate-700 hover:bg-slate-600 text-gray-200 text-xs px-2.5 py-1.5 rounded-full cursor-grab select-none transition-opacity ${dragIdx === i ? "opacity-30" : ""}`}>
                        <span className="opacity-30 text-xs">⠿</span>
                        <span className="max-w-[120px] truncate">{opt}</span>
                        <button type="button" onMouseDown={e => { e.preventDefault(); removeOption(i); }} className="text-gray-500 hover:text-red-400 ml-0.5 transition-colors"><X className="w-3 h-3" /></button>
                      </span>
                    )
                  ))}
                </div>
              ) : (
                <p className="text-xs text-gray-600 italic text-center py-2">Ajoutez des options. Utilisez <span className="text-amber-400 font-medium">≡ Catégorie</span> pour créer des groupes.</p>
              )}
            </div>
          )}
        </div>
...
type SigStyle = "cursive" | "double" | "moderne" | "encadre" | "elegant";

const SIG_STYLES: { id: SigStyle; name: string }[] = [
  { id: "cursive",  name: "Cursive" },
  { id: "double",   name: "Double trait" },
  { id: "moderne",  name: "Moderne" },
  { id: "encadre",  name: "Encadré" },
  { id: "elegant",  name: "Élégant" },
];

async function generateSignatureHtml(name: string, color: string = "#000000", style: SigStyle = "cursive"): Promise<string> {
  if (!name.trim()) return "";

  const useScriptFont = style !== "moderne";
  if (useScriptFont) {
    try { await document.fonts.load('700 48px "Dancing Script"'); } catch { /* use loaded font */ }
  }

  const probe = document.createElement("canvas").getContext("2d")!;
  const fontSize = style === "moderne"
    ? Math.max(22, Math.min(36, 30 - Math.max(0, name.length - 8) * 0.7))
    : Math.max(28, Math.min(46, 38 - Math.max(0, name.length - 6) * 1.2));

  const fontDecl = style === "moderne"
    ? `bold italic ${fontSize}px Arial, Helvetica, sans-serif`
    : `700 ${fontSize}px "Dancing Script", cursive`;

  probe.font = fontDecl;
  const tw = probe.measureText(name).width;

  const pad = 14;
  const tailW = (style === "double" || style === "moderne") ? 8 : 24;
  const extraH = style === "double" ? 18 : style === "encadre" ? 16 : 12;
  const W = Math.ceil(tw + pad * 2 + tailW);
  const H = Math.ceil(fontSize * 1.55 + extraH);
  ...
  ctx.font      = fontDecl;
  ctx.fillStyle = color;
  const textY = style === "encadre" ? fontSize + 6 : fontSize + 2;
  ...
  if (style === "cursive") {
    ...
    const lineY1 = fontSize + 8;
    const lineY2 = fontSize + 13;
    const lx1 = pad - 2, lx2 = pad + tw + 4;
    ctx.lineWidth   = 1;
    ctx.strokeStyle = color;
    ctx.lineCap     = "round";
    ctx.beginPath(); ctx.moveTo(lx1, lineY1); ctx.lineTo(lx2, lineY1); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(lx1, lineY2); ctx.lineTo(lx2, lineY2); ctx.stroke();
  } else if (style === "moderne") {
    const lineY = fontSize + 8;
    ctx.lineWidth   = 1.5;
    ctx.strokeStyle = color;
    ctx.lineCap     = "square";
    ctx.beginPath(); ctx.moveTo(pad, lineY); ctx.lineTo(pad + tw, lineY); ctx.stroke();
  } else if (style === "encadre") {
    const boxPad = 6;
    ctx.strokeStyle = color;
    ctx.lineWidth   = 1;
    ctx.strokeRect(pad - boxPad, 4, tw + boxPad * 2, fontSize + boxPad);
    ...
    ctx.strokeStyle = color;
    ctx.lineCap     = "round";
    ctx.moveTo(lineX1, lineY + 3);
    ctx.bezierCurveTo(
      lineX1 + 10,                        lineY + 5,
      lineX1 + (lineX2 - lineX1) * 0.3,  lineY - 3,
      lineX2,                             lineY - 1,
    );
    ctx.stroke();
  }
...
function SignaturePreview({ name, color = "#000000", style = "cursive" }: { name: string; color?: string; style?: SigStyle }) {
  const [imgHtml, setImgHtml] = useState("");
  useEffect(() => {
    if (!name.trim()) { setImgHtml(""); return; }
    generateSignatureHtml(name, color, style).then(setImgHtml);
  }, [name, color, style]);
...
const SIG_COLOR_PRESETS = [
  "#000000", "#1a1a1a", "#1a237e", "#0d47a1", "#b71c1c", "#1b5e20", "#4a148c", "#e65100",
];

function SigPreview({ name, color, style }: { name: string; color: string; style: SigStyle }) {
  const [html, setHtml] = useState("");
  useEffect(() => {
    generateSignatureHtml(name.trim() || "Exemple", color, style).then(setHtml);
  }, [name, color, style]);
  if (!html) return null;
  return <span className="shrink-0 bg-white/90 px-2 py-0.5 rounded inline-block" dangerouslySetInnerHTML={{ __html: html }} />;
}

function SigColorPicker({ value, onChange }: { value: string; onChange: (c: string) => void }) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-xs text-gray-400 shrink-0">Couleur :</span>
      {SIG_COLOR_PRESETS.map(c => (
        <button
          key={c}
          type="button"
          onClick={() => onChange(c)}
          title={c}
          className={`w-6 h-6 rounded-full border-2 transition-all ${value === c ? "border-white scale-110 shadow-md" : "border-transparent hover:scale-105"}`}
          style={{ background: c }}
        />
      ))}
      <label className="flex items-center gap-1 cursor-pointer" title="Couleur personnalisée">
        <span className="text-xs text-gray-400">Autre :</span>
        <input
          type="color"
          value={value}
...
function SigStylePicker({ value, onChange, sampleName, color }: { value: SigStyle; onChange: (s: SigStyle) => void; sampleName: string; color: string }) {
  const [preview, setPreview] = useState("");
  useEffect(() => {
    const sample = sampleName.trim() || "Exemple";
    generateSignatureHtml(sample, color, value).then(setPreview);
  }, [sampleName, color, value]);

  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-gray-400 shrink-0">Style :</span>
      <select
        value={value}
        onChange={e => onChange(e.target.value as SigStyle)}
        className="flex-1 bg-slate-800 border border-slate-600 text-white text-xs rounded-md px-2 py-1.5 focus:outline-none focus:border-amber-400"
      >
        {SIG_STYLES.map(({ id, name: label }) => (
          <option key={id} value={id}>{label}</option>
        ))}
      </select>
      {preview
        ? <span className="shrink-0 bg-white/90 px-2 py-0.5 rounded inline-block" dangerouslySetInnerHTML={{ __html: preview }} />
        : <span className="shrink-0 text-xs text-gray-600 italic">aperçu...</span>
      }
    </div>
  );
}
...
function StepsManagerDialog({
  open, onClose, templateVars, currentContent, onSave,
}: {
  open: boolean; onClose: () => void; templateVars: TemplateVar[]; currentContent: string; onSave: (steps: TemplateStep[]) => void;
}) {
  const [steps, setSteps] = useState<TemplateStep[]>([]);
  const [newStepTitle, setNewStepTitle] = useState("");

  useEffect(() => {
    if (open) { setSteps(extractSteps(currentContent)); setNewStepTitle(""); }
  }, [open, currentContent]);

  const assignedKeys = steps.flatMap(s => s.varKeys);
  const unassignedVars = templateVars.filter(v => !assignedKeys.includes(v.key));

  const addStep = () => {
    if (!newStepTitle.trim()) return;
    setSteps(prev => [...prev, { title: newStepTitle.trim(), varKeys: [] }]);
    setNewStepTitle("");
  };
  const removeStep = (idx: number) => setSteps(prev => prev.filter((_, i) => i !== idx));
  const renameStep = (idx: number, title: string) => setSteps(prev => prev.map((s, i) => i === idx ? { ...s, title } : s));
  const addVarToStep = (stepIdx: number, key: string) => setSteps(prev => prev.map((s, i) => i === stepIdx ? { ...s, varKeys: [...s.varKeys, key] } : s));
  const removeVarFromStep = (stepIdx: number, key: string) => setSteps(prev => prev.map((s, i) => i === stepIdx ? { ...s, varKeys: s.varKeys.filter(k => k !== key) } : s));
  const moveStep = (idx: number, dir: -1 | 1) => setSteps(prev => {
    const arr = [...prev]; const next = idx + dir;
    if (next < 0 || next >= arr.length) return arr;
    [arr[idx], arr[next]] = [arr[next], arr[idx]]; return arr;
  });

  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="bg-slate-900 border-[#ffe599]/30 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ListOrdered className="w-5 h-5 text-[#ffe599]" /> Gérer les étapes du template
          </DialogTitle>
          <p className="text-sm text-gray-400 mt-1">Créez des étapes pour guider le remplissage du template. Chaque étape regroupe un ensemble de variables.</p>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          {templateVars.length > 0 && (
            <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-3">
              <p className="text-xs text-gray-400 font-medium mb-2">
                {unassignedVars.length > 0 ? `Variables non assignées (${unassignedVars.length}) :` : "✅ Toutes les variables sont assignées à une étape"}
              </p>
              {unassignedVars.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {unassignedVars.map(v => (
                    <span key={v.key} className="text-xs bg-[#ffe599]/20 border border-[#ffe599]/30 text-[#ffe599] rounded px-2 py-0.5">
                      {v.label}
                    </span>
                  ))}
                </div>
          ...
          {steps.map((step, stepIdx) => {
            const stepVars = step.varKeys.map(k => templateVars.find(v => v.key === k)).filter(Boolean) as TemplateVar[];
            return (
              <div key={stepIdx} className="border border-amber-600/30 rounded-xl p-4 space-y-3 bg-amber-500/5">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-amber-400 bg-amber-500/20 rounded-full w-6 h-6 flex items-center justify-center shrink-0">{stepIdx + 1}</span>
                  <input
                    value={step.title}
                    onChange={e => renameStep(stepIdx, e.target.value)}
                    className="flex-1 bg-slate-800 border border-amber-500/40 rounded-md px-3 py-1.5 text-white text-sm focus:outline-none focus:border-amber-400"
                    placeholder="Titre de l'étape..."
                  />
                  <button onClick={() => moveStep(stepIdx, -1)} disabled={stepIdx === 0} className="text-gray-500 hover:text-amber-400 disabled:opacity-20 p-1 transition-colors"><ChevronUp className="w-4 h-4" /></button>
                  <button onClick={() => moveStep(stepIdx, 1)} disabled={stepIdx === steps.length - 1} className="text-gray-500 hover:text-amber-400 disabled:opacity-20 p-1 transition-colors"><ChevronDown className="w-4 h-4" /></button>
                  <button onClick={() => removeStep(stepIdx)} className="text-red-500 hover:text-red-400 p-1 transition-colors"><Trash2 className="w-4 h-4" /></button>
                </div>

                {/* Variables assigned to this step */}
                <div className="flex flex-wrap gap-1.5 min-h-[28px]">
                  {stepVars.length === 0
                    ? <span className="text-xs text-gray-600 italic">Aucune variable — cliquez sur les variables ci-dessous pour les ajouter</span>
                    : stepVars.map(v => (
                      <span key={v.key} className="flex items-center gap-1 text-xs bg-[#ffe599]/20 border border-[#ffe599]/30 text-[#ffe599] rounded px-2 py-0.5">
                        {v.label}
                        <button type="button" onClick={() => removeVarFromStep(stepIdx, v.key)} className="text-gray-500 hover:text-red-400 ml-0.5 transition-colors">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))
                ...
                {/* Unassigned vars to add */}
                {unassignedVars.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">Cliquer pour ajouter à cette étape :</p>
                    <div className="flex flex-wrap gap-1.5">
                      {unassignedVars.map(v => (
                        <button key={v.key} type="button" onClick={() => addVarToStep(stepIdx, v.key)}
                          className="text-xs bg-slate-700 hover:bg-amber-500/20 border border-slate-600 hover:border-amber-500/50 text-gray-300 hover:text-amber-300 rounded px-2 py-0.5 transition-colors">
                          + {v.label}
                        </button>
                      ))}
                    </div>
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Nom de la nouvelle étape (ex: Informations personnelles)..."
              className="flex-1 bg-slate-800 border border-slate-600 text-white text-sm rounded-md px-3 py-2 focus:outline-none focus:border-[#ffe599] placeholder-gray-600"
            />
            <Button onClick={addStep} disabled={!newStepTitle.trim()} className="bg-[#ffe599] hover:bg-[#ffd966] text-gray-900 font-semibold shrink-0">
              <Plus className="w-4 h-4 mr-1" /> Ajouter étape
            </Button>
          </div>

          {steps.length === 0 && (
            <p className="text-xs text-gray-500 text-center italic">Aucune étape définie. Le formulaire s'affichera en une seule page.</p>
          )}
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Annuler</Button>
          <Button onClick={() => { onSave(steps); onClose(); }} className="bg-[#ffe599] hover:bg-[#ffd966] text-gray-900 font-semibold">
            <Check className="w-4 h-4 mr-1" /> Enregistrer les étapes
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function UseTemplateDialog({
  open, onClose, template, onCreated, existingFolders,
}: {
  open: boolean; onClose: () => void; template: Document | null; onCreated: (doc: Document) => void; existingFolders: string[];
}) {
  const [docTitle, setDocTitle] = useState("");
  const [variables, setVariables] = useState<Record<string, string>>({});
  const [creating, setCreating] = useState(false);
  const [sigImages, setSigImages] = useState<Record<string, string>>({});
  const [sigColors, setSigColors] = useState<Record<string, string>>({});
  const [sigStyles, setSigStyles] = useState<Record<string, SigStyle>>({});
  const [isPublic, setIsPublic] = useState(false);
  const [docFolder, setDocFolder] = useState("");
  const [folderIsNew, setFolderIsNew] = useState(false);
  const [currentStepIdx, setCurrentStepIdx] = useState(0);

  const rawVars = template ? extractVariables(template.content) : [];
  const savedOrder = template ? extractVarOrder(template.content) : [];
  const templateVars = savedOrder.length
    ? [...savedOrder.map(k => rawVars.find(v => v.key === k)).filter(Boolean) as typeof rawVars,
       ...rawVars.filter(v => !savedOrder.includes(v.key))]
    : rawVars;
  const orderedVars = templateVars;

  // Steps / wizard logic
  const templateSteps = template ? extractSteps(template.content) : [];
  const hasSteps = templateSteps.length > 0;
  const allAssignedKeys = templateSteps.flatMap(s => s.varKeys);
  const unassignedFromSteps = orderedVars.filter(v => !allAssignedKeys.includes(v.key));
  const wizardSteps = hasSteps
    ? [
        ...templateSteps.map(s => ({
          title: s.title,
          vars: s.varKeys.map(k => orderedVars.find(v => v.key === k)).filter(Boolean) as typeof orderedVars,
        })),
        ...(unassignedFromSteps.length > 0 ? [{ title: "Autres informations", vars: unassignedFromSteps }] : []),
      ]
    : [{ title: "", vars: orderedVars }];
  const totalSteps = wizardSteps.length;
  const safeStepIdx = Math.min(currentStepIdx, totalSteps - 1);
  const currentWizardStep = wizardSteps[safeStepIdx] ?? { title: "", vars: [] };
  const isLastStep = safeStepIdx >= totalSteps - 1;
  ...
  useEffect(() => {
    if (template && open) {
      ...
      const initialStyles: Record<string, SigStyle> = {};
      const todayISO = new Date().toISOString().split("T")[0];
      for (const { key, isSig, isDate } of extractVariables(template.content)) {
        initial[key] = isDate ? todayISO : "";
        if (isSig) { initialColors[key] = "#000000"; initialStyles[key] = "cursive"; }
      }
      setVariables(initial);
      setSigImages({});
      setSigColors(initialColors);
      setSigStyles(initialStyles);
      setIsPublic(false);
      setDocFolder("");
  ...
  useEffect(() => {
    ...
    for (const { key } of sigVars) {
      ...
      const color = sigColors[key] || "#000000";
      const style = sigStyles[key] || "cursive";
      if (!name.trim()) { setSigImages(p => ({ ...p, [key]: "" })); continue; }
      generateSignatureHtml(name, color, style).then(html => setSigImages(p => ({ ...p, [key]: html })));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [variables, sigColors, sigStyles]);
    ...
    try {
      ...
      for (const [k, v] of Object.entries(variables)) {
        ...
        if (varDef?.isSig && v.trim()) {
          processedVars[k] = sigImages[k] || await generateSignatureHtml(v, sigColors[k] || "#000000", sigStyles[k] || "cursive");
        } else if (varDef?.isDate && v) {
          const parts = v.split("-");
          processedVars[k] = parts.length === 3 ? `${parts[2]}/${parts[1]}/${parts[0]}` : v;
        } else {
          processedVars[k] = v;
        }
      }
      const res = await fetch(`/api/admin/documents/${template.id}/from-template`, {
        ...
        body: JSON.stringify({
          title: docTitle || `${template.title} (rempli)`,
          variables: processedVars,
          is_public: isPublic,
          folder: docFolder.trim() || undefined,
        }),
      });
  ...
  function renderVarField(v: typeof orderedVars[0]) {
    const { key, label, isSig, isDate, options } = v;
    return (
      <div key={key} className={`space-y-1.5 ${isSig || isDate || options ? "col-span-2" : "col-span-1"}`}>
        <label className="text-sm text-gray-200 font-medium flex items-center gap-2 flex-wrap">
          ...
          {isDate && <CalendarDays className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />}
          ...
          {isDate && <span className="text-xs bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded font-normal">Date</span>}
          {options && <span className="text-xs bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded font-normal">Liste</span>}
        </label>
        {isDate ? (
          <input type="date" value={variables[key] || ""}
            onChange={e => setVariables(prev => ({ ...prev, [key]: e.target.value }))}
            className="w-full bg-slate-800 border border-amber-500/40 rounded-md px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Input value={variables[key] || ""} onChange={e => setVariables(prev => ({ ...prev, [key]: e.target.value }))}
              ...
              className="flex-1 min-w-[120px] bg-slate-800 border-amber-500/40 text-white focus:border-amber-400" />
            <select
              value={sigColors[key] || "#000000"}
              onChange={e => setSigColors(prev => ({ ...prev, [key]: e.target.value }))}
              className="bg-slate-800 border border-slate-600 text-white text-xs rounded-md px-2 py-2 focus:outline-none focus:border-amber-400 shrink-0"
            >
              {([ ["#000000","Noir"], ["#1a1a1a","Gris foncé"], ["#1a237e","Bleu nuit"], ["#0d47a1","Bleu"], ["#b71c1c","Rouge"], ["#1b5e20","Vert"], ["#4a148c","Violet"], ["#e65100","Orange"] ] as [string,string][]).map(([c, name]) => (
                <option key={c} value={c}>{name}</option>
              ))}
            </select>
            <select
              value={sigStyles[key] || "cursive"}
              onChange={e => setSigStyles(prev => ({ ...prev, [key]: e.target.value as SigStyle }))}
              className="bg-slate-800 border border-slate-600 text-white text-xs rounded-md px-2 py-2 focus:outline-none focus:border-amber-400 shrink-0"
            >
              {SIG_STYLES.map(({ id, name: label }) => (
                <option key={id} value={id}>{label}</option>
            ...
            <SigPreview
              name={variables[key] || ""}
              color={sigColors[key] || "#000000"}
              style={sigStyles[key] || "cursive"}
            />
          </div>
        ) : options ? (
          <select value={variables[key] || ""}
            onChange={e => setVariables(prev => ({ ...prev, [key]: e.target.value }))}
            className="w-full bg-slate-800 border border-slate-600 text-white text-sm rounded-md px-3 py-2 focus:outline-none focus:border-[#ffe599]">
            <option value="" disabled>— Choisir une option —</option>
            {renderOptionsGroups(options)}
          </select>
        ) : (
          <Input value={variables[key] || ""} onChange={e => setVariables(prev => ({ ...prev, [key]: e.target.value }))}
            placeholder={`Valeur pour ${label}...`} className="bg-slate-800 border-slate-600 text-white focus:border-[#ffe599]" />
        )}
      </div>
    );
  }
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="bg-slate-900 border-[#ffe599]/30 text-white max-w-2xl max-h-[90vh] overflow-y-auto overflow-x-hidden w-full">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
          ...
          {hasSteps && totalSteps > 1 ? (
            <div className="mt-3 space-y-2">
              <div className="flex gap-1">
                {wizardSteps.map((_, i) => (
                  <div key={i} className={`flex-1 h-1.5 rounded-full transition-all duration-300 ${i <= safeStepIdx ? "bg-[#ffe599]" : "bg-slate-700"}`} />
                ))}
              </div>
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-[#ffe599]">{currentWizardStep.title}</p>
                <p className="text-xs text-gray-500">Étape {safeStepIdx + 1} / {totalSteps}</p>
              </div>
            </div>
          ) : (
        ...
        <div className="space-y-4 py-2">
          ...
          {currentWizardStep.vars.length > 0 ? (
            <div className="space-y-3">
              {!hasSteps && (
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                {currentWizardStep.vars.map(v => renderVarField(v))}
              </div>
            </div>
          ) : (
            !hasSteps && (
          ...
          {/* Public + folder options: only on last step */}
          {isLastStep && (
            <div className="space-y-3 pt-3 border-t border-slate-700/60">
              <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Options du document créé</p>
              <div className="flex items-center justify-between bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2.5">
                <div className="flex items-center gap-2">
                  {isPublic ? <Globe className="w-4 h-4 text-green-400" /> : <Lock className="w-4 h-4 text-gray-500" />}
                  <div>
                    <p className="text-sm text-white font-medium">Rendre public</p>
                    <p className="text-xs text-gray-400">Accessible via lien de partage</p>
                  </div>
                </div>
              ...
              <div>
                <label className="text-sm text-gray-300 font-medium mb-1 flex items-center gap-2">
                  <Folder className="w-3.5 h-3.5 text-amber-400" /> Dossier <span className="text-gray-500 font-normal text-xs">(optionnel)</span>
                </label>
                <select value={folderIsNew ? "__new__" : docFolder}
                  onChange={e => { if (e.target.value === "__new__") { setFolderIsNew(true); setDocFolder(""); } else { setFolderIsNew(false); setDocFolder(e.target.value); } }}
                  className="w-full bg-slate-800 border border-slate-600 text-white text-sm rounded-md px-3 py-2 focus:outline-none focus:border-amber-400 mb-1.5">
                  <option value="">— Aucun dossier —</option>
                  {existingFolders.map(f => <option key={f} value={f}>{f}</option>)}
                  <option value="__new__">✏️ Nouveau dossier...</option>
                </select>
                {folderIsNew && (
                  <Input value={docFolder} onChange={e => setDocFolder(e.target.value)}
                    placeholder="Nom du nouveau dossier..." className="bg-slate-800 border-slate-600 text-white text-sm" autoFocus />
                )}
              </div>
            </div>
          )}
        ...
        <DialogFooter className="flex items-center justify-between sm:justify-between gap-2 flex-row">
          <div>
            {hasSteps && safeStepIdx > 0 && (
              <Button variant="outline" onClick={() => setCurrentStepIdx(i => i - 1)} className="border-slate-600 text-gray-300 hover:bg-slate-800">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={onClose}>Annuler</Button>
            {!isLastStep ? (
              <Button onClick={() => setCurrentStepIdx(i => i + 1)} className="bg-[#ffe599] hover:bg-[#ffd966] text-gray-900 font-semibold">
...
export function DocumentsAdmin({ permissions }: DocumentsAdminProps) {
  ...
  const [stepsManagerOpen, setStepsManagerOpen] = useState(false);
  ...
  const saveDoc = useCallback(async (docId: number, patch: Partial<Pick<Document, "title" | "content" | "is_public" | "is_template" | "is_readonly" | "folder">>) => {
    setSaving(true);
    try {
      ...
      if (res.ok) {
        ...
        setCurrentDoc(prev => {
          if (!prev) return null;
          const merged: Document = { ...prev, updated_at: updated.updated_at };
          if ("title" in patch) merged.title = updated.title;
          if ("content" in patch) merged.content = updated.content;
          if ("is_public" in patch) merged.is_public = updated.is_public;
          if ("is_template" in patch) merged.is_template = updated.is_template;
          if ("is_readonly" in patch) merged.is_readonly = updated.is_readonly;
          if ("folder" in patch) merged.folder = updated.folder;
          return merged;
        });
        setDocs(prev => prev.map(d => d.id === updated.id ? {
          ...
          is_readonly: updated.is_readonly,
  ...
  const handleEditorClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    else if (flagPart === "date") type = "date";
  ...
  const buildVarSpan = (key: string, label: string, isSig: boolean, options?: string[], isDate?: boolean): HTMLElement => {
    let flag = "";
    if (isSig) flag = "|sig";
    else if (isDate) flag = "|date";
    else if (options && options.length > 0) flag = `|drop:${options.join(",")}`;
    const raw = `{{${key}|${label}${flag}}}`;
    const icon = isSig ? " ✍" : options ? " ▾" : isDate ? " 📅" : "";
    const span = document.createElement("span");
  ...
  const handleInsertVariable = useCallback((key: string, label: string, isSig: boolean, options?: string[], isDate?: boolean) => {
    if (!editorRef.current) return;

    const newSpan = buildVarSpan(key, label, isSig, options, isDate);
  ...
  const [varDisplayOrder, setVarDisplayOrder] = useState<string[]>([]);

  // Sync varDisplayOrder when switching documents
  useEffect(() => {
    if (currentDoc?.is_template) {
      const saved = extractVarOrder(currentDoc.content);
      const keys = extractVariables(currentDoc.content).map(v => v.key);
      if (saved.length) {
        // merge: keep saved order, append any new keys not yet in order
        const ordered = [...saved.filter(k => keys.includes(k)), ...keys.filter(k => !saved.includes(k))];
        setVarDisplayOrder(ordered);
      } else {
        setVarDisplayOrder(keys);
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentDoc?.id]);

  // When content changes (new variable inserted), keep order in sync
  useEffect(() => {
    if (currentDoc?.is_template) {
      const keys = extractVariables(currentDoc.content).map(v => v.key);
      setVarDisplayOrder(prev => {
  ...
  const orderedTemplateVars = varDisplayOrder.length
    ? varDisplayOrder.map(k => templateVarsInCurrent.find(v => v.key === k)).filter(Boolean) as typeof templateVarsInCurrent
    : templateVarsInCurrent;

  const moveVarInTemplate = (idx: number, dir: -1 | 1) => {
    if (!currentDoc || !editorRef.current) return;
    setVarDisplayOrder(prev => {
      const arr = [...prev];
      const next = idx + dir;
      ...
      const newContent = embedVarOrder(editorRef.current!.innerHTML, arr);
      editorRef.current!.innerHTML = newContent;
      saveDoc(currentDoc.id, { content: newContent });
      return arr;
    });
  };
  ...
  const filteredDocs = useMemo(() => docs.filter(d => {
    ...
    if (folderFilter !== null && (d.folder || null) !== folderFilter) return false;
    return true;
  }), [docs, filterMode, searchQuery, folderFilter]);

  useEffect(() => { setDocPage(1); }, [filterMode, searchQuery, folderFilter]);
  ...
  if (currentDoc) {
    ...
    return (
      <div className="space-y-4">
        {/* Header — Row 1: back + title + folder */}
        <div className="space-y-2">
          <div className="flex items-center gap-3 min-w-0">
            <Button variant="outline" size="sm" onClick={() => { setCurrentDoc(null); fetchDocs(); }}
              className="shrink-0 border-amber-600/30 text-amber-400 hover:bg-amber-500/10">
            ...
            {editingTitle ? (
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <Input value={titleDraft} onChange={e => setTitleDraft(e.target.value)}
                  ...
                  autoFocus
                />
                <Button size="sm" onClick={saveTitle} className="bg-amber-500 text-black h-8 shrink-0"><Check className="w-4 h-4" /></Button>
                <Button size="sm" variant="ghost" onClick={() => setEditingTitle(false)} className="h-8 shrink-0"><X className="w-4 h-4" /></Button>
              </div>
            ) : (
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <h2 className="text-xl font-bold text-white truncate">{currentDoc.title}</h2>
                {isTemplateDoc && (
                ...
                {canEdit && (
                  folderEditing ? (
                      ...
                      <input
                        ...
                        onKeyDown={async e => {
                          if (e.key === "Enter") {
                            ...
                            setCurrentDoc(prev => prev ? { ...prev, folder: nf } : null);
                            await saveDoc(currentDoc.id, { folder: nf ?? undefined });
                          }
                    ...
                    <button
                      onClick={() => { setFolderDraft(currentDoc.folder || ""); setFolderEditing(true); }}
                      ...
                      {currentDoc.folder ? <span>{currentDoc.folder}</span> : <span className="italic">Dossier</span>}
                    </button>
                  )
                )}
              </div>
        ...
        {isTemplateDoc && (
          <div className="bg-[#ffe599]/10 border border-[#ffe599]/20 rounded-lg px-4 py-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs text-[#ffe599] font-medium flex items-center gap-2">
                <Variable className="w-4 h-4" /> Variables détectées dans ce template
              </p>
              {canEdit && orderedTemplateVars.length > 0 && (
                <button
                  type="button"
                  onClick={() => setStepsManagerOpen(true)}
                  className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg bg-[#ffe599]/20 border border-[#ffe599]/40 text-[#ffe599] hover:bg-[#ffe599]/30 transition-colors font-medium shrink-0"
                >
                  <ListOrdered className="w-3.5 h-3.5" />
                  {extractSteps(currentDoc?.content ?? "").length > 0
                    ? `Étapes (${extractSteps(currentDoc?.content ?? "").length})`
                    : "Gérer les étapes"}
                </button>
              )}
            </div>
            {orderedTemplateVars.length > 0 ? (
              <div className="flex flex-col gap-1.5">
                <p className="text-xs text-[#ffe599]/50 italic">Glissez ↑↓ pour définir l'ordre du formulaire</p>
                <div className="flex flex-wrap gap-2">
                {orderedTemplateVars.map(({ key, label }, idx) => (
                  <span key={key} className="flex items-center gap-1 text-xs bg-[#ffe599]/20 border border-[#ffe599]/30 text-[#ffe599] rounded px-2 py-0.5">
                    <span className="flex flex-col">
                      <button type="button" onClick={() => moveVarInTemplate(idx, -1)} disabled={idx === 0}
                        className="leading-none text-[#ffe599]/60 hover:text-[#ffe599] disabled:opacity-20 disabled:cursor-not-allowed">
                        <ChevronUp className="w-2.5 h-2.5" />
                      </button>
                      <button type="button" onClick={() => moveVarInTemplate(idx, 1)} disabled={idx === orderedTemplateVars.length - 1}
                        className="leading-none text-[#ffe599]/60 hover:text-[#ffe599] disabled:opacity-20 disabled:cursor-not-allowed">
        ...
        <StepsManagerDialog
          open={stepsManagerOpen}
          onClose={() => setStepsManagerOpen(false)}
          templateVars={templateVarsInCurrent}
          currentContent={currentDoc?.content ?? ""}
          onSave={(steps) => {
            if (!currentDoc || !editorRef.current) return;
            const newContent = embedSteps(editorRef.current.innerHTML, steps);
            editorRef.current.innerHTML = newContent;
    ...
    <div className="space-y-6">
      ...
      {docs.length > 0 && allFolders.length > 0 && (
        <div className="flex gap-2 flex-wrap items-center">
          <span className="text-xs text-gray-500 flex items-center gap-1 shrink-0"><Folder className="w-3.5 h-3.5" /> Dossier :</span>
          <button
      ...
      {/* Pagination controls */}
      {!loading && filteredDocs.length > DOCS_PER_PAGE && (
        <div className="flex items-center justify-center gap-3 pt-2">
          <Button
            size="sm" variant="outline"
            onClick={() => setDocPage(p => Math.max(1, p - 1))}
            disabled={docPage === 1}
            className="border-slate-700 text-gray-400 hover:text-white disabled:opacity-30">
            <ChevronLeft className="w-4 h-4" />
          </Button>
          <span className="text-sm text-gray-400">
            Page <span className="text-white font-semibold">{docPage}</span> / {totalDocPages}
            <span className="text-gray-600 ml-2">({filteredDocs.length} au total)</span>
          </span>
          <Button
            size="sm" variant="outline"
