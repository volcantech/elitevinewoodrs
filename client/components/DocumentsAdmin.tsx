...
function InsertVariableDialog({ open, onClose, onInsert }: {
  ...
  const preview = varKey.trim() ? `{{${varKey.trim()}|${varLabel.trim() || varKey.trim()}${flag}}}` : "";

  return (
    <Dialog open={open} onOpenChange={v => { if (!v) { reset(); onClose(); } }}>
      <DialogContent className="bg-slate-900 border-[#ffe599]/30 text-white max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Variable className="w-5 h-5 text-[#ffe599]" /> Insérer une variable
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-1">
          <div>
            ...
            <Input value={varKey} onChange={e => setVarKey(e.target.value.replace(/\s+/g, "_"))}
              onKeyDown={e => e.key === "Enter" && handle()}
              placeholder="poste_recrutement"
              className="bg-slate-800 border-[#ffe599]/30 text-white font-mono" autoFocus />
            {preview && <p className="text-xs text-[#ffe599] mt-1 font-mono break-all">→ <code className="bg-slate-800 px-1 rounded">{preview}</code></p>}
          </div>
          <div>
          ...
          {/* Type selector */}
          <div>
            <label className="text-xs text-gray-400 mb-1.5 block">Type de champ</label>
            <div className="grid grid-cols-3 gap-1.5">
              {([["text","Texte libre","✏️"],["sig","Signature","✍️"],["drop","Liste choix","▾"]] as [VarType,string,string][]).map(([t, lbl, icon]) => (
                <button key={t} type="button"
                  onMouseDown={e => { e.preventDefault(); setVarType(t); }}
                  className={`flex flex-col items-center gap-0.5 py-2 rounded-lg border text-xs font-medium transition-colors ${varType === t ? "bg-amber-500/20 border-amber-500 text-amber-300" : "bg-slate-800 border-slate-700 text-gray-400 hover:border-slate-500"}`}>
          ...
          {varType === "sig" && (
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
              <p className="text-xs text-amber-400 mb-2 font-medium">Aperçu :</p>
              <div style={{ fontFamily: "'Dancing Script', cursive", fontSize: "28px", color: "#1a237e", letterSpacing: "1px", borderBottom: "2px solid #1a237e", paddingBottom: "4px", display: "inline-block" }}>
          ...
          {varType === "drop" && (
            <div className="space-y-2">
              <label className="text-xs text-gray-400 block">Options de la liste</label>
              <div className="flex gap-1.5">
                ...
                <Button type="button" size="sm" onMouseDown={e => { e.preventDefault(); addOption(); }}
                  className="h-8 px-2 bg-slate-700 hover:bg-slate-600 text-white border-0">
              ...
              {dropOptions.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {dropOptions.map((opt, i) => (
                    <span key={i} className="flex items-center gap-1 bg-slate-700 text-gray-200 text-xs px-2 py-1 rounded-full">
                      {opt}
                      <button type="button" onMouseDown={e => { e.preventDefault(); setDropOptions(p => p.filter((_, j) => j !== i)); }}
                        className="text-gray-400 hover:text-red-400 ml-0.5"><X className="w-3 h-3" /></button>
                    </span>
                  ))}
                </div>
              )}
              {dropOptions.length === 0 && <p className="text-xs text-gray-600 italic">Ajoutez au moins une option</p>}
            </div>
          )}
        </div>
        <DialogFooter>
          ...
          <Button onClick={handle}
            ...
            className="bg-[#ffe599] hover:bg-[#ffd966] text-gray-900 font-semibold">Insérer</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function DocumentsAdmin({ permissions }: DocumentsAdminProps) {
  ...
  const handleInsertVariable = (key: string, label: string, isSig: boolean, options?: string[]) => {
    ...
    // contenteditable="false" prevents color from bleeding into next line/text
    // The trailing reset span ensures cursor lands in neutral styling
    const display = `<span style="color:#5a4000;background:#ffe599;border-radius:4px;padding:1px 5px;font-weight:600;font-size:0.92em;font-family:monospace;user-select:all;" data-var="${raw}" contenteditable="false">${raw}${icon}</span><span style="background:none;background-color:transparent;color:inherit;font-family:inherit;font-size:inherit;font-weight:normal;"> </span>`;
