...
function UseTemplateDialog({
  ...
  // Regenerate signature canvas image whenever a sig variable value, color, or style changes
  useEffect(() => {
        ...
        <div className="space-y-4 py-2">
          ...
          {templateVars.length > 0 ? (
            <div className="space-y-3">
              ...
              {orderedVars.map(({ key, label, isSig, isDate, options }) => (
                  ...
                  {isDate ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <SigColorPicker
                        ...
                        onChange={c => setSigColors(prev => ({ ...prev, [key]: c }))}
                      />
                      <SigStylePicker
                        value={sigStyles[key] || "cursive"}
                        onChange={s => setSigStyles(prev => ({ ...prev, [key]: s }))}
                        sampleName={variables[key] || ""}
                        color={sigColors[key] || "#000000"}
                      />
                      <div className="rounded-xl border border-amber-500/20 bg-white/5 p-3 flex flex-col items-start gap-2 min-h-[60px]">
                        <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Aperçu final</p>
                        {sigImages[key]
          ...
          <div className="space-y-3 pt-3 border-t border-slate-700/60">
            ...
            <div>
              ...
              <select
                value={folderIsNew ? "__new__" : docFolder}
                onChange={e => {
                  if (e.target.value === "__new__") { setFolderIsNew(true); setDocFolder(""); }
                  else { setFolderIsNew(false); setDocFolder(e.target.value); }
                }}
