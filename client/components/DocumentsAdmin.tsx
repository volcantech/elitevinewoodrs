...
import {
  ...
  Search, Eye, EyeOff, Folder, FolderOpen, CalendarDays,
} from "lucide-react";
...
async function generateSignatureHtml(name: string, color: string = "#000000"): Promise<string> {
  if (!name.trim()) return "";
...
function SignaturePreview({ name, color = "#000000" }: { name: string; color?: string }) {
  const [imgHtml, setImgHtml] = useState("");
  useEffect(() => {
    if (!name.trim()) { setImgHtml(""); return; }
    generateSignatureHtml(name, color).then(setImgHtml);
  }, [name, color]);
...
function UseTemplateDialog({
  ...
  useEffect(() => {
    if (template && open) {
      ...
      for (const { key, isSig, isDate } of extractVariables(template.content)) {
        ...
        if (isSig) initialColors[key] = "#000000";
      }
  ...
  // Regenerate signature canvas image whenever a sig variable value or color changes
  useEffect(() => {
    ...
    for (const { key } of sigVars) {
      ...
      generateSignatureHtml(name, color).then(html => setSigImages(p => ({ ...p, [key]: html })));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [variables, sigColors]);
    ...
    try {
      ...
      for (const [k, v] of Object.entries(variables)) {
        ...
        if (varDef?.isSig && v.trim()) {
          processedVars[k] = sigImages[k] || await generateSignatureHtml(v, sigColors[k] || "#000000");
        ...
        <div className="space-y-4 py-2">
          ...
          {templateVars.length > 0 ? (
            <div className="space-y-4">
              ...
              {templateVars.map(({ key, label, isSig, isDate, options }) => (
