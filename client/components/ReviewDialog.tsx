...
import { Star, MessageSquare, Send, Loader2, User, Lock, ChevronLeft, ChevronRight, Flag, ThumbsUp, BarChart2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  const handleLike = async (reviewId: number, reactionType: "like" | "utile" | "drole") => {
    try {
      const res = await fetch(`/api/reviews/${reviewId}/like`, {
        ...
        body: JSON.stringify({ visitorId: visitorId(), reactionType }),
      });
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          ...
                          {(likesData[review.id]?.like || 0) > 0 && (
                            <span>{likesData[review.id].like}</span>
                          )}
                        </button>
                        <button
                          onClick={() => handleLike(review.id, "utile")}
                          className={`flex items-center gap-0.5 text-xs px-1.5 py-0.5 rounded transition-colors ${
                            likesData[review.id]?.utiled
                        ...
                        >
                          ...
                          {(likesData[review.id]?.utile || 0) > 0 && (
                            <span>{likesData[review.id].utile}</span>
                          )}
                        </button>
                        <button
                          onClick={() => handleLike(review.id, "drole")}
                          className={`flex items-center gap-0.5 text-xs px-1.5 py-0.5 rounded transition-colors ${
                            likesData[review.id]?.droled
                        ...
                        >
                          <span className="text-xs">😂</span>
                          {(likesData[review.id]?.drole || 0) > 0 && (
                            <span>{likesData[review.id].drole}</span>
                          )}
                        </button>
                      </div>
