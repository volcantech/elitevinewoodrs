...
import { Star, MessageSquare, Send, Loader2, User, Lock, ChevronLeft, ChevronRight, Flag, ThumbsUp } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function ReviewDialog({ vehicle, open, onClose }: ReviewDialogProps) {
  ...
  const handleLike = async (reviewId: number) => {
    try {
      ...
      setLikesData(prev => ({
        ...prev,
        [reviewId]: { count: result.likeCount, liked: result.liked },
      }));
    } catch {}
  };
  ...
  const sortedReviews = data?.reviews
    ? sortByLikes
      ? [...data.reviews].sort((a, b) => (likesData[b.id]?.count || 0) - (likesData[a.id]?.count || 0))
      : data.reviews
    : [];
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <ThumbsUp className={`w-3 h-3 ${likesData[review.id]?.liked ? "fill-amber-400" : ""}`} />
                        {(likesData[review.id]?.count || 0) > 0 && (
                          <span>{likesData[review.id].count}</span>
                        )}
                      </button>
