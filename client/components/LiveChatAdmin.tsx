...
export function LiveChatAdmin() {
  ...
  const statusFilterRef = useRef<"open" | "closed">("open");
  ...
  const loadMessages = useCallback(async (sessionId: number) => {
    ...
    window.dispatchEvent(new CustomEvent("livechat-messages-read"));
  }, []);
  ...
  useEffect(() => {
    ...
    const handleSessionClosed = (e: Event) => {
      ...
      if (statusFilterRef.current === "open") {
        ...
        if (selectedIdRef.current === data.sessionId) {
    ...
    const handleDataChanged = () => { fetchSessions(); };
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {session.status === "closed" && <span className="text-[10px] bg-red-500/20 text-red-400 border border-red-500/30 rounded px-1 shrink-0">Fermé</span>}
                      </div>
