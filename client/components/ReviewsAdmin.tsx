...
export function ReviewsAdmin({ token, permissions }: ReviewsAdminProps) {
  ...
  const fetchReviews = useCallback(async (p: number, pseudo: string) => {
    ...
    try {
      ...
      const res = await authenticatedFetch(`/api/reviews/all?${params.toString()}`);
  ...
  const handleDelete = async (reviewId: number, pseudo: string) => {
    ...
    try {
      const res = await authenticatedFetch(`/api/reviews/${reviewId}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Erreur");
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-amber-600/20 hover:bg-transparent">
                <TableHead className="text-amber-300 font-semibold w-8">#</TableHead>
                ...
                <TableRow key={review.id} className="border-amber-600/10 hover:bg-amber-500/5 transition-colors">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 bg-amber-500/20 rounded-full flex items-center justify-center shrink-0">
                  ...
                  <TableCell className="max-w-[260px]">
                    {review.comment ? (
                      <p className="text-sm text-gray-300 truncate" title={review.comment}>
                        {review.comment}
                      </p>
                    ) : (
