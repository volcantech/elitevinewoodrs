...
import { useGroupCallContext, GCParticipant, QUALITY_PRESETS } from "@/contexts/GroupCallContext";

function QualityPicker() {
  const [open, setOpen] = useState(false);
  const [current, setCurrent] = useState<string>(QUALITY_PRESETS[1].label);
      ...
      {open && (
        <div className="absolute bottom-full mb-2 right-0 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden z-10 min-w-[150px]">
          {QUALITY_PRESETS.map(p => (
            <button
              key={p.label}
              onClick={() => { applyLocalScreenQuality(p); setCurrent(p.label); setOpen(false); }}
