...
function ParticipantAvatar({ p, isMe, localMuted, speaking }: { p: GCParticipant; isMe: boolean; localMuted?: boolean; speaking?: boolean }) {
  const showMuted = isMe ? !!localMuted : !!p.isMuted;
  return (
    <div className="relative flex-shrink-0" title={`${isMe ? "Vous" : p.username}${showMuted ? " (micro coupé)" : ""}${speaking ? " (parle)" : ""}`}>
      ...
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-3 bg-gray-900 border border-green-500/30 rounded-full px-4 py-2.5 shadow-2xl">
        ...
        <div className="flex items-center -space-x-1.5">
          {allParticipants.slice(0, 5).map(p => (
            <ParticipantAvatar key={p.userId} p={p} isMe={p.userId === localUser?.userId} localMuted={isMuted} speaking={speakingUserIds.has(p.userId)} />
          ))}
