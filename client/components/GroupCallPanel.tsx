...
function QualityPicker({ screenShareUserId, isLocal, onSelect }: { screenShareUserId: number | null; isLocal: boolean; onSelect: (p: QualityPreset) => void }) {
  const [open, setOpen] = useState(false);
  const { requestQuality } = useGroupCallContext();

  function pick(p: QualityPreset) {
    if (isLocal) {
      onSelect(p);
    } else if (screenShareUserId !== null) {
      requestQuality(screenShareUserId, p);
    }
    setOpen(false);
  }
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute bottom-full mb-2 right-0 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden z-10 min-w-[150px]">
...
function ScreenShareOverlay({
  stream, sharer, onStop, isLocal, minimized, onToggleMinimize, screenShareUserId,
}: {
  stream: MediaStream;
    ...
    <div className="fixed inset-0 z-[180] bg-black/95 flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <QualityPicker screenShareUserId={screenShareUserId} isLocal={isLocal} onSelect={() => {}} />
          <button
...
function ParticipantAvatar({ p, isMe, localMuted, speaking, isMutedLocally, onToggleMute }: { p: GCParticipant; isMe: boolean; localMuted?: boolean; speaking?: boolean; isMutedLocally?: boolean; onToggleMute?: () => void }) {
  ...
  return (
    <div className="relative flex-shrink-0 group/avatar" title={`${isMe ? "Vous" : p.username}${showMuted ? " (micro coupé)" : ""}${speaking ? " (parle)" : ""}`}>
      ...
      {showMuted && (
        <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-red-500 rounded-full flex items-center justify-center z-10">
