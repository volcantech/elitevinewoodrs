...
export default function Admin() {
  ...
  const [particularities, setParticularities] = useState<string[]>([]);
  ...
  const fetchParticularities = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setParticularities(data.map((p: any) => p.name).sort());
      }
    } catch (error) {
              ...
              <Tabs defaultValue="catalog" className="w-full">
                  ...
                  <div className="bg-gradient-to-r from-slate-900/50 to-slate-800/50 border border-amber-600/30 rounded-lg p-6 space-y-4 shadow-lg">
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                      ...
                      <AddVehicleDialog 
                        ...
                        token={token} 
                        onVehicleAdded={() => fetchVehicles(token)} 
                      />
                    </div>
                ...
                {currentUser?.permissions?.particularities?.view && (
                  <TabsContent value="particularities">
                    <ParticularityAdmin token={token} onRefresh={() => fetchParticularities()} />
                  </TabsContent>
                )}
              </Tabs>
            </TabsContent>
