...
import { SpreadsheetsAdmin } from "@/components/SpreadsheetsAdmin";
import { DocumentsAdmin } from "@/components/DocumentsAdmin";
import { Search, Car, ShoppingCart, Users, Shield, ChevronLeft, ChevronRight, Loader2, Lock, EyeOff, Eye, BarChart2, MessageSquare, Table2, FileText } from "lucide-react";
...
export default function Admin() {
  ...
  const BADGE_LS_KEY = "admin_badge_seen_at";
  type BadgeKey = "orders" | "chat" | "reviews" | "tickets" | "reports";
  const NOTIF_TYPE_MAP: Record<string, BadgeKey> = {
    order_new: "orders", order_message: "orders",
    chat_new: "chat", chat_message: "chat",
    review_new: "reviews",
    ticket_new: "tickets", ticket_reply: "tickets",
    report_new: "reports",
  };
  const TAB_BADGE_MAP: Record<string, BadgeKey[]> = {
    orders: ["orders"],
    chat: ["chat"],
    moderation: ["reviews", "tickets", "reports"],
  };

  const [tabBadges, setTabBadges] = useState<Record<BadgeKey, number>>({ orders: 0, chat: 0, reviews: 0, tickets: 0, reports: 0 });

  const getSeenAt = useCallback((): Record<BadgeKey, string> => {
    const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    try {
      const raw = JSON.parse(localStorage.getItem(BADGE_LS_KEY) || "{}");
      return { orders: raw.orders || weekAgo, chat: raw.chat || weekAgo, reviews: raw.reviews || weekAgo, tickets: raw.tickets || weekAgo, reports: raw.reports || weekAgo };
    } catch { return { orders: weekAgo, chat: weekAgo, reviews: weekAgo, tickets: weekAgo, reports: weekAgo }; }
  }, []);

  const saveSeenAt = useCallback((keys: BadgeKey[]) => {
    try {
      const current = JSON.parse(localStorage.getItem(BADGE_LS_KEY) || "{}");
      const now = new Date().toISOString();
      keys.forEach(k => { current[k] = now; });
      localStorage.setItem(BADGE_LS_KEY, JSON.stringify(current));
    } catch {}
  }, []);

  const fetchTabBadges = useCallback(async (token?: string | null) => {
    if (!token) return;
    const seen = getSeenAt();
    try {
      const params = new URLSearchParams({ orders_since: seen.orders, chat_since: seen.chat, reviews_since: seen.reviews, tickets_since: seen.tickets, reports_since: seen.reports });
      const res = await fetch(`/api/admin/tab-badges?${params}`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
        const data = await res.json();
        setTabBadges({ orders: data.orders || 0, chat: data.chat || 0, reviews: data.reviews || 0, tickets: data.tickets || 0, reports: data.reports || 0 });
      }
    } catch {}
  }, [getSeenAt]);
  ...
  const setActiveTab = (tab: string) => {
    ...
    const keys = TAB_BADGE_MAP[tab];
    if (keys) {
      const nonChatKeys = keys.filter(k => k !== "chat");
      if (nonChatKeys.length) saveSeenAt(nonChatKeys);
      setTabBadges(prev => {
        const next = { ...prev };
      ...
      if (keys.includes("chat")) {
        setTimeout(() => fetchTabBadges(publicToken), 800);
      }
    }
  };

  const markModSubTabSeen = useCallback((subTab: string) => {
    const keyMap: Record<string, BadgeKey> = { reviews: "reviews", tickets: "tickets", reports: "reports" };
    const key = keyMap[subTab];
    if (key) {
      saveSeenAt([key]);
  ...
  useEffect(() => {
    if (!publicUser?.is_admin) return;
    fetchTabBadges(publicToken);
    const handle = (e: Event) => {
      const data = (e as CustomEvent).detail;
      if (data?.type !== "admin_notification") return;
      const key = NOTIF_TYPE_MAP[data?.notifType as string];
      if (!key) return;
      if (key === "chat") {
        fetchTabBadges(publicToken);
      } else {
        setTabBadges(prev => ({ ...prev, [key]: prev[key] + 1 }));
      }
    };
    const handleChatRead = () => { fetchTabBadges(publicToken); };
    window.addEventListener("admin-data-changed", handle);
    window.addEventListener("livechat-messages-read", handleChatRead);
    return () => {
      window.removeEventListener("admin-data-changed", handle);
      window.removeEventListener("livechat-messages-read", handleChatRead);
    };
  }, [publicUser?.is_admin, publicToken, fetchTabBadges]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        {!currentUser?.permissions?.vehicles?.view &&
          ...
          !currentUser?.permissions?.badges?.view &&
          !currentUser?.permissions?.loyalty?.manage &&
          ...
          <>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="bg-slate-800/50 border border-amber-600/30 flex-nowrap md:flex-wrap overflow-x-auto md:overflow-x-visible w-full justify-start">
                {currentUser?.permissions?.vehicles?.view && (
                  <TabsTrigger
                    value="vehicles"
                ...
                {currentUser?.permissions?.orders?.view && (
                  ...
                  >
                    ...
                    Commandes
                    <TabBadge count={tabBadges.orders} />
                  </TabsTrigger>
                )}
                {currentUser?.permissions?.users?.view && (
                  <TabsTrigger
                ...
                {(currentUser?.permissions?.moderation?.view ||
                  ...
                  >
                    ...
                    Modération
                    <TabBadge count={tabBadges.reviews + tabBadges.tickets + tabBadges.reports} />
                  </TabsTrigger>
                )}
                {currentUser?.permissions?.orders?.view && (
                  <TabsTrigger
                ...
                {currentUser?.permissions?.spreadsheets?.view && (
                  <TabsTrigger
                    value="spreadsheets"
                    className="data-[state=active]:bg-amber-500 data-[state=active]:text-black"
                  >
                ...
                {currentUser?.permissions?.documents?.view && (
                  <TabsTrigger
                    value="documents"
                    className="data-[state=active]:bg-amber-500 data-[state=active]:text-black"
                  >
                ...
                <TabsContent value="vehicles" className="space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-slate-900/50 to-slate-800/50 border border-amber-600/30 rounded-lg p-6 space-y-4 shadow-lg">
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                      ...
                      <AddVehicleDialog categories={categories} token={token} onVehicleAdded={() => { fetchVehicles(currentPage); fetchCategories(); }} />
                    </div>
                  ...
                  {isLoading ? (
                    ...
                    <>
                      ...
                      <VehicleAdminTable
                        ...
                        token={token}
                        onRefresh={() => { fetchVehicles(currentPage); fetchCategories(); }}
                        onSort={(field, order) => {
                          setSortBy(field);
                          setSortOrder(order);
              ...
              {(currentUser?.permissions?.moderation?.view ||
                ...
                <TabsContent value="moderation">
                  <ModerationAdmin token={token} currentUser={currentUser || undefined} permissions={currentUser?.permissions} subBadges={{ reviews: tabBadges.reviews, tickets: tabBadges.tickets, reports: tabBadges.reports }} onSubTabSeen={markModSubTabSeen} />
                </TabsContent>
              )}
              {currentUser?.permissions?.orders?.view && (
              ...
              {currentUser?.permissions?.spreadsheets?.view && (
                <TabsContent value="spreadsheets">
                  <SpreadsheetsAdmin permissions={currentUser.permissions.spreadsheets} />
                </TabsContent>
              )}
              {currentUser?.permissions?.documents?.view && (
                <TabsContent value="documents">
                  <DocumentsAdmin permissions={currentUser.permissions.documents} />
                </TabsContent>
              )}
