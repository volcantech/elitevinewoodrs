...
import {
  ...
  Pencil, X, Save, Calendar, Upload, Trash2, ImageIcon, Settings2, Eye, Camera,
} from "lucide-react";
...
export default function Profile() {
  ...
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setAvatarPreview(profile?.user.avatar_url || null);
    } finally {
  ...
  const { user, friendCount, lastLogin, lastSeen, earnedBadges: earnedBadgesRaw, earnedCustomBadges, favoriteVehicles, isOnline, friendshipStatus, friendshipRequester, friendsSince, iBlockedThem } = profile;
              ...
              {isOwn ? (
                <div className="relative inline-block group">
                  ...
                  <button
                    ...
                    className="relative w-24 h-24 rounded-full bg-amber-500/20 border-4 border-slate-900 flex items-center justify-center overflow-hidden ring-2 ring-amber-500/20 shadow-2xl focus:outline-none"
                    ...
                    {avatarUploading ? (
                      ...
                      <>
                        {avatarPreview ? (
                          ...
                          <span className="font-bold text-3xl text-amber-400">{user.username[0]?.toUpperCase()}</span>
                        )}
                  ...
                  <span
                    className={`absolute bottom-1 right-1 w-5 h-5 border-[3px] rounded-full border-slate-900 shadow-sm ${isOnline ? "bg-green-500" : "bg-red-500"}`}
                    title={isOnline ? "En ligne" : "Hors ligne"}
                  />
                </div>
              ) : (
                <AvatarComp user={{ ...user, avatar_url: avatarPreview ?? user.avatar_url }} size="xl" isOnline={isOnline} />
              )}
              <div className="flex gap-2 pb-1">
                ...
                {isOwn && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-700/60 hover:bg-slate-700 border border-gray-600/50 transition-all text-gray-400 hover:text-gray-200 text-xs font-medium"
            ...
            <div className="flex flex-wrap items-center gap-4 mb-5 text-sm">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {isOnline ? (
                  ...
                  <span className="text-gray-400 text-sm">Hors ligne — Vu le {new Date(lastSeen).toLocaleString("fr-FR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                ) : lastLogin ? (
              ...
              {friendsSince && (
                <div className="flex items-center gap-1.5 text-gray-500">
                  ...
                  <span className="text-xs">Ami(e) depuis le {new Date(friendsSince).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</span>
                </div>
              )}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {friendshipStatus === "accepted" ? (
                  ...
                  <>
                    ...
                    {!profile.user.messages_from_friends_only && (
                      <button
                        onClick={() => navigate(`/messages?userId=${targetId}`)}
