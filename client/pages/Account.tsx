...
import {
  ...
  Lock, Shield, Zap, Plus, Ticket, Clock, AlertTriangle, Award,
} from "lucide-react";
...
function OrderCard({ order, token, onCancelled }: { order: Order; token: string | null; onCancelled: () => void }) {
  ...
  const downloadInvoice = async () => {
    ...
    const ptsGagnes = Math.floor(order.total_price / 250000) * 5;
    if (ptsGagnes > 0) {
      y += 10;
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
...
export default function Account() {
  ...
  const rankInfo = accountDays >= 90 ? { label: "VIP", color: "text-amber-400", bg: "bg-amber-500/15 border-amber-500/30" }
    : accountDays >= 30 ? { label: "Membre", color: "text-blue-400", bg: "bg-blue-500/15 border-blue-500/30" }
    : { label: "Nouveau", color: "text-green-400", bg: "bg-green-500/15 border-green-500/30" };
          ...
          <div className="flex items-center gap-4 sm:gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                ...
                <span className={`text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 rounded-full border font-semibold ${rankInfo.bg} ${rankInfo.color}`}>
                  {rankInfo.label}
                </span>
              </div>
              ...
              {badgesData && badgesData.earnedCount > 0 && (
                <div className="mt-2 flex items-center gap-1 sm:gap-1.5 flex-wrap">
                  {badgesData.badges.filter((b: any) => b.unlocked).slice(0, 4).map((b: any) => (
                    <span
                      key={b.id}
                      className="inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 rounded-lg text-[10px] sm:text-[11px] font-semibold bg-gray-800/80 border border-gray-700/50"
            ...
            {loyaltyLoading ? (
              ...
              <div className="space-y-5">
                <div className="bg-gray-900 border border-amber-600/30 rounded-xl p-3.5 sm:p-5">
                  ...
                  <p className="text-xs text-gray-500 mt-4">5 points par tranche de 250 000$ commandée (commandes livrées). Les remises sont appliquées par le patron sur demande.</p>
                </div>
          ...
          <TabsContent value="badges">
            {badgesLoading ? (
              ...
              <div className="space-y-6">
                ...
                {(["purchase", "review", "social", "special"] as const).map((cat) => {
                  const catLabels: Record<string, string> = { purchase: "Achats", review: "Avis", social: "Social", special: "Spécial" };
