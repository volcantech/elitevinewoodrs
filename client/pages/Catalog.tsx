...
export default function Catalog() {
  ...
  const { data: reviewsSummary = {} } = useQuery<Record<number, { average_rating: number; review_count: number }>>({
  ...
  const filteredVehicles = useMemo(() => {
    ...
    let filtered = vehicles.filter((vehicle) => {
      if (showFavorites && !favorites.has(vehicle.id)) return false;
    ...
    return sorted;
  }, [vehicles, selectedCategory, searchQuery, minWeight, minBudget, maxBudget, selectedSeats, selectedParticularity, selectedManufacturer, sortBy, showFavorites, favorites]);
  ...
  const { data: apiParticularities = [] } = useQuery({
    queryKey: ["activeParticularities"],
    queryFn: async () => {
  ...
  const availableParticularities = useMemo(() => {
    ...
    const fromApi = apiParticularities.map((p: any) => p.name);
    return Array.from(new Set([...fromVehicles, ...fromApi])).sort();
  }, [vehicles, apiParticularities]);
  ...
  }, [selectedCategory, searchQuery, minWeight, minBudget, maxBudget, selectedSeats, selectedParticularity, selectedManufacturer, sortBy, showFavorites]);
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-lg transition-all border ${
                      showFavorites
                        ...
                        : "bg-gray-700/50 border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-red-400"
                    }`}
                  >
                    <Heart className={`w-4 h-4 ${showFavorites ? "fill-red-500 text-red-500" : ""}`} />
                    {showFavorites
                      ? `Afficher le catalogue complet`
                      : `Afficher mes favoris (${favorites.size})`}
                  </button>
                  {hasActiveFilters && (
                    <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visibleVehicles.map((vehicle) => (
                  <VehicleCard
                    ...
                    ratingSummary={reviewsSummary[Number(vehicle.id)]}
                    onVehicleClick={(v) => {
