...
import { Search, ChevronDown, Package, DollarSign, X, Users, ArrowUpDown, Sparkles, Building, Info, Heart, LayoutGrid, List } from "lucide-react";
...
import { GTA_BRAND_MAPPING } from "@/data/brands";

const VEHICLES_PER_PAGE = 9;

type SortOption = "alphabetical" | "price-asc" | "price-desc" | "trunk-asc" | "trunk-desc" | "rating-desc";
...
export default function Catalog() {
  ...
  useEffect(() => {
    const handler = () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["category-max-pages"] });
    };
  ...
  const { favorites, toggleFavorite, isFavorite } = useFavorites(publicToken);
  ...
  const filteredVehicles = useMemo(() => {
    ...
    let filtered = vehicles.filter((vehicle) => {
      ...
      const matchesSearch = vehicle.name.toLowerCase().includes(searchLower) ||
        (vehicle.manufacturer && vehicle.manufacturer.toLowerCase().includes(searchLower)) ||
        (vehicle.realname && vehicle.realname.toLowerCase().includes(searchLower));
      ...
      const matchesFavorites = !showFavoritesOnly || isFavorite(vehicle.id);
      return matchesCategory && matchesSearch && matchesWeight && matchesBudget && matchesSeats && matchesParticularity && matchesManufacturer && matchesFavorites;
    });
    ...
    const sorted = [...filtered].sort((a, b) => {
      switch (sortBy) {
        ...
        case "rating-desc": {
          const aRating = reviewSummaries[String(a.id)]?.average ?? -1;
          const bRating = reviewSummaries[String(b.id)]?.average ?? -1;
    ...
    return sorted;
  }, [vehicles, selectedCategory, searchQuery, minWeight, minBudget, maxBudget, selectedSeats, selectedParticularity, selectedManufacturer, sortBy, showFavoritesOnly, favorites, reviewSummaries]);
  ...
  // Derive available categories from loaded vehicles (already filtered by API for disabled ones)
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-black via-gray-950 to-black">
        <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-amber-500/20 rounded-xl p-4 shadow-lg">
                    ...
                    <select
                      ...
                      className="custom-select w-full bg-gray-700/50 border border-gray-600 rounded-lg pl-4 pr-10 py-2.5 text-white focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/30 transition-all cursor-pointer hover:bg-gray-700"
                    >
                  ...
                  <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-amber-500/20 rounded-xl p-4 shadow-lg">
                    ...
                    >
                      ...
                      <option value="rating-desc">Avis : de la meilleure à la moins bonne note</option>
                    </select>
                  </div>
          ...
          {/* Results Info */}
          <div className="mb-8 flex flex-wrap items-center justify-between gap-3 bg-gradient-to-r from-gray-800/50 to-gray-900/50 border border-gray-700 rounded-xl p-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="flex items-center border border-gray-600 rounded-lg overflow-hidden">
                    <button
                      ...
                      className={`p-2 transition-colors ${viewMode === "grid" ? "bg-amber-500/20 text-amber-400" : "bg-gray-700/50 text-gray-400 hover:text-white"}`}
                      title="Vue grille"
                    >
                    ...
                    <button
                      ...
                      className={`p-2 transition-colors ${viewMode === "list" ? "bg-amber-500/20 text-amber-400" : "bg-gray-700/50 text-gray-400 hover:text-white"}`}
                      title="Vue liste"
                    >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 text-sm font-semibold transition-all px-4 py-2 rounded-lg border ${
                      showFavoritesOnly
                        ...
                        : "bg-gray-700/50 border-gray-600 text-gray-300 hover:bg-red-500/10 hover:border-red-500/30 hover:text-red-400"
                    }`}
                  >
                    <Heart className={`w-4 h-4 ${showFavoritesOnly ? "fill-red-400 text-red-400" : ""}`} />
                    {showFavoritesOnly
                      ? `Afficher le catalogue complet`
                      : `Afficher mes favoris${favorites.size > 0 ? ` (${favorites.size})` : ""}`}
                  </button>
                  {hasActiveFilters && (
                    <button
          ...
          {isLoading ? (
              ...
              <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "flex flex-col gap-4"}>
                {visibleVehicles.map((vehicle) => (
                  <VehicleCard
                    ...
                    reviewSummary={reviewSummaries[vehicle.id]}
            ...
            <div className="text-center py-24">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">
                {showFavoritesOnly ? "Aucun favori trouvé" : "Aucun véhicule trouvé"}
              </h3>
              <p className="text-gray-400 mb-6 max-w-sm mx-auto">
                {showFavoritesOnly
                  ? "Ajoutez des véhicules à vos favoris en cliquant sur le cœur ❤️"
                  : "Essayez de modifier vos critères de recherche"}
              </p>
              {showFavoritesOnly ? (
                <button
                  onClick={() => setShowFavoritesOnly(false)}
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-red-500/20 to-pink-500/20 border border-red-500/30 text-red-400 hover:bg-red-500/30 font-bold py-2 px-6 rounded-lg transition-all duration-300"
                >
