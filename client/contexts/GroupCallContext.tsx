...
export interface QualityPreset { label: string; maxBitrate: number; maxFramerate: number; }
...
export function GroupCallProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const handler = async (e: Event) => {
      ...
      if (data.type === "group_call_force_leave") {
        // Server auto-kicked us (30s alone timer)
        leaveGroupCall();
        toast.info("📵 Appel terminé — aucun participant n'a rejoint");
        return;
      }
  ...
  return (
    <GroupCallContext.Provider value={{
      ...
      mutedParticipants, toggleParticipantMute, requestQuality,
