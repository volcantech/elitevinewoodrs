...
interface PublicAuthContextType {
  ...
  register: (username: string, password: string, uniqueId: string) => Promise<void>;
  logout: () => Promise<void>;
...
export function PublicAuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (username: string, password: string, uniqueId: string) => {
    const res = await fetch("/api/public/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username, password, uniqueId }),
    });
    const data = await res.json();
