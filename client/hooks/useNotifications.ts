...
export interface StoredNotification {
  id: string;
  type: "message" | "delivered" | "cancelled" | "ticket" | "report" | "ticket_closed" | "giveaway_win" | "private_message" | "friend_request" | "friend_accepted" | "badge" | "chat" | "reaction" | "like";
  title: string;
  body: string;
  date: string;
  orderId?: number;
...
function playAudioMessageSound() {
  try {
    const ctx = new (window.AudioContext || (window as any).webkitAudioContext)();
    const gain = ctx.createGain();
    gain.connect(ctx.destination);
...
export function useNotifications(token: string | null, userId?: number | null) {
  ...
  const userIdRef = useRef<number | null | undefined>(null);
  ...
  const processNotificationEvent = useCallback((data: any, currentToken: string) => {
    ...
    if (data.type === "status_change") {
      ...
      window.dispatchEvent(new CustomEvent("user-data-changed", { detail: data }));
      ...
      if (data.messageType === "audio") {
        playAudioMessageSound();
        setUnreadMessages((prev) => prev + 1);
        setUnreadCount((prev) => prev + 1);
        toast.info(`🎤 ${data.senderUsername}`, {
          description: "vous a envoyé un message vocal",
          action: { label: "Écouter", onClick: () => { window.location.href = `/messages?userId=${data.senderId}`; } },
          duration: 8000,
        });
        persistNotifications([{
      ...
      const preview = data.messageType === "image" ? "📷 A envoyé une image" : (data.content || "").slice(0, 80);
      toast.info(`💬 ${data.senderUsername}`, {
        description: preview,
        action: {
          label: "Répondre",
    ...
    try {
          ...
          if (tok && uid) {
            fetch("/api/public/notifications/history?limit=50", {
              ...
              .then((d) => {
                ...
                const lastSeen = localStorage.getItem(key);
                const lastSeenDate = lastSeen ? new Date(lastSeen) : new Date(0);
                const newCount = d.notifications.filter((n: any) => new Date(n.date || n.created_at) > lastSeenDate).length;
      ...
      ws.onmessage = (event) => {
        try {
          ...
          const GROUP_CALL_TYPES = ["group_call_members","group_call_user_joined","group_call_user_left","group_call_offer","group_call_answer","group_call_ice","group_call_screen_start","group_call_screen_stop","group_call_active_members","call_group_banned","group_call_incoming","group_call_force_leave","group_call_mute","group_call_quality_request"];
          ...
          const GROUP_TYPES = ["group_message","group_created","group_renamed","group_member_added","group_member_removed","group_member_left","group_deleted","group_message_deleted","group_message_edited","group_settings_updated","group_lead_changed","group_message_pinned","group_message_reaction","group_typing","group_typing_stop"];
          if (GROUP_TYPES.includes(data.type)) {
            if (data.type === "group_message") {
              ...
              if (!isCallLog) {
                ...
                if (isAudio) { playAudioMessageSound(); } else { playMessageSound(); }
                setUnreadMessages((prev) => prev + 1);
                const preview = isAudio ? "🎤 a envoyé un message vocal" : data.messageType === "image" ? "📷 A envoyé une image" : (data.content || "").slice(0, 80);
                const toastTitle = isAudio ? `🎤 ${data.senderUsername}` : `💬 ${data.senderUsername} dans le groupe : ${groupLabel}`;
                const toastDesc = isAudio ? `a envoyé un message vocal dans le groupe : ${groupLabel}` : preview;
                toast.info(toastTitle, {
                  description: toastDesc,
                  action: { label: isAudio ? "Écouter" : "Voir", onClick: () => { window.location.href = `/messages`; } },
                  duration: 8000,
                });
                if (currentToken) {
                  persistNotifications([{
            ...
            if (data.type === "group_member_added" && Number(data.user?.id) === Number(userIdRef.current)) {
              playNotificationSound();
              setUnreadCount((prev) => prev + 1);
              const addedByText = data.actorUsername ? `${data.actorUsername} vous a ajouté` : "Vous avez été ajouté";
              toast.success(`➕ Ajouté au groupe « ${data.groupName} »`, {
                description: `${addedByText} au groupe « ${data.groupName} »`,
                action: { label: "Voir", onClick: () => { window.location.href = `/messages`; } },
                duration: 10000,
              });
              if (currentToken) {
                persistNotifications([{
                  ...
                  type: "private_message",
                  title: `➕ Ajouté au groupe « ${data.groupName} »`,
                  body: `${addedByText} au groupe « ${data.groupName} »`,
                  date: new Date().toISOString(),
                }], currentToken);
              }
            }
            if (data.type === "group_member_removed" && Number(data.userId) === Number(userIdRef.current)) {
              playNotificationSound();
              setUnreadCount((prev) => prev + 1);
            ...
            if (
              data.type === "group_message_reaction" &&
              data.action === "added" &&
              Number(data.reactorId) !== Number(userIdRef.current) &&
              Number(data.messageSenderId) === Number(userIdRef.current)
            ) {
              playNotificationSound();
              setUnreadCount((prev) => prev + 1);
              if (!window.location.pathname.startsWith("/messages")) {
                toast.info(`${data.reactorUsername || "Quelqu'un"} a réagi à votre message`, {
                  description: `${data.emoji} dans « ${data.groupName || "le groupe"} »`,
                  duration: 5000,
                });
              }
              if (currentToken) {
                persistNotifications([{
                  id: `reaction-grp-${data.messageId}-${data.reactorId}-${Date.now()}`,
                  type: "reaction",
                  title: `${data.reactorUsername || "Quelqu'un"} a réagi à votre message`,
                  body: `${data.emoji} dans « ${data.groupName || "le groupe"} »`,
                  date: new Date().toISOString(),
          ...
          if (data.type === "pm_reaction") {
            const uid = Number(userIdRef.current);
            if (data.action === "added" && Number(data.reactorId) !== uid && Number(data.messageSenderId) === uid) {
              playNotificationSound();
              setUnreadCount((prev) => prev + 1);
              if (!window.location.pathname.startsWith("/messages")) {
                toast.info(`${data.reactorUsername || "Quelqu'un"} a réagi à votre message`, {
                  description: `${data.emoji} en message privé`,
                  duration: 5000,
                });
              }
              if (currentToken) {
                persistNotifications([{
                  id: `reaction-pm-${data.messageId}-${data.reactorId}-${Date.now()}`,
                  type: "reaction",
                  title: `${data.reactorUsername || "Quelqu'un"} a réagi à votre message`,
                  body: `${data.emoji} en message privé`,
                  date: new Date().toISOString(),
                  senderId: data.reactorId,
                }], currentToken);
              }
