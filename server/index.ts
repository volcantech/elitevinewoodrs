...
import { initReviewsTable, getVehicleReviews, createVehicleReview, getReviewsSummary } from "./routes/reviews";
...
export function createServer() {
  ...
  app.get("/api/vehicles/max-pages", (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.getCategoryMaxPages(req, res));
  });
  app.get("/api/vehicles/:id", getVehicleById);
  app.get("/api/reviews/summary", getReviewsSummary);
  app.get("/api/vehicles/:id/reviews", getVehicleReviews);
  app.post("/api/vehicles/:id/reviews", publicLimiter, createVehicleReview);
  ...
  // Category management
  app.post("/api/categories", adminAuth, requireVehiclePermission("create"), mutationLimiter, (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.createCategory(req, res));
  });
  app.put("/api/categories/:id", adminAuth, requireVehiclePermission("update"), mutationLimiter, (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.updateCategory(req, res));
  });
  app.delete("/api/categories/:id", adminAuth, requireVehiclePermission("delete"), mutationLimiter, (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.deleteCategory(req, res));
  });

  // Particularity management
  const requirePartPermission = (permission: keyof UserPermissions["particularities"]) => (req: any, res: any, next: any) => {
    if (!req.user?.permissions?.particularities?.[permission]) {
      return res.status(403).json({ error: "🔒 Permission refusée" });
    }
    next();
  };

  app.get("/api/particularities", adminAuth, requirePartPermission("view"), (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.getParticularities(req, res));
  });
  app.post("/api/particularities", adminAuth, requirePartPermission("create"), mutationLimiter, (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.createParticularity(req, res));
  });
  app.put("/api/particularities/:id", adminAuth, requirePartPermission("update"), mutationLimiter, (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.updateParticularity(req, res));
  });
  app.delete("/api/particularities/:id", adminAuth, requirePartPermission("delete"), mutationLimiter, (req: any, res: any) => {
    import("./routes/vehicles").then(m => m.deleteParticularity(req, res));
  });
