...
import { initActivityLogsTable, addIpColumnIfMissing, logActivity } from "./services/activityLog";
...
import { publicRegister, publicLogin, publicLogout, publicMe, publicUpdateProfile, publicMyOrders, publicMyReviews, publicCancelOrder, adminGetPublicUsers, adminBanPublicUser, adminDeletePublicUser, adminSetPublicUserAdmin, adminEditPublicUser, optionalPublicAuth, adminGetUserReviewHistory, adminGetUserOrderHistory, publicUploadAvatar, avatarUpload, initAvatarColumn, initReferralColumns, getMyReferralInfo, publicChangePassword, publicAccountSummary } from "./routes/publicAuth";
import { initWebhookSettingsTable, getWebhookSettings, saveWebhookSettings, testWebhook } from "./routes/webhookSettings";
import { initTicketsTables, createTicket, getMyTickets, getMyTicketMessages, postMyTicketMessage, adminGetTickets, adminGetTicketDetails, adminReplyTicket, adminUpdateTicket } from "./routes/tickets";
import { initReviewReportsTable, reportReview, adminGetReviewReports, adminResolveReport } from "./routes/reportedReviews";
import { initAdminNotificationsTable, getAdminNotifications } from "./routes/adminNotifications";
import { initLoginHistoryTable, getLoginHistory, adminGetLoginHistoryForUser } from "./routes/loginHistory";
import { getFavorites, addFavorite, removeFavorite, initFavoritesTable } from "./routes/favorites";
import { getOrderMessagesAdmin, postOrderMessageAdmin, getOrderMessagesPublic, postOrderMessagePublic, initOrderMessagesTable } from "./routes/orderMessages";
import { getNotifications, saveNotificationHistory, getNotificationHistory, clearNotificationHistory, initUserNotificationsTable } from "./routes/notifications";
import { getAdminStats } from "./routes/stats";
import { initLoyaltyTables, getLoyaltyUsers, adjustLoyaltyPoints, redeemLoyaltyDiscount, getPublicLoyalty, getLoyaltyHistory, getOrderLoyaltyAdmin } from "./routes/loyalty";
...
export function createServer() {
  ...
  app.get("/api/orders/:id/messages", adminAuth, requireOrderPermission("view"), getOrderMessagesAdmin);
  app.post("/api/orders/:id/messages", adminAuth, requireOrderPermission("validate"), mutationLimiter, postOrderMessageAdmin);
  ...
  const requireLoyaltyManage = (req: any, res: any, next: any) => {
    ...
    if (!req.user.permissions?.loyalty?.manage) return res.status(403).json({ error: "🔒 Permission refusée - Réservé aux patrons" });
    next();
  };
  app.get("/api/admin/loyalty/users", adminAuth, requireLoyaltyManage, getLoyaltyUsers);
  app.post("/api/admin/loyalty/adjust", adminAuth, requireLoyaltyManage, mutationLimiter, adjustLoyaltyPoints);
  app.post("/api/admin/loyalty/redeem", adminAuth, requireLoyaltyManage, mutationLimiter, redeemLoyaltyDiscount);
  app.get("/api/admin/loyalty/:userId/history", adminAuth, requireLoyaltyManage, getLoyaltyHistory);
  app.get("/api/admin/orders/:orderId/loyalty", adminLimiter, adminAuth, getOrderLoyaltyAdmin);
  app.get("/api/public/loyalty", publicLimiter, optionalPublicAuth, getPublicLoyalty);
  ...
  const requireWebhookManage = (req: any, res: any, next: any) => {
    ...
    if (!p?.webhooks?.manage && !p?.loyalty?.manage) {
      return res.status(403).json({ error: "🔒 Permission refusée — vous n'avez pas accès à la gestion des webhooks" });
    }
    next();
  };
  app.get("/api/admin/settings/webhooks", adminAuth, requireWebhookManage, getWebhookSettings);
  app.post("/api/admin/settings/webhooks", adminAuth, requireWebhookManage, mutationLimiter, saveWebhookSettings);
  app.post("/api/admin/settings/webhooks/test", adminAuth, requireWebhookManage, mutationLimiter, testWebhook);
  ...
  app.post("/api/public/notifications/history", mutationLimiter, saveNotificationHistory);
  app.delete("/api/public/notifications/history", mutationLimiter, clearNotificationHistory);
  app.post("/api/public/favorites/:vehicleId", mutationLimiter, addFavorite);
  app.delete("/api/public/favorites/:vehicleId", mutationLimiter, removeFavorite);
  app.get("/api/public/orders/:id/messages", getOrderMessagesPublic);
  app.post("/api/public/orders/:id/messages", mutationLimiter, postOrderMessagePublic);

  // Login history
  app.get("/api/public/login-history", publicLimiter, getLoginHistory);

  // Password change
  app.post("/api/public/change-password", mutationLimiter, publicChangePassword);
  ...
  app.post("/api/public/tickets/:id/messages", mutationLimiter, postMyTicketMessage);

  // Review reports (public)
  app.post("/api/public/reviews/:id/report", mutationLimiter, optionalPublicAuth, reportReview);

  // Tickets (admin)
  const requireTicketsPermission = (req: any, res: any, next: any) => {
    if (!req.user) return res.status(401).json({ error: "❌ Authentification requise" });
    const p = req.user.permissions;
    const hasAccess = p?.tickets?.manage || p?.tickets?.view || p?.tickets?.reply || p?.tickets?.close || p?.tickets?.assign || p?.moderation?.view;
    if (!hasAccess) {
      return res.status(403).json({ error: "🔒 Permission refusée — accès aux tickets non autorisé" });
    }
    next();
  };
  app.get("/api/admin/tickets", adminAuth, requireTicketsPermission, adminGetTickets);
  app.get("/api/admin/tickets/:id", adminAuth, requireTicketsPermission, adminGetTicketDetails);
  app.post("/api/admin/tickets/:id/reply", adminAuth, requireTicketsPermission, mutationLimiter, adminReplyTicket);
  app.patch("/api/admin/tickets/:id", adminAuth, requireTicketsPermission, mutationLimiter, adminUpdateTicket);
  ...
  const requireReportsPermission = (req: any, res: any, next: any) => {
    ...
    if (!p?.moderation?.view_reports && !p?.moderation?.ban_uniqueids) {
      return res.status(403).json({ error: "🔒 Permission refusée — accès aux signalements non autorisé" });
    }
    next();
  };
  app.get("/api/admin/review-reports", adminAuth, requireReportsPermission, adminGetReviewReports);
  app.post("/api/admin/review-reports/:id/resolve", adminAuth, requireReportsPermission, mutationLimiter, adminResolveReport);
  ...
  app.get("/api/admin/public-users/:id/history/logins", adminAuth, requireModOrUsersPermission, adminGetLoginHistoryForUser);
