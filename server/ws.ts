...
export async function loadLastSeenFromDb() {
  try {
    const rows = await sql`SELECT id, last_seen_at FROM users WHERE last_seen_at IS NOT NULL`;
    for (const row of rows) {
      lastSeenMap.set(Number(row.id), new Date(row.last_seen_at));
    }
    console.log(`✅ Loaded ${rows.length} last_seen entries from DB`);
  } catch (e) {
    console.error("⚠️ Failed to load last_seen from DB:", e);
  }
}

async function persistLastSeen(userId: number, date: Date) {
  try {
    await sql`UPDATE users SET last_seen_at = ${date.toISOString()} WHERE id = ${userId}`;
  } catch (e) {
    console.error(`⚠️ Failed to persist last_seen for user ${userId}:`, e);
  }
}
    ...
    try {
      ...
      ws.on("message", async (rawData) => {
        try {
          ...
          const GROUP_CALL_RELAY = ["group_call_offer","group_call_answer","group_call_ice","group_call_screen_start","group_call_screen_stop","group_call_quality_request"];
