...
// In-memory auth cache — avoids a DB round-trip on every authenticated request
const AUTH_CACHE_TTL = 60_000; // 60 seconds
const authCache = new Map<number, { permissions: UserPermissions; username: string; expiry: number }>();

function getCachedAuth(userId: number) {
  const cached = authCache.get(userId);
  if (!cached || Date.now() > cached.expiry) { authCache.delete(userId); return null; }
  return cached;
}

function setCachedAuth(userId: number, username: string, permissions: UserPermissions) {
  authCache.set(userId, { username, permissions, expiry: Date.now() + AUTH_CACHE_TTL });
}
...
export async function adminAuth(
  ...
  if (adminTokenCookie) {
    try {
      ...
      if (decoded.authenticated && decoded.userId) {
        // Check cache first to avoid a DB round-trip on every request
        const cached = getCachedAuth(decoded.userId);
        if (cached) {
          req.user = { userId: decoded.userId, username: cached.username, permissions: cached.permissions, authenticated: true } as JWTPayload;
          return next();
        }
        ...
        if (result && result.length > 0) {
          ...
          setCachedAuth(result[0].id, result[0].username, permissions);
          req.user = { userId: result[0].id, username: result[0].username, permissions, authenticated: true } as JWTPayload;
          return next();
        }
      }
    } catch {}
  ...
  if (!publicToken) {
    return res
      .status(401)
      .json({ error: "❌ Authentification requise - Merci de vous connecter" });
  }
  ...
  try {
    ...
    // Check cache first
    const cached = decoded.userId ? getCachedAuth(decoded.userId) : null;
    if (cached) {
      req.user = { userId: decoded.userId, username: cached.username, permissions: cached.permissions, authenticated: true } as JWTPayload;
      return next();
    }
    ...
    return res
      .status(403)
      .json({ error: "❌ Session expirée - Merci de vous reconnecter" });
  }
}
...
type GiveawayPermission = keyof UserPermissions["giveaways"];
type BadgePermission = keyof UserPermissions["badges"];
...
export function requirePermission(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!permissions) {
      return res.status(403).json({
        error: "❌ Accès refusé - Aucune permission assignée à votre compte",
      });
    }
    ...
    if (
      !categoryPermissions ||
      !(categoryPermissions as Record<string, boolean>)[permission]
    ) {
...
export function requireGiveawayPermission(permission: GiveawayPermission) {
  return requirePermission("giveaways", permission);
}

export function requireBadgePermission(permission: BadgePermission) {
  return requirePermission("badges", permission);
}
