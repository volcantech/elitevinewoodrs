...
export async function initAuditLogsTable() {
  try {
    ...
    // Error logged Error initializing audit logs table:", error);
  }
}
    ...
    if (req.user && req.method !== "GET") {
      try {
        ...
        if (resourceType && action) {
          ...
          }).catch((err) => // Error logged Erreur audit log:", err));
        }
      } catch (err) {
        // Error logged Error in audit log middleware:", err);
      }
    }
...
async function logAuditEvent(event: any) {
  try {
    ...
    // Error logged Error logging audit event:", error);
  }
}
...
function extractResourceName(body: any, resourceType: string, responseData: any): string | null {
  try {
    ...
    // Error logged Error extracting resource name:", err);
    return null;
  }
}
