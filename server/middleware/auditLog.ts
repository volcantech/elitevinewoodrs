      ...
      try {
        ...
        if (resourceType && action) {
          ...
          logAuditEvent({
            ...
            description,
            changes: req.body,
            ip_address: (() => { const xff = req.headers["x-forwarded-for"]; if (xff) { const first = (Array.isArray(xff) ? xff[0] : xff).split(",")[0].trim(); if (first) return first; } return req.ip || req.socket?.remoteAddress || "Unknown"; })(),
