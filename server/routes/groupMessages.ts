...
export async function sendGroupMessage(req: Request, res: Response) {
  ...
  try {
    ...
    const [senderCheck] = await sql`SELECT COALESCE(is_messages_blocked, FALSE) AS is_messages_blocked, avatar_url FROM users WHERE id = ${me.userId}`;
    if (senderCheck?.is_messages_blocked) return res.status(403).json({ error: "❌ L'envoi de messages a été bloqué sur votre compte." });
    const senderAvatar = senderCheck?.avatar_url || null;
    ...
    res.json({ ...msg, sender_username: me.username, sender_avatar: senderAvatar });
  } catch (error) {
...
export async function sendGroupImageMessage(req: Request, res: Response) {
  ...
  try {
    ...
    const [senderCheck] = await sql`
      SELECT COALESCE(is_messages_blocked, FALSE) AS is_messages_blocked, COALESCE(is_images_blocked, FALSE) AS is_images_blocked, avatar_url
...
export async function addGroupMember(req: Request, res: Response) {
  ...
  try {
    ...
    const memberRows = await sql`SELECT user_id FROM group_members WHERE group_id = ${groupId}`;
    const memberIds = memberRows.map((r: any) => r.user_id);
    broadcastToUsers(memberIds, { type: "group_member_added", groupId, groupName: group.name, user: targetUser, actorUsername: me.username });
...
export async function removeGroupMember(req: Request, res: Response) {
  ...
  try {
    ...
    broadcastToUser(targetUserId, { type: "group_call_force_leave", groupId, reason: "removed" });
...
export async function reactGroupMessage(req: Request, res: Response) {
  ...
  try {
    ...
    const [[msgRow], [groupRow]] = await Promise.all([
      sql`SELECT sender_id FROM group_messages WHERE id = ${messageId} AND group_id = ${groupId}`,
      sql`SELECT name FROM group_conversations WHERE id = ${groupId}`,
    ]);
    ...
    const reactions = await sql`
      SELECT r.emoji, r.user_id, COALESCE(u.username, 'Utilisateur supprimé') as username
      FROM message_reactions r
    ...
    const senderIdNum = msgRow ? Number(msgRow.sender_id) : null;

    if (action === "added" && senderIdNum && senderIdNum !== me.userId) {
      sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${senderIdNum}, 'reaction', ${`${me.username} a réagi à votre message`}, ${`${emoji} dans ${groupName}`})`.catch(() => {});
    }
...
export async function reactPrivateMessage(req: Request, res: Response) {
  ...
  try {
    ...
    const senderIdDM = Number(msg.sender_id);

    if (action === "added" && senderIdDM !== me.userId) {
      sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${senderIdDM}, 'reaction', ${`${me.username} a réagi à votre message`}, ${`${emoji}`})`.catch(() => {});
    }
...
export async function sendGroupAudioMessage(req: Request, res: Response) {
  ...
  try {
    ...
    res.json({ ...msg, sender_username: me.username, sender_avatar: senderAvatar, reactions: [] });
  } catch (error) {
