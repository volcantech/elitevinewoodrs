...
export async function reportReview(req: Request, res: Response) {
  try {
    ...
    insertAdminNotification("report_new", `🚩 Signalement d'avis — ${username}`, `L'avis de "${review.pseudo}" a été signalé${cleanReason ? ` : ${cleanReason.slice(0, 100)}` : ""}`).catch(() => {});
...
export async function adminResolveReport(req: Request, res: Response) {
  try {
    ...
    console.log(`🔔 resolveReport: action=${action}, reporter_user_id=${report.reporter_user_id}, reportId=${reportId}`);
    if (action === "delete_review") {
      if (report.reporter_user_id) {
        ...
        try {
          ...
          console.log(`🔔 broadcastToUser(${report.reporter_user_id}, report accepted)`);
          broadcastToUser(report.reporter_user_id, {
            type: "report",
            title: "✅ Signalement accepté",
            body: "Votre signalement a été examiné : l'avis a été supprimé par l'équipe.",
          });
        } catch {}
      }
      ...
      if (report.reporter_user_id) {
        ...
        try {
          const { broadcastToUser } = await import("../ws");
          console.log(`🔔 broadcastToUser(${report.reporter_user_id}, report rejected)`);
          broadcastToUser(report.reporter_user_id, {
            type: "report",
            title: "❌ Signalement refusé",
            body: "Votre signalement a été examiné : aucune action n'a été entreprise par l'équipe.",
          });
        } catch {}
      }
      await sql`
