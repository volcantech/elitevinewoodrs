...
export async function getDocuments(_req: Request, res: Response) {
  try {
    const rows = await sql`SELECT id, title, is_public, is_template, share_token, created_by, created_at, updated_at FROM admin_documents ORDER BY updated_at DESC`;
    res.json(rows);
  } catch {
...
export async function createDocument(req: Request, res: Response) {
  const { title, is_template } = req.body;
  ...
  try {
    const result = await sql`
      ...
      VALUES (${docTitle}, '', FALSE, ${is_template ? true : false}, ${shareToken}, ${admin.username})
      RETURNING *
    `;
    await logActivity(
...
export async function createFromTemplate(req: Request, res: Response) {
  ...
  try {
    ...
    if (variables && typeof variables === "object") {
      // Step 1: strip all styled variable spans → restore raw {{key|label}} text
      content = content.replace(/<span[^>]*data-var="([^"]*)"[^>]*>[\s\S]*?<\/span>/gi, "$1");
      // Step 2: replace each {{key}} / {{key|label}} with user-supplied value
      for (const [key, value] of Object.entries(variables)) {
        const safeKey = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        content = content.replace(new RegExp(`\\{\\{\\s*${safeKey}\\s*(?:\\|[^}]*)?\\s*\\}\\}`, "gi"), String(value));
      }
    }
...
export async function updateDocument(req: Request, res: Response) {
  const { id } = req.params;
  const { title, content, is_public, is_template } = req.body;
  const admin = adminInfo(req);
  try {
    ...
    const result = await sql`
      UPDATE admin_documents SET title = ${newTitle}, content = ${newContent}, is_public = ${newIsPublic}, is_template = ${newIsTemplate}, updated_at = NOW()
