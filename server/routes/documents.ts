...
export async function getDocuments(_req: Request, res: Response) {
  try {
    const rows = await sql`SELECT id, title, is_public, is_template, is_readonly, share_token, created_by, created_at, updated_at FROM admin_documents ORDER BY updated_at DESC`;
    res.json(rows);
  } catch {
...
export async function updateDocument(req: Request, res: Response) {
  const { id } = req.params;
  const { title, content, is_public, is_template, is_readonly } = req.body;
  const admin = adminInfo(req);
  try {
    ...
    const result = await sql`
      UPDATE admin_documents SET title = ${newTitle}, content = ${newContent}, is_public = ${newIsPublic}, is_template = ${newIsTemplate}, is_readonly = ${newIsReadonly}, updated_at = NOW()
