...
export async function createFromTemplate(req: Request, res: Response) {
  const { id } = req.params;
  const { title, variables, is_public, folder } = req.body as { title?: string; variables?: Record<string, string>; is_public?: boolean; folder?: string };
  const admin = adminInfo(req);
  const shareToken = generateShareToken();
  try {
    ...
    const docIsPublic = is_public === true;
    const docFolder = folder?.trim() || null;
    const result = await sql`
      INSERT INTO admin_documents (title, content, is_public, is_template, folder, share_token, created_by)
      VALUES (${docTitle}, ${content}, ${docIsPublic}, FALSE, ${docFolder}, ${shareToken}, ${admin.username})
      RETURNING *
    `;
...
export async function updateDocument(req: Request, res: Response) {
  ...
  const isContentOnly = content !== undefined
    && title === undefined && is_public === undefined
    && is_template === undefined && is_readonly === undefined && folder === undefined;

  try {
    // Fast path: content-only save — single UPDATE, no SELECT needed
    if (isContentOnly) {
      const result = await sql`
        UPDATE admin_documents
        SET content = ${content}, updated_at = NOW()
        WHERE id = ${id}
        RETURNING id, title, is_public, is_template, is_readonly, folder, share_token, updated_at
      `;
      if (!result.length) return res.status(404).json({ error: "Document introuvable" });
      if (result[0].is_public && result[0].share_token) {
        broadcastSse(result[0].share_token, "doc_update", {
          content,
    ...
    // Metadata save: need current values for fallbacks and change tracking
    const current = await sql`
      SELECT id, title, is_public, is_template, is_readonly, folder, share_token
      FROM admin_documents WHERE id = ${id}
    `;
    ...
    const result = await sql`
      UPDATE admin_documents SET
        title = ${newTitle},
        content = CASE WHEN ${content !== undefined} THEN ${content ?? null}::text ELSE content END,
    ...
    if (Object.keys(changes).length > 0) {
      logActivity(admin.id, admin.username, "Modification", "Document", newTitle,
        `${admin.username} a modifié le document "${newTitle}"`, changes, null, admin.ip
      ).catch(() => {});
    }

    const token = result[0]?.share_token || current[0].share_token;
    if (newIsPublic && token) {
      broadcastSse(token, "doc_update", {
    ...
    res.json({ ...result[0], ...(content !== undefined ? { content } : {}) });
  } catch {
    res.status(500).json({ error: "Erreur serveur" });
  }
}
