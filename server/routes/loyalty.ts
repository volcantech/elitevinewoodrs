...
export async function awardLoyaltyPoints(userId: number, orderId: number, totalPrice: number) {
  try {
    ...
    const points = Math.floor(totalPrice / 350000) * 5;
...
export async function adjustLoyaltyPoints(req: Request, res: Response) {
  try {
    ...
    const [user] = await sql`SELECT id, username, loyalty_points FROM users WHERE id = ${userId}`;
    ...
    try {
      ...
      const notifTitle = points > 0 ? "🎖️ Points de fidélité ajoutés" : "🎖️ Points de fidélité retirés";
      const notifBody = points > 0
        ? `+${points} point${points > 1 ? "s" : ""} de fidélité ont été ajoutés à votre compte. Nouveau solde : ${newPoints} pts.`
        : `${points} point${Math.abs(points) > 1 ? "s" : ""} de fidélité ont été retirés de votre compte. Nouveau solde : ${newPoints} pts.`;
      broadcastToUser(userId, { type: "notification", title: notifTitle, body: notifBody });
      await sql`
        INSERT INTO user_notifications (public_user_id, type, title, body, order_id)
        VALUES (${userId}, 'loyalty', ${notifTitle}, ${notifBody}, NULL)
      `;
    } catch {}
