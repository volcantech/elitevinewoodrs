    ...
    await fetch(ticketWebhookUrl, {
      ...
      body: JSON.stringify({
        embeds: [{
          ...
          footer: { text: embed.footer ?? "Elite Vinewood Auto — Support" },
        }],
      }),
    });
  } catch {}
...
export async function createTicket(req: Request, res: Response) {
  try {
    ...
    const { ticketWebhookUrl, discordWebhookUrl } = await getWebhookUrls();
    const typeLabel = cleanType === "delivery_issue" ? "Problème de livraison" : cleanType === "claim" ? "Réclamation" : "Autre";

    if (ticketWebhookUrl) {
      await sendTicketWebhook(ticketWebhookUrl, {
        title: "🎫 Nouveau Ticket Support",
        description: `**${payload.username}** a ouvert un nouveau ticket.`,
        fields: [
          { name: "📋 Sujet", value: cleanSubject, inline: false },
          { name: "🏷️ Type", value: typeLabel, inline: true },
          { name: "🎫 Ticket", value: `#${ticket.id}`, inline: true },
          { name: "💬 Message initial", value: cleanMessage.slice(0, 500) + (cleanMessage.length > 500 ? "…" : ""), inline: false },
        ],
        color: 3447003,
      });
    }
    ...
    insertAdminNotification("ticket_new", `🎫 Nouveau ticket — ${payload.username}`, `Sujet : ${cleanSubject} (${typeLabel})`).catch(() => {});
...
export async function postMyTicketMessage(req: Request, res: Response) {
  try {
    ...
    const [msg] = await sql`
      INSERT INTO ticket_messages (ticket_id, sender_type, sender_id, sender_username, message)
      VALUES (${ticketId}, 'client', ${payload.userId}, ${payload.username}, ${cleanMsg})
      RETURNING *
    `;
    await sql`UPDATE tickets SET updated_at = CURRENT_TIMESTAMP WHERE id = ${ticketId}`;

    insertAdminNotification("ticket_reply", `💬 Réponse client — ${payload.username}`, `"${ticket.subject}" : "${cleanMsg.slice(0, 100)}${cleanMsg.length > 100 ? "…" : ""}"`).catch(() => {});

    const { ticketWebhookUrl } = await getWebhookUrls();
    if (ticketWebhookUrl) {
      await sendTicketWebhook(ticketWebhookUrl, {
        title: `💬 Réponse Client — Ticket #${ticketId}`,
        description: `**${payload.username}** a répondu à son ticket.`,
        fields: [
          { name: "📋 Sujet", value: ticket.subject, inline: false },
          { name: "💬 Message", value: cleanMsg.slice(0, 1024) + (cleanMsg.length > 1024 ? "…" : ""), inline: false },
        ],
        color: 16766976,
      });
    }
...
export async function adminReplyTicket(req: Request, res: Response) {
  try {
    ...
    const [msg] = await sql`
      INSERT INTO ticket_messages (ticket_id, sender_type, sender_id, sender_username, message)
      VALUES (${ticketId}, 'admin', ${adminId}, ${adminUsername}, ${cleanMsg})
      RETURNING *
    `;
    await sql`UPDATE tickets SET updated_at = CURRENT_TIMESTAMP WHERE id = ${ticketId}`;
    ...
    await logActivity(
      ...
      { "Ticket": ticketId, "Message": cleanMsg.slice(0, 100) },
      null,
      (req as any).ip ?? null
    );
    ...
    if (ticketWebhookUrl) {
      await sendTicketWebhook(ticketWebhookUrl, {
        title: `💬 Réponse Admin — Ticket #${ticketId}`,
        description: `**${adminUsername}** a répondu au ticket de **${ticket.username}**.`,
        fields: [
          { name: "📋 Sujet", value: ticket.subject, inline: false },
          { name: "💬 Réponse", value: cleanMsg.slice(0, 1024) + (cleanMsg.length > 1024 ? "…" : ""), inline: false },
          { name: "👤 Staff", value: adminUsername, inline: true },
        ],
        color: 5763719,
      });
    }
    ...
    if (ticket.user_id) {
      try {
        ...
        broadcastToUser(ticket.user_id, {
          type: "ticket_reply",
          ticketId,
        ...
        if (!isUserOnline(ticket.user_id)) {
          await sql`
            INSERT INTO user_notifications (public_user_id, type, title, body)
            VALUES (${ticket.user_id}, 'ticket', ${'💬 Réponse à votre ticket'}, ${`Ticket "${ticket.subject}" : "${cleanMsg.slice(0, 100)}${cleanMsg.length > 100 ? '…' : ''}"`})
          `.catch(() => {});
        }
      } catch {}
    }
...
export async function adminUpdateTicket(req: Request, res: Response) {
  try {
    ...
    if (status === "closed" || status === "open" || status === "in_progress") {
      const { ticketWebhookUrl, discordWebhookUrl } = await getWebhookUrls();

      const statusLabels: Record<string, { label: string; emoji: string; color: number }> = {
        closed:      { label: "Fermé",       emoji: "🔒", color: 15158332 },
        open:        { label: "Réouvert",    emoji: "🔓", color: 3447003  },
        in_progress: { label: "En cours",    emoji: "⏳", color: 16766976 },
      };
      const statusInfo = statusLabels[status] ?? { label: status, emoji: "📋", color: 9807270 };

      if (ticketWebhookUrl) {
        await sendTicketWebhook(ticketWebhookUrl, {
          title: `${statusInfo.emoji} Ticket #${ticketId} — ${statusInfo.label}`,
          description: `Le ticket de **${ticket.username}** est passé au statut **${statusInfo.label}** par **${adminUsername}**.`,
          fields: [
            { name: "📋 Sujet", value: ticket.subject, inline: false },
            { name: "👤 Staff", value: adminUsername, inline: true },
            { name: "📊 Nouveau statut", value: statusInfo.label, inline: true },
          ],
      ...
      if (ticket.user_id) {
        ...
        const notifTitle = isClosed ? "🔒 Ticket fermé" : isReopened ? "🔓 Ticket réouvert" : "⏳ Ticket en cours";
        const notifBody = isClosed
          ? `Votre ticket "${ticket.subject}" a été fermé par l'équipe.`
          : isReopened
          ? `Votre ticket "${ticket.subject}" a été réouvert par l'équipe.`
          : `Votre ticket "${ticket.subject}" est maintenant pris en charge.`;
        const notifType = isClosed ? "ticket_closed" : "ticket";
        try {
          await sql`
            INSERT INTO user_notifications (public_user_id, type, title, body, order_id)
            VALUES (${ticket.user_id}, ${notifType}, ${notifTitle}, ${notifBody}, NULL)
          `;
        } catch {}
        try {
