...
export async function postMyTicketMessage(req: Request, res: Response) {
  try {
    ...
    insertAdminNotification("ticket_reply", `💬 Réponse client — ${payload.username}`, `"${ticket.subject}" : "${message.trim().slice(0, 100)}${message.trim().length > 100 ? "…" : ""}"`).catch(() => {});
      ...
      try {
        ...
        if (!isUserOnline(ticket.user_id)) {
          await sql`
            ...
            VALUES (${ticket.user_id}, 'ticket', ${'💬 Réponse à votre ticket'}, ${`Ticket "${ticket.subject}" : "${message.trim().slice(0, 100)}${message.trim().length > 100 ? '…' : ''}"`})
          `.catch(() => {});
        }
      } catch {}
    }
  ...
  try {
    ...
    if (status === "closed" || status === "open") {
      if (ticket.user_id) {
        ...
        const notifTitle = isClosed ? "🔒 Ticket fermé" : "🔓 Ticket réouvert";
