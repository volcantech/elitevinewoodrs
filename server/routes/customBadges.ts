...
export const TRIGGER_LABELS: Record<TriggerType, string> = {
  ...
  loyalty_points_used: "Points de fidélité utilisés",
  friends_count: "Nombre d'amis",
  messages_sent_all: "Messages envoyés (DM + groupe)",
  messages_received: "Messages reçus (DM + groupe)",
  reactions_sent: "Réactions envoyées (messages + avis)",
  reactions_received: "Réactions reçues (messages + avis)",
  calls_participated: "Appels participés",
  images_sent: "Images envoyées (DM + groupe)",
  images_received: "Images reçues (DM + groupe)",
  ...
  profiles_visited: "Profils visités par l'utilisateur",
  profile_views_received: "Visites reçues sur son profil",
...
export async function getExtendedStats(userId: number): Promise<ExtendedStats> {
  const [
    ...
    favsRow,
    friendsReqRow,
    // New messaging triggers
    msgSentGroupRow,
    msgReceivedDMRow,
    msgReceivedGroupRow,
    reactionsSentRow,
    reactionsReceivedPMRow,
    reactionsReceivedGroupRow,
    callsRow,
    imgSentDMRow,
    imgSentGroupRow,
    imgReceivedDMRow,
    ...
    sql`SELECT COUNT(*) as count FROM review_likes WHERE visitor_id = ${userId}::varchar`.then((r) => r[0]).catch(() => ({ count: "0" })),
    ...
    sql`SELECT COUNT(*) as count FROM private_messages WHERE sender_id = ${userId} AND COALESCE(message_type,'text') NOT IN ('system','call','missed_call','deleted')`.then((r) => r[0]),
    ...
    // group messages sent
    sql`SELECT COUNT(*) as count FROM group_messages WHERE sender_id = ${userId} AND COALESCE(message_type,'text') NOT IN ('system','system_alert','deleted')`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // DM messages received
    sql`SELECT COUNT(*) as count FROM private_messages WHERE receiver_id = ${userId} AND COALESCE(message_type,'text') NOT IN ('system','call','missed_call','deleted')`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // group messages received (in groups the user belongs to, from others)
    sql`SELECT COUNT(*) as count FROM group_messages gm JOIN group_members gmem ON gmem.group_id = gm.group_id AND gmem.user_id = ${userId} WHERE gm.sender_id != ${userId} AND COALESCE(gm.message_type,'text') NOT IN ('system','system_alert','deleted')`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // reactions sent by user
    sql`SELECT COUNT(*) as count FROM message_reactions WHERE user_id = ${userId}`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // reactions received on user's PM
    sql`SELECT COUNT(*) as count FROM message_reactions mr JOIN private_messages pm ON mr.message_id = pm.id WHERE mr.message_table = 'private_messages' AND pm.sender_id = ${userId}`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // reactions received on user's group messages
    sql`SELECT COUNT(*) as count FROM message_reactions mr JOIN group_messages gm ON mr.message_id = gm.id WHERE mr.message_table = 'group_messages' AND gm.sender_id = ${userId}`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // calls participated
    sql`SELECT COUNT(*) as count FROM private_messages WHERE message_type = 'call' AND (sender_id = ${userId} OR receiver_id = ${userId})`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // images sent DM
    sql`SELECT COUNT(*) as count FROM private_messages WHERE sender_id = ${userId} AND message_type = 'image'`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // images sent group
    sql`SELECT COUNT(*) as count FROM group_messages WHERE sender_id = ${userId} AND message_type = 'image'`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // images received DM
    sql`SELECT COUNT(*) as count FROM private_messages WHERE receiver_id = ${userId} AND message_type = 'image'`.then((r) => r[0]).catch(() => ({ count: "0" })),
    // images received group
    sql`SELECT COUNT(*) as count FROM group_messages gm JOIN group_members gmem ON gmem.group_id = gm.group_id AND gmem.user_id = ${userId} WHERE gm.sender_id != ${userId} AND gm.message_type = 'image'`.then((r) => r[0]).catch(() => ({ count: "0" })),
  ]);

  const [loyaltyRow, loyaltyUsedRow, profileVisitsRow] = await Promise.all([
    sql`SELECT COALESCE(loyalty_points, 0) as points FROM users WHERE id = ${userId}`.then((r) => r[0]).catch(() => null),
    sql`SELECT COALESCE(SUM(ABS(points)), 0) as total FROM loyalty_transactions WHERE user_id = ${userId} AND type = 'redeemed'`.then((r) => r[0]).catch(() => null),
    sql`SELECT COALESCE(profiles_visited, 0) as profiles_visited, COALESCE(profile_views, 0) as profile_views FROM users WHERE id = ${userId}`.then((r) => r[0]).catch(() => null),
  ]);
  ...
  const msgSentDM = parseInt(messagesRow?.count || "0");
  const msgSentGroup = parseInt(msgSentGroupRow?.count || "0");
  const msgReceivedDM = parseInt(msgReceivedDMRow?.count || "0");
  const msgReceivedGroup = parseInt(msgReceivedGroupRow?.count || "0");
  ...
  return {
    ...
    loyalty_points_used: parseInt(loyaltyUsedRow?.total || "0"),
    friends_count: parseInt(friendsRow?.count || "0"),
    messages_sent_all: msgSentDM + msgSentGroup,
    messages_received: msgReceivedDM + msgReceivedGroup,
    reactions_sent: parseInt(reactionsSentRow?.count || "0") + parseInt(likesSentRow?.count || "0"),
    reactions_received: parseInt(reactionsReceivedPMRow?.count || "0") + parseInt(reactionsReceivedGroupRow?.count || "0") + parseInt(likesReceivedRow?.count || "0"),
    calls_participated: parseInt(callsRow?.count || "0"),
    images_sent: parseInt(imgSentDMRow?.count || "0") + parseInt(imgSentGroupRow?.count || "0"),
    images_received: parseInt(imgReceivedDMRow?.count || "0") + parseInt(imgReceivedGroupRow?.count || "0"),
    ...
    profiles_visited: parseInt(profileVisitsRow?.profiles_visited || "0"),
    profile_views_received: parseInt(profileVisitsRow?.profile_views || "0"),
  };
}
