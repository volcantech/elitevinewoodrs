...
export const TRIGGER_LABELS: Record<TriggerType, string> = {
  ...
  reactions_sent: "Réactions envoyées sur des messages",
  reactions_received: "Réactions reçues sur ses messages",
...
export async function getExtendedStats(userId: number): Promise<ExtendedStats> {
  ...
  const [loyaltyRow, loyaltyUsedRow] = await Promise.all([
  ...
  return {
    ...
    reactions_sent: parseInt(reactionsSentRow?.count || "0"),
    reactions_received: parseInt(reactionsReceivedPMRow?.count || "0") + parseInt(reactionsReceivedGroupRow?.count || "0"),
