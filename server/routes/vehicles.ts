  ...
  try {
    await sql`
      CREATE TABLE IF NOT EXISTS vehicles (
        ...
        trunk_weight INTEGER DEFAULT 0,
        image_url TEXT,
        seats INTEGER DEFAULT 4,
        particularity VARCHAR(100) DEFAULT 'Aucune',
    ...
    const allCategories = [...new Set([...(existingVehicles || []).map((v: any) => v.category), ...staticCategories])];
