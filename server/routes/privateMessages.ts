...
import { broadcastToUser, isUserOnline, getLastSeen } from "../ws";
...
export async function initPrivateMessagesTable() {
  try {
    ...
    await sql`ALTER TABLE private_messages ADD COLUMN IF NOT EXISTS caption TEXT`;
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_images_blocked BOOLEAN NOT NULL DEFAULT FALSE`; } catch {}
...
export async function searchPlayersGlobal(req: Request, res: Response) {
  ...
  try {
    ...
    console.error("❌ searchPlayersGlobal:", error);
...
export async function getConversations(req: Request, res: Response) {
  ...
  try {
    const rows = await sql`
      SELECT
        u.id, u.username, u.avatar_url, u.unique_id, u.last_seen_at,
        u.is_calls_blocked,
        sub.last_message, sub.last_message_type, sub.last_at, sub.last_sender_id,
        COALESCE(unread.unread_count, 0) AS unread_count
      FROM (
        SELECT
          CASE WHEN sender_id = ${myId} THEN receiver_id ELSE sender_id END AS partner_id,
          content AS last_message,
          COALESCE(message_type, 'text') AS last_message_type,
    ...
    const enriched = (rows as any[]).map((r) => ({
      ...r,
      id: Number(r.id),
      isOnline: isUserOnline(Number(r.id)),
      last_seen: r.last_seen_at ? new Date(r.last_seen_at).toISOString() : (getLastSeen(Number(r.id))?.toISOString() || null),
    }));
    res.json(enriched);
  } catch (error) {
...
export async function getMessages(req: Request, res: Response) {
  ...
  try {
    const messages = await sql`
      SELECT id, sender_id, receiver_id, content, caption, is_read, created_at, COALESCE(message_type, 'text') AS message_type
      FROM private_messages
    ...
    if (msgIds.length > 0) {
      const reactions = await sql`
        ...
        WHERE r.message_id = ANY(${msgIds}) AND r.message_table = 'private_messages'
      `;
    ...
    const messagesWithReactions = (messages as any[]).map((m: any) => ({ ...m, reactions: reactionsMap[Number(m.id)] || [] }));

    res.json({ partner, messages: messagesWithReactions });
  } catch (error) {
...
export async function sendMessage(req: Request, res: Response) {
  ...
  const [senderCheck] = await sql`SELECT COALESCE(is_messages_blocked, FALSE) AS is_messages_blocked, avatar_url FROM users WHERE id = ${myId}`;
  const myAvatar = senderCheck?.avatar_url || null;
  ...
  try {
    ...
    if (!isUserOnline(partnerId)) {
      await sql`
        ...
        VALUES (${partnerId}, 'private_message', ${`💬 ${myUsername}`}, ${(content.trim()).slice(0, 120)}, ${myId}, NOW())
      `.catch(() => {});
    }
...
export async function sendCallLog(req: Request, res: Response) {
  ...
  } else if (messageType === "call_banned") {
    content = `Appel refusé — ${partner.username} est banni des appels vocaux — ${dateStr} à ${timeStr}`;
  } else {
    return res.status(400).json({ error: "Type invalide" });
  }

  try {
    const dbMessageType = messageType === "call_banned" ? "missed_call" : messageType;
    const [msg] = await sql`
      INSERT INTO private_messages (sender_id, receiver_id, content, message_type)
      VALUES (${me.userId}, ${pid}, ${content}, ${dbMessageType})
      RETURNING *
    `;
...
export async function sendImageMessage(req: Request, res: Response) {
  ...
  try {
    ...
    if (!ALLOWED_IMAGE_MIMES.includes(file.mimetype)) {
      return res.status(400).json({ error: "Type de fichier non autorisé (JPEG, PNG, GIF, WebP uniquement)" });
    }
    ...
    const [senderCheck] = await sql`
      SELECT
        COALESCE(is_messages_blocked, FALSE) AS is_messages_blocked,
        COALESCE(is_images_blocked, FALSE) AS is_images_blocked,
        avatar_url
      FROM users WHERE id = ${myId}
    `;
    const myAvatar = senderCheck?.avatar_url || null;
    if (senderCheck?.is_messages_blocked) {
      return res.status(403).json({ error: "❌ L'envoi de messages privés a été bloqué sur votre compte." });
    }
    if (senderCheck?.is_images_blocked) {
      return res.status(403).json({ error: "❌ L'envoi d'images a été désactivé sur votre compte. Contactez le support." });
    }

    const [userBlock] = await sql`
      SELECT id FROM user_blocks WHERE blocker_id = ${partnerId} AND blocked_id = ${myId}
    `;
    if (userBlock) return res.status(403).json({ error: "Vous ne pouvez pas envoyer de message à cet utilisateur." });
    ...
    if (partnerPrefs?.messages_from_friends_only) {
      const [friendship] = await sql`
        SELECT id FROM friendships WHERE status = 'accepted'
          AND ((requester_id = ${myId} AND addressee_id = ${partnerId}) OR (requester_id = ${partnerId} AND addressee_id = ${myId}))
      `;
      if (!friendship) return res.status(403).json({ error: "Ce joueur n'accepte les messages que de ses amis." });
    }
    ...
    const [msg] = await sql`
      INSERT INTO private_messages (sender_id, receiver_id, content, message_type, caption)
      VALUES (${myId}, ${partnerId}, ${imageUrl}, 'image', ${caption})
      RETURNING *
    `;
    ...
    if (!isUserOnline(partnerId)) {
      await sql`
        ...
        VALUES (${partnerId}, 'private_message', ${`📷 ${myUsername}`}, ${'vous a envoyé une image.'}, ${myId})
      `.catch(() => {});
    }
...
export async function searchPrivateMessages(req: Request, res: Response) {
  const me = getPublicUser(req);
  if (!me) return res.status(401).json({ error: "Non authentifié" });
  const myId = me.userId;
  const partnerId = parseInt((req.query.partnerId as string) || "0", 10);
  const q = ((req.query.q as string) || "").trim();

  if (isNaN(partnerId) || partnerId <= 0) return res.status(400).json({ error: "partnerId requis" });
  ...
  try {
    ...
    const results = await sql`
      ...
      FROM private_messages
      WHERE ((sender_id = ${myId} AND receiver_id = ${partnerId}) OR (sender_id = ${partnerId} AND receiver_id = ${myId}))
        AND message_type NOT IN ('deleted', 'call', 'missed_call', 'audio', 'image')
    ...
    console.error("❌ searchPrivateMessages:", error);
    res.status(500).json({ error: "Erreur serveur" });
  }
}

export async function sendPrivateAudioMessage(req: Request, res: Response) {
  const me = getPublicUser(req);
  if (!me) return res.status(401).json({ error: "Non authentifié" });
  const myId = me.userId;
  const myUsername = me.username;
  ...
  if (isNaN(partnerId) || partnerId === myId) return res.status(400).json({ error: "Destinataire invalide" });
  ...
  try {
    ...
    const [senderCheck] = await sql`
      SELECT COALESCE(is_messages_blocked, FALSE) AS is_messages_blocked, avatar_url FROM users WHERE id = ${myId}
    `;
    const myAvatar = senderCheck?.avatar_url || null;
    if (senderCheck?.is_messages_blocked) return res.status(403).json({ error: "❌ L'envoi de messages privés a été bloqué sur votre compte." });
    ...
    const [msg] = await sql`
      INSERT INTO private_messages (sender_id, receiver_id, content, message_type)
      VALUES (${myId}, ${partnerId}, ${audioData}, 'audio')
      RETURNING *
    `;
    ...
    if (!isUserOnline(partnerId)) {
      await sql`
        INSERT INTO user_notifications (public_user_id, type, title, body, sender_id, created_at)
        VALUES (${partnerId}, 'private_message', ${`🎤 ${myUsername}`}, ${"vous a envoyé un message vocal"}, ${myId}, NOW())
      `.catch(() => {});
    }

    res.json({ ...msg, reactions: [] });
  } catch (error) {
    console.error("❌ sendPrivateAudioMessage:", error);
    res.status(500).json({ error: "Erreur serveur" });
  }
}
