...
export async function getNotificationHistory(req: Request, res: Response) {
  try {
    ...
    const [countRow] = typeFilter && typeFilter.length > 0
      ? await sql`SELECT COUNT(*) as total FROM user_notifications WHERE public_user_id = ${userId} AND type = ANY(${typeFilter})`
      : await sql`SELECT COUNT(*) as total FROM user_notifications WHERE public_user_id = ${userId}`;
    const total = parseInt(countRow.total);

    const rows = typeFilter && typeFilter.length > 0
      ? await sql`
          SELECT id, type, title, body, order_id, sender_id, created_at
          FROM user_notifications
          WHERE public_user_id = ${userId} AND type = ANY(${typeFilter})
