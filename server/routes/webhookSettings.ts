...
// ── Global calls setting ──────────────────────────────────────────────────────

export async function getCallsEnabledSetting(): Promise<boolean> {
  try {
    const rows = await sql`SELECT value FROM settings WHERE key = 'calls_enabled'`;
    if (rows.length === 0) return true;
    return rows[0].value === "true";
  } catch { return true; }
}

export async function getCallsEnabled(req: Request, res: Response) {
  const enabled = await getCallsEnabledSetting();
  res.json({ enabled });
}

export async function setCallsEnabled(req: Request, res: Response) {
  try {
    const { enabled } = req.body;
    if (typeof enabled !== "boolean") return res.status(400).json({ error: "enabled doit être un booléen" });
    await sql`
      INSERT INTO settings (key, value, updated_by, updated_at)
      VALUES ('calls_enabled', ${enabled ? "true" : "false"}, ${(req as any).user?.username ?? "admin"}, CURRENT_TIMESTAMP)
      ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_by = EXCLUDED.updated_by, updated_at = CURRENT_TIMESTAMP
    `;
    res.json({ success: true, enabled });
  } catch (error) {
    console.error("❌ setCallsEnabled:", error);
    res.status(500).json({ error: "Erreur serveur" });
  }
}
