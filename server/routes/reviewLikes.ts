...
function getPublicUser(req: Request): { userId: number; username: string } | null {
  const token = req.headers.authorization?.replace("Bearer ", "") || (req as any).cookies?.public_token || null;
  if (!token) return null;
  try { const d = jwt.verify(token, JWT_SECRET) as any; return { userId: d.userId, username: d.username }; }
  catch { return null; }
}
...
export async function initReviewLikesTable() {
  try {
    ...
    try {
      await sql`ALTER TABLE review_likes ADD COLUMN IF NOT EXISTS reactor_username VARCHAR(100)`;
    } catch {}
...
export async function toggleReviewLike(req: Request, res: Response) {
  try {
    ...
    const visitorId = me ? String(me.userId) : (req.body.visitorId || req.ip || "anonymous");
    const reactionType: ReactionType = VALID_REACTIONS.includes(req.body.reactionType)
      ? req.body.reactionType
      : "like";
    const reactorUsername = (req.body.reactorUsername as string | undefined)?.trim() || null;
    ...
    if (existing.length > 0) {
      ...
      await sql`INSERT INTO review_likes (review_id, visitor_id, reaction_type, reactor_username) VALUES (${reviewId}, ${visitorId}, ${reactionType}, ${reactorUsername})`;
      active = true;
    }
    ...
    if (active) {
      try {
        ...
        if (review?.public_user_id) {
          ...
          const emojiMap: Record<string, string> = { like: "❤️", utile: "👍", drole: "😂" };
          const labelMap: Record<string, string> = { like: "a aimé", utile: "a trouvé utile", drole: "a trouvé drôle" };
          const emoji = emojiMap[reactionType] || "❤️";
          const label = labelMap[reactionType] || "a réagi à";
          const reactor = (req.body.reactorUsername as string | undefined)?.trim() || "Quelqu'un";
          const title = `${reactor} ${label} votre avis ${emoji}`;
          const body = vehicleName
            ? `${reactor} a réagi à votre avis sur ${vehicleName} ${emoji}`
            : `${reactor} a réagi à votre avis ${emoji}`;

          broadcastToUser(authorId, { type: "notification", level: "success", title, body });
          await sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${authorId}, 'reaction', ${title}, ${body})`;
          checkAndAwardBadges(authorId).catch(() => {});
        }
      } catch (e) {
        console.error("⚠️ Reaction notification error:", e);
      }
    }
...
export async function getReviewReactors(req: Request, res: Response) {
  try {
    const reviewId = parseInt(req.params.id, 10);
    if (isNaN(reviewId) || reviewId <= 0) return res.status(400).json({ error: "ID invalide" });

    const rows = await sql`
      SELECT reaction_type, reactor_username
      FROM review_likes
      WHERE review_id = ${reviewId} AND reactor_username IS NOT NULL
      ORDER BY created_at ASC
    `;

    const result: Record<string, string[]> = { like: [], utile: [], drole: [] };
    for (const row of rows) {
      if (result[row.reaction_type] !== undefined && row.reactor_username) {
        result[row.reaction_type].push(row.reactor_username);
      }
    }
    ...
    if (error?.code === "42703") {
      return res.json({ like: [], utile: [], drole: [] });
    }
    console.error("❌ getReviewReactors:", error);
    res.json({ like: [], utile: [], drole: [] });
  }
}
