    ...
    await sql`
      DO $$ BEGIN
        IF NOT EXISTS (
          ...
          WHERE table_name = 'vehicle_particularities' AND constraint_type = 'UNIQUE'
        ) THEN
          ALTER TABLE vehicle_particularities ADD CONSTRAINT vehicle_particularities_name_key UNIQUE (name);
        END IF;
      END $$
    `.catch(() => {});
    const defaults = ["Les plus rapides", "Drift", "Suspension hydraulique", "Karting", "Électrique", "Décapotable"];
    for (const name of defaults) {
      const existing = await sql`SELECT id FROM vehicle_particularities WHERE LOWER(name) = LOWER(${name})`;
      if (existing.length === 0) {
        await sql`INSERT INTO vehicle_particularities (name) VALUES (${name})`.catch(() => {});
      }
    }
