...
function formatPermissionsReadable(permissions: any): string {
  ...
  if (permissions.reviews) {
    ...
    if (permissions.reviews.delete) reviewPerms.push(permissionLabels["delete"]);
    if (permissions.reviews.reassign) reviewPerms.push("Réattribuer");
...
const FULL_PERMISSIONS: UserPermissions = {
  ...
  badges: { view: true, create: true, edit: true, delete: true, assign: true },
  settings: { view: true, toggle_calls: true },
...
export function normalizePermissions(perms: any): UserPermissions {
  return {
    ...
    reviews: {
      ...
      reassign: perms?.reviews?.reassign ?? false,
      update: perms?.reviews?.update ?? false,
    },
    loyalty: {
    ...
    badges: {
      view: perms?.badges?.view ?? false,
      create: perms?.badges?.create ?? false,
      edit: perms?.badges?.edit ?? false,
      delete: perms?.badges?.delete ?? false,
      assign: perms?.badges?.assign ?? false,
    },
    settings: {
      view: perms?.settings?.view ?? false,
      toggle_calls: perms?.settings?.toggle_calls ?? false,
    },
    spreadsheets: {
      view: perms?.spreadsheets?.view ?? false,
      create: perms?.spreadsheets?.create ?? false,
      edit: perms?.spreadsheets?.edit ?? false,
      delete: perms?.spreadsheets?.delete ?? false,
    },
    documents: {
      view: perms?.documents?.view ?? false,
      create: perms?.documents?.create ?? false,
      edit: perms?.documents?.edit ?? false,
      delete: perms?.documents?.delete ?? false,
    },
  };
}
...
export async function initUsersTable() {
  try {
    ...
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_messages_blocked BOOLEAN NOT NULL DEFAULT FALSE`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS bio VARCHAR(300)`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS banner_color VARCHAR(30) DEFAULT 'amber'`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS banner_url VARCHAR(500)`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS allow_friend_requests BOOLEAN NOT NULL DEFAULT TRUE`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS messages_from_friends_only BOOLEAN NOT NULL DEFAULT FALSE`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS show_badges BOOLEAN NOT NULL DEFAULT TRUE`;
    } catch {}
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS show_rp_info BOOLEAN NOT NULL DEFAULT FALSE`;
    } catch {}
    try {
    ...
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_calls_blocked BOOLEAN NOT NULL DEFAULT FALSE`;
    } catch {}
