...
function formatPermissionsReadable(permissions: any): string {
  ...
  if (permissions.moderation) {
    ...
    if (permissions.moderation.ban_players) modPerms.push(permissionLabels["ban_players"]);
...
const DEFAULT_PERMISSIONS: UserPermissions = {
  vehicles: { view: true, create: false, update: false, delete: false, toggle_categories: false },
...
const FULL_PERMISSIONS: UserPermissions = {
  vehicles: { view: true, create: true, update: true, delete: true, toggle_categories: true },
  orders: { view: true, validate: true, cancel: true, delete: true },
  users: { view: true, update: true, manage_admin: true },
  moderation: { view: true, ban_uniqueids: true, view_logs: true, ban_players: true },
...
export async function initUsersTable() {
  try {
    ...
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS password_hash VARCHAR(255)`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS unique_id VARCHAR(7)`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_admin BOOLEAN NOT NULL DEFAULT FALSE`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_banned BOOLEAN NOT NULL DEFAULT FALSE`; } catch {}
    try { await sql`ALTER TABLE users DROP COLUMN IF EXISTS access_key`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_orders_blocked BOOLEAN NOT NULL DEFAULT FALSE`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_reviews_blocked BOOLEAN NOT NULL DEFAULT FALSE`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS ban_reason VARCHAR(500)`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS rp_phone VARCHAR(20)`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS rp_firstname VARCHAR(64)`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS rp_lastname VARCHAR(64)`; } catch {}
    try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS last_notif_polled_at TIMESTAMP`; } catch {}
    ...
    if (existingAdmins.length === 0) {
      ...
      `;
      console.log("Compte admin par défaut 'AK' créé avec toutes les permissions");
    }
...
export async function createUser(req: Request, res: Response) {
  ...
  try {
    ...
    res.status(500).json({ error: "❌ Impossible de créer l'utilisateur. Vérifiez que le pseudonyme n'existe pas déjà" });
  }
}
...
export async function updateUser(req: Request, res: Response) {
  ...
  try {
    ...
    const hasChanges = username !== undefined || unique_id !== undefined || permissions !== undefined;
...
export async function deleteUser(req: Request, res: Response) {
  ...
  try {
    const usersCount = await sql`SELECT COUNT(*) as count FROM users WHERE is_admin = TRUE`;
