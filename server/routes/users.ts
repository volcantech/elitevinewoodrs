...
function formatPermissionsReadable(permissions: any): string {
  ...
  if (permissions.categories) {
    const catPerms = Object.entries(permissions.categories)
      .filter(([_, v]: any) => v)
      .map(([k]) => permissionLabels[k] || k);
    if (catPerms.length > 0) {
      lines.push(`📂 Catégories: ${catPerms.join(", ")}`);
    }
  }

  if (permissions.particularities) {
    const partPerms = Object.entries(permissions.particularities)
      .filter(([_, v]: any) => v)
      .map(([k]) => permissionLabels[k] || k);
    if (partPerms.length > 0) {
      lines.push(`✨ Particularités: ${partPerms.join(", ")}`);
    }
  }
...
const FULL_PERMISSIONS: UserPermissions = {
  ...
  particularities: { view: true, create: true, update: true, delete: true },
};
...
export function normalizePermissions(perms: any): UserPermissions {
  return {
    ...
    categories: {
      view: perms?.categories?.view ?? false,
      create: perms?.categories?.create ?? false,
      update: perms?.categories?.update ?? false,
      delete: perms?.categories?.delete ?? false,
    },
    particularities: {
      view: perms?.particularities?.view ?? false,
      create: perms?.particularities?.create ?? false,
      update: perms?.particularities?.update ?? false,
