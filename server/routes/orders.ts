...
import { awardLoyaltyPoints, LOYALTY_TIERS, refundLoyaltyOnCancel, reclaimLoyaltyOnRestore } from "./loyalty";
...
export async function initOrdersTables() {
  try {
    ...
    try {
      await sql`ALTER TABLE orders ADD COLUMN public_user_id INTEGER`;
    } catch {
    }

    try {
      await sql`ALTER TABLE orders ADD COLUMN discord_username VARCHAR(255)`;
    } catch {
    }
...
export async function createOrder(req: Request, res: Response) {
  try {
    const { firstName, lastName, phone, items, totalPrice, uniqueId, loyaltyPointsToUse } = req.body;
    ...
    if (publicUserId) {
      const userCheck = await sql`SELECT is_orders_blocked, loyalty_points FROM users WHERE id = ${publicUserId} LIMIT 1`;
      if (userCheck.length > 0 && userCheck[0].is_orders_blocked) {
        return res.status(403).json({ error: "❌ Votre accès aux commandes a été bloqué. Veuillez contacter le support." });
      }
    }
    ...
    if (loyaltyPointsToUse && publicUserId) {
      const validTier = LOYALTY_TIERS.find((t) => t.points === loyaltyPointsToUse);
      if (!validTier) {
        return res.status(400).json({ error: "Palier de fidélité invalide" });
      }
      const [userPts] = await sql`SELECT loyalty_points FROM users WHERE id = ${publicUserId}`;
      if (!userPts || (userPts.loyalty_points || 0) < loyaltyPointsToUse) {
        return res.status(400).json({ error: "Points de fidélité insuffisants" });
      }
      loyaltyDiscountPct = validTier.discount;
      loyaltyDiscountAmount = Math.round(totalPrice * loyaltyDiscountPct / 100);
      finalTotal = totalPrice - loyaltyDiscountAmount;
      validatedLoyaltyPoints = loyaltyPointsToUse;
    }

    const [order] = await sql`
      INSERT INTO orders (unique_id, first_name, last_name, phone, total_price, status, client_ip, public_user_id)
      VALUES (${uniqueId.trim()}, ${firstName}, ${lastName}, ${cleanPhone}, ${finalTotal}, 'pending', ${clientIp}, ${publicUserId})
      RETURNING *
    `;
    ...
    if (validatedLoyaltyPoints > 0 && publicUserId) {
      await sql`UPDATE users SET loyalty_points = loyalty_points - ${validatedLoyaltyPoints} WHERE id = ${publicUserId}`;
      await sql`
        INSERT INTO loyalty_transactions (user_id, order_id, type, points, description)
        VALUES (${publicUserId}, ${order.id}, 'redeemed', ${-validatedLoyaltyPoints},
          ${'Remise ' + loyaltyDiscountPct + '% — commande #' + order.id + ' (-' + loyaltyDiscountAmount + '$)'})
      `;
    }

    const publicUsername = (req as any).publicUser?.username ?? null;
    await logActivity(
      publicUserId, publicUsername,
      "Nouvelle commande",
      "Commande joueur",
      `${firstName} ${lastName}`,
      `Commande #${order.id} passée par ${firstName} ${lastName} (ID: ${uniqueId.trim()})${publicUsername ? ` — compte: ${publicUsername}` : ""}`,
      {
        "N° commande": { old: "N/A", new: order.id },
        "ID Unique": { old: "N/A", new: uniqueId.trim() },
        "Prénom": { old: "N/A", new: firstName },
        "Nom": { old: "N/A", new: lastName },
        "Prix total": { old: "N/A", new: `${totalPrice}$` },
        "Compte lié": { old: "N/A", new: publicUsername || "—" },
      },
      null,
      clientIp ?? null
    );
    ...
    if (webhookUrl) {
      try {
        ...
        if (publicUserId) {
          const [lu] = await sql`SELECT loyalty_points, COALESCE(total_spent, 0) AS total_spent FROM users WHERE id = ${publicUserId} LIMIT 1`;
          if (lu) loyaltyInfo = { points: lu.loyalty_points || 0, totalSpent: Number(lu.total_spent) };
        }
        const payload = {
          type: "new_order",
          ...
          order: {
            ...
            loyaltyDiscount: loyaltyDiscountAmount > 0 ? { pct: loyaltyDiscountPct, amount: loyaltyDiscountAmount } : null,
            status: order.status,
            createdAt: order.created_at,
            linkedAccount: publicUsername || null,
            loyaltyInfo,
            ...
            embeds: [
              {
                ...
                fields: [
                  ...
                  },
                  {
                    name: "💰 Prix",
                    value: (() => {
                      let priceValue = `Prix de base : **${totalPrice.toLocaleString()}$**`;
                      if (loyaltyDiscountAmount > 0) priceValue += `\nRemise fidélité (-${loyaltyDiscountPct}%) : -${loyaltyDiscountAmount.toLocaleString()}$`;
                      if (loyaltyDiscountAmount > 0) priceValue += `\n💵 **Prix final : ${order.total_price.toLocaleString()}$**`;
                      return priceValue;
                    })(),
                    inline: false,
                  },
    ...
    insertAdminNotification(
      "order_new",
      `🛒 Commande de ${firstName} ${lastName}`,
      `${firstName} ${lastName} (ID: ${order.unique_id}) — ${order.total_price.toLocaleString()}$`
    ).catch(() => {});
...
export async function getAllOrders(req: Request, res: Response) {
  try {
    ...
    const orders = status && status !== "all"
      ? await sql`
          SELECT o.*, pu.username AS public_username
          FROM orders o
          LEFT JOIN users pu ON o.public_user_id = pu.id
          WHERE o.status = ${statusValue}
...
export async function updateOrderStatus(req: Request, res: Response) {
  try {
    ...
    // Notify user via WebSocket in real-time
    if (order.public_user_id && (status === "delivered" || status === "cancelled")) {
      try {
        const { broadcastToUser, isUserOnline } = await import("../ws");
        broadcastToUser(order.public_user_id, {
          type: "status_change",
          orderId: order.id,
          status,
          cancellationReason: cancellationReason || null,
        });
        if (!isUserOnline(order.public_user_id)) {
          const notifTitle = status === "delivered" ? "🎉 Commande livrée !" : "❌ Commande annulée";
          ...
          await sql`
            ...
            VALUES (${order.public_user_id}, ${status}, ${notifTitle}, ${notifBody}, ${order.id})
          `.catch(() => {});
        }
      } catch {}
    }

    // Award loyalty points + update total_spent when delivered
    if (status === "delivered" && order.public_user_id) {
      await awardLoyaltyPoints(order.public_user_id, order.id, order.total_price);
      try {
        await sql`UPDATE users SET total_spent = COALESCE(total_spent, 0) + ${order.total_price} WHERE id = ${order.public_user_id}`;
      } catch (e) { console.error("❌ total_spent update:", e); }
      try {
        const { checkAndAwardBadges } = await import("./badges");
        checkAndAwardBadges(order.public_user_id).catch(() => {});
      } catch {}
    }
    // Refund redeemed points when cancelled; reclaim refund if restored to pending
    if (status === "cancelled") {
      await refundLoyaltyOnCancel(order.id);
    } else if (status === "pending") {
      await reclaimLoyaltyOnRestore(order.id);
    }
          ...
          embeds: [
            {
              ...
              fields: [
                ...
                },
                {
                  name: "💰 Prix",
                  value: (() => {
                    const basePrice = orderItems.reduce((sum: number, item: any) => sum + item.vehicle_price * item.quantity, 0);
                    if (basePrice !== order.total_price && basePrice > order.total_price) {
                      const discountAmount = basePrice - order.total_price;
                      const discountPct = Math.round((discountAmount / basePrice) * 100);
                      return `Prix de base : **${basePrice.toLocaleString()}$**\nRemise (-${discountPct}%) : -${discountAmount.toLocaleString()}$\n💵 **Prix final : ${order.total_price.toLocaleString()}$**`;
                    }
