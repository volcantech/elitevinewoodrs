...
export async function createOrder(req: Request, res: Response) {
  try {
    ...
    if (publicUserId) {
      const userCheck = await sql`SELECT is_orders_blocked FROM users WHERE id = ${publicUserId} LIMIT 1`;
    ...
    const [order] = await sql`
      ...
      VALUES (${uniqueId.trim()}, ${firstName}, ${lastName}, ${cleanPhone}, ${totalPrice}, 'pending', ${clientIp}, ${publicUserId})
      RETURNING *
    `;
    ...
    await logActivity(
      ...
      { orderId: order.id, uniqueId: uniqueId.trim(), firstName, lastName, totalPrice, publicUsername },
      null,
      clientIp ?? null
    );
