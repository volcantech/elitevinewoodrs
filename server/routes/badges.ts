...
async function getUserStats(userId: number): Promise<UserStats> {
  const [ordersRow] =
    await sql`SELECT COUNT(*) as count FROM orders WHERE public_user_id = ${userId} AND status = 'delivered'`;
  const [reviewsRow] =
    await sql`SELECT COUNT(*) as count FROM reviews WHERE public_user_id = ${userId}`;
  const [likesRow] = await sql`
    SELECT COUNT(rl.id) as count 
  ...
  let giveawayWins = 0;
  try {
    const [winsRow] = await sql`
      SELECT COUNT(*) as count FROM giveaways 
      WHERE status = 'ended' AND winners_json IS NOT NULL 
      AND winners_json::text LIKE ${'%"user_id":' + userId + "%"}
    `;
    giveawayWins = parseInt(winsRow?.count || "0");
  } catch {}

  const [userRow] =
    await sql`SELECT rp_firstname, rp_lastname, rp_phone, created_at FROM users WHERE id = ${userId}`;
  const profileComplete = !!(
    userRow?.rp_firstname &&
    userRow?.rp_lastname &&
    userRow?.rp_phone
  );
  ...
  const [favsRow] =
    await sql`SELECT COUNT(*) as count FROM favorites WHERE user_id = ${userId}`;

  let referralCount = 0;
  try {
    const [refRow] =
      await sql`SELECT COUNT(*) as count FROM users WHERE referred_by = ${userId}`;
    referralCount = parseInt(refRow?.count || "0");
  } catch {}
...
export async function checkAndAwardBadges(userId: number) {
  try {
    ...
    const [existingBadges] = [
      await sql`SELECT badge_id FROM user_badges WHERE user_id = ${userId}`,
    ];
...
export async function getUserBadges(req: Request, res: Response) {
  try {
    ...
    const earned =
      await sql`SELECT badge_id, unlocked_at FROM user_badges WHERE user_id = ${userId} ORDER BY unlocked_at DESC`;
    const earnedMap = new Map(
      earned.map((b: any) => [b.badge_id, b.unlocked_at]),
    );
