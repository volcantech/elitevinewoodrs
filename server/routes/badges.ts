...
export async function checkAndAwardBadges(userId: number, precomputedStats?: UserStats) {
  try {
    const [stats, existingBadges, userRows] = await Promise.all([
    ...
    const username: string = userRows[0]?.username || `User#${userId}`;
    ...
    for (const badge of newBadges) {
      ...
      insertAdminNotification(
        "badge",
        `🏅 Badge débloqué — ${username}`,
        `${username} a débloqué le badge ${badge.icon} ${badge.name}`
      ).catch(() => {});
    }
