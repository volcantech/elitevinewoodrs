...
export async function enterGiveaway(req: Request, res: Response) {
  try {
    ...
    const [giveaway] = await sql`SELECT id, status, title FROM giveaways WHERE id = ${giveawayId}`;
    ...
    const [profile] = await sql`SELECT unique_id, rp_firstname, rp_lastname, rp_phone FROM users WHERE id = ${userId}`;
    if (!profile) return res.status(404).json({ error: "Utilisateur introuvable" });

    const missing: string[] = [];
    if (!profile.unique_id?.trim()) missing.push("ID unique");
    if (!profile.rp_firstname?.trim()) missing.push("prénom RP");
    if (!profile.rp_lastname?.trim()) missing.push("nom RP");
    if (!profile.rp_phone?.trim()) missing.push("numéro de téléphone RP");

    if (missing.length > 0) {
      return res.status(403).json({
        error: "Profil incomplet",
        missing,
        message: `Pour participer, vous devez renseigner les informations suivantes dans votre profil : ${missing.join(", ")}.`,
      });
    }
    ...
    insertAdminNotification("giveaway_join", `🎁 Inscription giveaway — ${username}`, `${username} vient de s'inscrire : "${giveaway.title}".`).catch(() => {});
...
async function notifyNonWinners(giveawayId: number, giveawayTitle: string, winnerIds: number[]) {
  try {
    const entries = await sql`SELECT DISTINCT user_id FROM giveaway_entries WHERE giveaway_id = ${giveawayId}`;
    const nonWinners = entries.filter((e: any) => !winnerIds.includes(e.user_id));
    const title = `😔 Giveaway terminé — "${giveawayTitle}"`;
    const body = `Vous n'avez pas été sélectionné cette fois. Merci d'avoir participé, bonne chance pour la prochaine fois !`;
    for (const entry of nonWinners) {
      await sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${entry.user_id}, 'giveaway_win', ${title}, ${body})`.catch(() => {});
      broadcastToUser(entry.user_id, { type: "notification", level: "info", title, body });
    }
  } catch (e) {
    console.error("❌ notifyNonWinners:", e);
  }
}
...
export async function adminDrawWinner(req: Request, res: Response) {
  try {
    ...
    await notifyNonWinners(id, giveaway.title, winners.map((w: any) => w.user_id));
...
export async function autoDrawExpiredGiveaways() {
  try {
    ...
    for (const giveaway of rows) {
      ...
      await notifyWinners(winners, giveaway.title);
      await notifyNonWinners(giveaway.id, giveaway.title, winners.map((w: any) => w.user_id));
      await insertAdminNotification("giveaway_auto", `⏰ Tirage automatique — ${giveaway.title}`, `Gagnant(s) : ${winnersStr}`).catch(() => {});
