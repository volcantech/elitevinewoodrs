...
export async function postOrderMessageAdmin(req: Request, res: Response) {
  try {
    ...
    const [order] = await sql`SELECT id, public_user_id FROM orders WHERE id = ${orderId}`;
      ...
      try {
        ...
        if (!isUserOnline(order.public_user_id)) {
          await sql`
            ...
            VALUES (${order.public_user_id}, 'message', ${'💬 Nouveau message de l\'équipe'}, ${`Commande #${orderId} : "${message.trim().slice(0, 100)}${message.trim().length > 100 ? '…' : ''}"`}, ${orderId})
          `.catch(() => {});
        }
      } catch {}
    }
...
export async function postOrderMessagePublic(req: Request, res: Response) {
  try {
    ...
    const rows = await sql`
      ...
      RETURNING id, order_id, sender_type, sender_id, sender_username, message, created_at
    `;
    insertAdminNotification("order_message", `💬 Message de ${payload.username} dans la commande #${orderId}`, `${message.trim().slice(0, 100)}${message.trim().length > 100 ? "…" : ""}`).catch(() => {});
    res.json({ message: rows[0] });
  } catch (error) {
