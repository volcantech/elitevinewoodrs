...
export async function initFriendsTables() {
  try {
    ...
    try {
      await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS profiles_visited INTEGER NOT NULL DEFAULT 0`;
    } catch {}
    try {
...
export async function getPublicProfile(req: Request, res: Response) {
  ...
  try {
    ...
    const [user] = await sql`
      SELECT id, username, unique_id, avatar_url, created_at, last_seen_at,
             COALESCE(bio, '') AS bio,
    ...
    if (!myId || myId !== targetId) {
      ...
      if (myId) {
        sql`UPDATE users SET profiles_visited = COALESCE(profiles_visited, 0) + 1 WHERE id = ${myId}`.catch(() => {});
      }
    }
    ...
    let friendshipCreatedAt: string | null = null;
    let iBlockedThem = false;
    let theyBlockedMe = false;

    if (myId && myId !== targetId) {
      const [fs] = await sql`
        SELECT id, requester_id, status, created_at FROM friendships
      ...
      if (fs) {
        ...
        friendshipCreatedAt = fs.created_at || null;
      }
    ...
    res.json({
      ...
      lastSeen: user.last_seen_at ? new Date(user.last_seen_at).toISOString() : (getLastSeen(targetId)?.toISOString() || null),
      ...
      friendshipStatus,
      friendshipRequester,
      friendsSince: friendshipStatus === 'accepted' ? friendshipCreatedAt : null,
      iBlockedThem,
      theyBlockedMe,
    });
  } catch (error) {
  ...
  try {
    ...
    if (!isUserOnline(targetId)) {
      await sql`
        ...
        VALUES (${targetId}, 'friend_request', ${`👤 ${me.username}`}, ${'vous a envoyé une demande d\'ami.'})
      `.catch(() => {});
    }
...
export async function getFriends(req: Request, res: Response) {
  ...
  try {
    const rows = await sql`
      SELECT u.id, u.username, u.unique_id, u.avatar_url, u.last_seen_at,
