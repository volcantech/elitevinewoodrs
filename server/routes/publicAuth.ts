...
export async function publicRegister(req: Request, res: Response) {
  try {
    ...
    insertAdminNotification("account_new", `👤 Nouveau compte — ${cleanUsername}`, `Inscription de "${cleanUsername}" (ID unique : ${cleanUniqueId}).`).catch(() => {});
...
export async function publicLogin(req: Request, res: Response) {
  try {
    ...
    if (!valid) {
      const attempts = (loginAttempts.get(user.id) || 0) + 1;
      loginAttempts.set(user.id, attempts);
      if (attempts === 3 || (attempts > 3 && attempts % 5 === 0)) {
        const notifTitle = "🔐 Tentatives de connexion suspectes";
        const notifBody = `${attempts} tentative${attempts > 1 ? "s" : ""} de connexion avec un mauvais mot de passe ont été détectées sur votre compte.`;
        await sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${user.id}, 'report', ${notifTitle}, ${notifBody})`.catch(() => {});
        try {
          const { broadcastToUser } = await import("../ws");
          broadcastToUser(user.id, { type: "notification", level: "error", title: notifTitle, body: notifBody });
        } catch {}
      }
      return res.status(401).json({ error: "❌ Identifiants incorrects" });
    }
...
export async function publicMe(req: Request, res: Response) {
  try {
    ...
    const rows = await sql`SELECT id, username, unique_id, is_banned, is_admin, is_calls_blocked, COALESCE(is_messages_blocked, FALSE) AS is_messages_blocked, COALESCE(is_images_blocked, FALSE) AS is_images_blocked, created_at, rp_phone, rp_firstname, rp_lastname, avatar_url, COALESCE(banner_color, 'amber') AS banner_color, banner_url, COALESCE(total_spent, 0) AS total_spent FROM users WHERE id = ${payload.userId} AND password_hash IS NOT NULL LIMIT 1`;
...
export async function publicUpdateProfile(req: Request, res: Response) {
  try {
    ...
    const profileIsComplete = !!(updated.unique_id && updated.rp_firstname && updated.rp_lastname && updated.rp_phone);
    insertAdminNotification(
      "profile_update",
      `✏️ Profil mis à jour — ${updated.username}`,
      profileIsComplete
        ? `${updated.username} a complété son profil RP (ID: ${updated.unique_id}).`
        : `${updated.username} a modifié son profil RP.`
    ).catch(() => {});
...
export async function publicCancelOrder(req: Request, res: Response) {
  try {
    ...
    insertAdminNotification(
      "order_cancel",
      `❌ Commande annulée — ${payload.username}`,
      `${payload.username} a annulé sa commande #${orderId}.`
    ).catch(() => {});
  ...
  try {
    const users = await sql`
      SELECT u.id, u.username, u.unique_id, u.is_banned, u.is_admin, u.permissions, u.created_at, u.avatar_url,
             u.banner_url, COALESCE(u.banner_color, 'amber') AS banner_color,
             ...
             COALESCE(u.is_messages_blocked, FALSE) AS is_messages_blocked,
             COALESCE(u.is_calls_blocked, FALSE) AS is_calls_blocked,
             COALESCE(u.is_images_blocked, FALSE) AS is_images_blocked,
...
export async function adminEditPublicUser(req: Request, res: Response) {
  try {
    ...
    const { username, unique_id, password, is_orders_blocked, is_reviews_blocked, is_messages_blocked, is_calls_blocked, is_images_blocked, rp_phone, rp_firstname, rp_lastname, clear_avatar, clear_banner } = req.body;

    const rows = await sql`SELECT id, username, unique_id, is_orders_blocked, is_reviews_blocked, is_messages_blocked, is_calls_blocked, COALESCE(is_images_blocked, FALSE) AS is_images_blocked, rp_phone, rp_firstname, rp_lastname, avatar_url, banner_url FROM users WHERE id = ${userId} AND password_hash IS NOT NULL LIMIT 1`;
    if (rows.length === 0) return res.status(404).json({ error: "Compte introuvable" });
    const user = rows[0];
    ...
    const newMessagesBlocked = is_messages_blocked !== undefined ? is_messages_blocked : user.is_messages_blocked;
    const newCallsBlocked = is_calls_blocked !== undefined ? is_calls_blocked : user.is_calls_blocked;
    // Blocking messages automatically blocks images too
    const rawImagesBlocked = is_images_blocked !== undefined ? is_images_blocked : user.is_images_blocked;
    const newImagesBlocked = newMessagesBlocked ? true : rawImagesBlocked;
    ...
    const newBannerUrl = clear_banner === true ? null : user.banner_url;
    ...
    if (newPasswordHash) {
      await sql`
        UPDATE users SET username = ${cleanUsername}, unique_id = ${cleanUniqueId},
          ...
          is_reviews_blocked = ${newReviewsBlocked}, is_messages_blocked = ${newMessagesBlocked},
          is_calls_blocked = ${newCallsBlocked}, is_images_blocked = ${newImagesBlocked},
          rp_phone = ${newRpPhone}, rp_firstname = ${newRpFirstname}, rp_lastname = ${newRpLastname},
          avatar_url = ${newAvatarUrl}, banner_url = ${newBannerUrl}, updated_at = CURRENT_TIMESTAMP
        WHERE id = ${userId}
      `;
    } else {
      await sql`
        UPDATE users SET username = ${cleanUsername}, unique_id = ${cleanUniqueId},
          ...
          is_messages_blocked = ${newMessagesBlocked}, is_calls_blocked = ${newCallsBlocked},
          is_images_blocked = ${newImagesBlocked},
          rp_phone = ${newRpPhone}, rp_firstname = ${newRpFirstname}, rp_lastname = ${newRpLastname},
          avatar_url = ${newAvatarUrl}, banner_url = ${newBannerUrl}, updated_at = CURRENT_TIMESTAMP
        WHERE id = ${userId}
      `;
    ...
    if (newMessagesBlocked !== user.is_messages_blocked) changes["Messages privés bloqués"] = { old: user.is_messages_blocked ? "Oui" : "Non", new: newMessagesBlocked ? "Oui" : "Non" };
    if (newCallsBlocked !== user.is_calls_blocked) changes["Appels bloqués"] = { old: user.is_calls_blocked ? "Oui" : "Non", new: newCallsBlocked ? "Oui" : "Non" };
    ...
    if (clear_banner === true && user.banner_url) changes["Bannière"] = { old: "Présente", new: "Supprimée" };
    ...
    const [updated] = await sql`SELECT id, username, unique_id, is_banned, is_admin, is_orders_blocked, is_reviews_blocked, is_messages_blocked, is_calls_blocked, COALESCE(is_images_blocked, FALSE) AS is_images_blocked, created_at FROM users WHERE id = ${userId}`;

    // Notify user via WS if any restriction changed
    const restrictionChanges: string[] = [];
    if (newOrdersBlocked !== user.is_orders_blocked) restrictionChanges.push(newOrdersBlocked ? "❌ Commandes bloquées" : "✅ Commandes débloquées");
    if (newReviewsBlocked !== user.is_reviews_blocked) restrictionChanges.push(newReviewsBlocked ? "❌ Avis bloqués" : "✅ Avis débloqués");
    if (newMessagesBlocked !== user.is_messages_blocked) restrictionChanges.push(newMessagesBlocked ? "❌ Envoi de messages bloqué" : "✅ Envoi de messages débloqué");
    if (newImagesBlocked !== user.is_images_blocked) restrictionChanges.push(newImagesBlocked ? "❌ Envoi d'images bloqué" : "✅ Envoi d'images débloqué");
    if (newCallsBlocked !== user.is_calls_blocked) restrictionChanges.push(newCallsBlocked ? "❌ Appels vocaux bloqués" : "✅ Appels vocaux débloqués");
    if (restrictionChanges.length > 0) {
      try {
        const { broadcastToUser, isUserOnline } = await import("../ws");
        broadcastToUser(userId, {
          type: "notification",
          level: "error",
          title: "⚠️ Restriction de compte",
          body: restrictionChanges.join(" · "),
        });
        if (!isUserOnline(userId)) {
          await sql`
            INSERT INTO user_notifications (public_user_id, type, title, body)
            VALUES (${userId}, 'report', ${'⚠️ Restriction de compte'}, ${restrictionChanges.join(" · ")})
          `.catch(() => {});
        }
      } catch {}
    }
...
export async function publicUploadAvatar(req: Request, res: Response) {
  try {
    ...
    insertAdminNotification("avatar_update", `📷 Photo de profil mise à jour — ${payload.username}`, `Le joueur "${payload.username}" a mis à jour sa photo de profil.`).catch(() => {});

    const { checkAndAwardCustomBadges } = await import("./customBadges");
    checkAndAwardCustomBadges(payload.userId, payload.username).catch(() => {});
