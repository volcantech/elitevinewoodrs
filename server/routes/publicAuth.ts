...
export async function adminEditPublicUser(req: Request, res: Response) {
  try {
    ...
    const { username, unique_id, password, is_orders_blocked, is_reviews_blocked, is_messages_blocked, is_calls_blocked, rp_phone, rp_firstname, rp_lastname, clear_avatar, clear_banner } = req.body;

    const rows = await sql`SELECT id, username, unique_id, is_orders_blocked, is_reviews_blocked, is_messages_blocked, is_calls_blocked, rp_phone, rp_firstname, rp_lastname, avatar_url, banner_url FROM users WHERE id = ${userId} AND password_hash IS NOT NULL LIMIT 1`;
    if (rows.length === 0) return res.status(404).json({ error: "Compte introuvable" });
    const user = rows[0];
    ...
    if (newPasswordHash) {
      await sql`
        UPDATE users SET username = ${cleanUsername}, unique_id = ${cleanUniqueId},
          ...
          is_calls_blocked = ${newCallsBlocked},
    ...
    const [updated] = await sql`SELECT id, username, unique_id, is_banned, is_admin, is_orders_blocked, is_reviews_blocked, is_messages_blocked, is_calls_blocked, created_at FROM users WHERE id = ${userId}`;
