...
export async function publicUploadAvatar(req: Request, res: Response) {
  try {
    ...
    const [existing] = await sql`SELECT avatar_url FROM users WHERE id = ${payload.userId} LIMIT 1`;
    if (existing?.avatar_url) {
      const oldFile = path.join(avatarsDir, path.basename(existing.avatar_url));
