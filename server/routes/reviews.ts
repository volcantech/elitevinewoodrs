...
export async function getReviewsByVehicle(req: Request, res: Response) {
  try {
    ...
    const reviews = await sql`
      SELECT r.id, r.pseudo, r.rating, r.comment, r.created_at, r.updated_at,
             r.public_user_id, u.avatar_url
      FROM reviews r
...
export async function deleteReview(req: Request, res: Response) {
  try {
    ...
    const existing = await sql`
      SELECT r.id, r.pseudo, r.comment, r.public_user_id, v.name AS vehicle_name
      FROM reviews r
      LEFT JOIN vehicles v ON r.vehicle_id = v.id
      WHERE r.id = ${reviewId}
    `;
    ...
    if (deletedReview.public_user_id) {
      const notifTitle = "🗑️ Votre avis a été supprimé";
      const notifBody = deletedReview.vehicle_name
        ? `Votre avis sur ${deletedReview.vehicle_name} a été supprimé par l'équipe de modération.`
        : "Votre avis a été supprimé par l'équipe de modération.";
      broadcastToUser(deletedReview.public_user_id, { type: "notification", level: "error", title: notifTitle, body: notifBody });
      await sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${deletedReview.public_user_id}, 'report', ${notifTitle}, ${notifBody})`.catch(() => {});
    }
...
export async function updateReview(req: Request, res: Response) {
  try {
    ...
    const [existing] = await sql`
      SELECT r.id, r.rating, r.comment, r.pseudo, r.public_user_id, v.name AS vehicle_name
    ...
    if (existing.public_user_id) {
      const notifTitle = "✏️ Votre avis a été modifié";
      const notifBody = existing.vehicle_name
        ? `Votre avis sur ${existing.vehicle_name} a été modifié par l'équipe de modération.`
        : "Votre avis a été modifié par l'équipe de modération.";
      broadcastToUser(existing.public_user_id, { type: "notification", level: "info", title: notifTitle, body: notifBody });
      await sql`INSERT INTO user_notifications (public_user_id, type, title, body) VALUES (${existing.public_user_id}, 'report', ${notifTitle}, ${notifBody})`.catch(() => {});
    }
...
export async function editMyReview(req: Request, res: Response) {
  try {
    const publicUserId = (req as any).publicUser?.userId;
    if (!publicUserId) return res.status(401).json({ error: "Non authentifié" });

    const reviewId = parseInt(req.params.id);
    if (isNaN(reviewId)) return res.status(400).json({ error: "ID invalide" });

    const userCheck = await sql`SELECT is_reviews_blocked, username FROM users WHERE id = ${publicUserId} LIMIT 1`;
    if (userCheck.length === 0) return res.status(403).json({ error: "Compte introuvable" });
    if (userCheck[0].is_reviews_blocked) return res.status(403).json({ error: "❌ Votre accès aux avis a été bloqué." });
    ...
    `;
    if (!existing) return res.status(404).json({ error: "Avis introuvable" });
    if (Number(existing.public_user_id) !== Number(publicUserId)) {
    ...
    await sql`
      UPDATE reviews SET rating = ${parsedRating}, comment = ${cleanComment}, updated_at = NOW()
      WHERE id = ${reviewId}
    `;

    const username = userCheck[0].username as string;

    const changes: Record<string, { old: any; new: any }> = {};
    if (parsedRating !== existing.rating) changes["Note"] = { old: `${existing.rating}/5 ★`, new: `${parsedRating}/5 ★` };
    if (cleanComment !== existing.comment) changes["Commentaire"] = { old: existing.comment ?? "—", new: cleanComment ?? "—" };
    ...
    await logActivity(
      ...
      "Avis joueur",
      username,
      `Avis #${reviewId} modifié par ${username} sur ${existing.vehicle_name ?? "?"}`,
      changes,
      null,
      req.ip ?? null
    );
    ...
    insertAdminNotification(
      "review_update",
      `✏️ Avis modifié — ${username}`,
      `${username} a modifié son avis${existing.vehicle_name ? ` sur ${existing.vehicle_name}` : ""} (${parsedRating}★${cleanComment ? ` : "${cleanComment.slice(0, 80)}${cleanComment.length > 80 ? "…" : ""}"` : ""})`
    ).catch(() => {});

    res.json({ success: true, rating: parsedRating, comment: cleanComment, updated_at: new Date().toISOString() });
  } catch (error) {
    console.error("❌ editMyReview:", error);
    res.status(500).json({ error: "Erreur serveur" });
  }
}
