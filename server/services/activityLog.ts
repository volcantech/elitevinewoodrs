...
export async function initActivityLogsTable() {
  try {
    ...
    // Error logged Error initializing activity logs table:", error);
  }
}
...
export async function addIpColumnIfMissing() {
  try {
    ...
    // Error logged⚠️ IP column might already exist:", error);
  }
}
  ...
  try {
    ...
    if (!uniqueId && adminId) {
      try {
        ...
        // Error logged⚠️ Error fetching admin unique_id:", dbError);
      }
    }
    ...
    // Error logged Erreur enregistrement activity log:", error);
  }
}
...
export async function getAllActivityLogs() {
  try {
    ...
    // Error logged Erreur lecture activity logs:", error);
    return [];
  }
}
...
export async function getActivityLogsPaginated(page: number = 1, pageSize: number = 25) {
  try {
    ...
    // Error logged Erreur lecture activity logs paginated:", error);
