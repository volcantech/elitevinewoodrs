    ...
    if (!uniqueId && adminId) {
      try {
        const result = await sql`
          SELECT unique_id FROM users WHERE id = ${adminId} LIMIT 1
        `;
        uniqueId = result[0]?.unique_id || null;
      } catch (dbError) {
        console.error("⚠️ Impossible de récupérer l'ID unique de l'admin:", dbError);
      }
    }
...
export async function getActivityLogsPaginated(
  ...
  try {
    ...
    const logs = await sql`
      SELECT al.*, u.avatar_url AS admin_avatar_url
      FROM activity_logs al
      LEFT JOIN users u ON u.id = al.admin_id
      WHERE (
        ${searchPattern}::text IS NULL
        OR al.description ILIKE ${searchPattern}
        OR al.resource_name ILIKE ${searchPattern}
        OR al.admin_username ILIKE ${searchPattern}
      )
      AND (${actionVal}::text IS NULL OR al.action = ${actionVal})
      AND (${resourceTypeVal}::text IS NULL OR al.resource_type = ${resourceTypeVal})
    ...
    const countResult = await sql`
      ...
      WHERE (
        ...
        OR resource_name ILIKE ${searchPattern}
        OR admin_username ILIKE ${searchPattern}
      )
      AND (${actionVal}::text IS NULL OR action = ${actionVal})
      AND (${resourceTypeVal}::text IS NULL OR resource_type = ${resourceTypeVal})
    `;
